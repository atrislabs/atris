...
test('mission goal recovers a matching usage-limited native goal without duplicate create', () => {
  ...
  try {
    ...
    const run = runCli(['mission', 'run', 'usage limited recovery', '--json'], { cwd: dir });
    assert.equal(run.status, 0, run.stderr || run.stdout);
...
test('mission tick prints usage-limited recovery instead of native goal create', () => {
  ...
  try {
    ...
    const run = runCli(['mission', 'run', 'tick usage limited recovery', '--json'], { cwd: dir });
    assert.equal(run.status, 0, run.stderr || run.stdout);
    const mission = JSON.parse(run.stdout).mission;
...
test('mission help documents status filters', () => {
  ...
  try {
    ...
    assert.match(help.stdout, /mission goal \[--runtime codex\|atris\] \[--heartbeat\] \[--native-goal-status active\|paused\|usageLimited\] \[--native-goal-objective "\.\.\."\] \[--allow-native-goal-supersede\] \[--json\]/);
