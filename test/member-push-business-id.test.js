...
test('member push includes the workspace business_id as a form field', async () => {
  ...
  try {
    ...
    assert.equal(requests[0].url, '/api/agent/import-member?business_id=biz-roster');
    assert.equal(requests[0].contentType, 'text/markdown');
    assert.equal(requests[0].body, memberContent);
