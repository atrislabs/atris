...
test('digest acceptance history is not capped by the compact done-task projection', () => {
  ...
  try {
    ...
    assert.equal((digest.stdout.match(/^  - /gm) || []).length, 8);
    assert.match(digest.stdout, /^  - Accepted result 1 saves operator time, so the daily count stays trustworthy\.$/m);
    assert.match(digest.stdout, /^  - Title-only result lets operators inspect accepted work, so the daily story stays complete\.$/m);
    assert.doesNotMatch(digest.stdout, /^  REP-\d+/m);
  } finally {
    taskDb.close(db);
    cleanupTempDir(base);
  }
...
test('landing sweep escalates 48h-stale active work into the digest', () => {
  ...
  try {
    ...
    const plural = autoland.composeDigest({
      ...
      landed: null,
      project: 'demo',
      landingSweep: { stale_count: 2, stale: sweep.stale },
    });
    assert.match(plural, /2 pieces need a human/);
    assert.doesNotMatch(plural, /2 pieces needs/);
  } finally {
    cleanupTempDir(base);
  }
});
