...
test('default atris task is short and names the same next as bare atris', () => {
  ...
  try {
    ...
    assert.equal(nextLine(task.stdout), 'atris task ready CLI-9 --verify "git diff --check"');
    assert.ok(spokenLineCount(task.stdout) <= 4);
...
test('task desk and task --all still print the long desk', () => {
  ...
  try {
    ...
    assert.equal(nextLine(desk.stdout), 'atris task ready CLI-9 --verify "git diff --check"');
    assert.equal(nextLine(all.stdout), 'atris task ready CLI-9 --verify "git diff --check"');
  } finally {
    cleanupTempDir(dir);
  }
});
