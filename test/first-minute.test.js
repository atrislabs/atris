...
function keepWorkingNext(ref) {
  return taskCommand({ status: 'claimed', display_id: ref });
}

function assertKeepWorkingNext(stdout, ref) {
  const next = nextLine(stdout);
  if (ref) {
    assert.equal(next, keepWorkingNext(ref));
    return next;
  }
  assert.match(next, /^atris task ready \S+ --verify "git diff --check"$/);
  assert.notEqual(next, 'atris do');
  return next;
}
...
test('keep-working minute is claimed ready, and the win line drops the greeting', () => {
  assert.equal(isKeepWorkingMinute({ nextCommand: 'atris do' }), true);
  assert.equal(isKeepWorkingMinute({
    nextCommand: 'atris task ready CLI-9 --verify "git diff --check"',
  }), true);
...
test('claimed task first-minute names the person or title and one next command', () => {
  const text = renderWorkspace({
    ...
    nextCommand: keepWorkingNext('CLI-9'),
  });
...
test('desk next command uses first-minute verbs without ready templates', () => {
  ...
  assert.equal(taskCommand({ status: 'claimed', display_id: 'LDY-1' }, 'keshav'), keepWorkingNext('LDY-1'));
...
test('buildFirstMinute reads a claimed task from the local projection', () => {
  ...
  try {
    ...
    assert.equal(screen.nextCommand, keepWorkingNext('CLI-9'));
  } finally {
    cleanupTempDir(dir);
  }
});
...
test('workspace with a claimed task names the person or title and one next command', () => {
  ...
  try {
    ...
    assert.match(res.stdout, /^next: atris task ready CLI-9 --verify "git diff --check"$/m);
...
test('first talk files the user sentence and names it as the win', () => {
  ...
  try {
    ...
    assertKeepWorkingNext(talk.stdout);
    ...
    assert.equal(nextLine(doit.stdout), keepWorking);
...
test('unbound dirty git folder names open work and does not mint', () => {
  ...
  try {
    ...
    assert.equal(after.status, 0, after.stderr || after.stdout);
    assert.match(after.stdout, /"a notes app for keshav" is already yours/);
    assertKeepWorkingNext(after.stdout);
...
test('atris do in an unbound folder starts from files already here', () => {
  ...
  try {
    ...
    assert.equal(nextLine(after.stdout), keepWorking);
    assert.notEqual(nextLine(after.stdout), 'atris do');
...
test('atris do in an unbound folder starts from a few files with one title', () => {
  ...
  try {
    ...
    try {
      ...
      assertKeepWorkingNext(manyDo.stdout);
...
test('atris launchpad in an empty folder talks first-talk, not factory init', () => {
  const dir = makeTempDir();
  const home = path.join(dir, 'home');
  fs.mkdirSync(home, { recursive: true });
  const env = { HOME: home, USER: 'keshav', ATRIS_TASKS_DB: path.join(dir, 'tasks.db') };
  try {
    ...
    assert.equal(minute.status, 0, minute.stderr || minute.stdout);
    assert.equal(now.status, 0, now.stderr || now.stdout);
    assert.equal(pad.status, 0, pad.stderr || pad.stdout);
    ...
    assert.equal(spokenLineCount(pad.stdout), spokenLineCount(minute.stdout));
    assert.doesNotMatch(
      pad.stdout + pad.stderr,
      /Run "atris init"|atris init|Set up this workspace|WHAT HAPPENED|WHAT TO WORK ON NEXT|No waiting work was found|folder not found|init --minimal|claim|\+---/,
    );
    assert.equal(fs.existsSync(path.join(dir, 'atris')), false);
    assert.equal(fs.existsSync(path.join(dir, '.atris')), false);

    const jsonPad = runCli(['launchpad', '--json'], { cwd: dir, env });
    const jsonMinute = runCli(['--json'], { cwd: dir, env });
    assert.equal(jsonPad.status, jsonMinute.status);
    const payload = JSON.parse(jsonPad.stdout);
    assert.equal(payload.reason, 'this folder is empty');
    assert.equal(payload.next_action, 'atris "what do you want here?"');
    assert.doesNotMatch(jsonPad.stdout, /folder not found|init|claim|atris\.launchpad\.v1|bootstrap/);
    assert.equal(fs.existsSync(path.join(dir, 'atris')), false);
    assert.equal(fs.existsSync(path.join(dir, '.atris')), false);

    const help = runCli(['launchpad', '--help'], { cwd: dir, env });
    assert.equal(help.status, 0, help.stderr || help.stdout);
    assert.match(help.stdout, /Usage: atris launchpad/);
    assert.doesNotMatch(help.stdout, /this folder is empty|Run "atris init"|Set up this workspace|WHAT HAPPENED/);
    assert.equal(fs.existsSync(path.join(dir, 'atris')), false);
    assert.equal(fs.existsSync(path.join(dir, '.atris')), false);
  } finally {
    cleanupTempDir(dir);
...
test('atris launchpad in a folder with a file names the file, not init', () => {
  const dir = makeTempDir();
  const home = path.join(dir, 'home');
  fs.mkdirSync(home, { recursive: true });
  fs.writeFileSync(path.join(dir, 'notes.txt'), 'already writing\n', 'utf8');
  ...
  try {
    ...
    assert.doesNotMatch(
      pad.stdout + pad.stderr,
      /Run "atris init"|atris init|Set up this workspace|WHAT HAPPENED|WHAT TO WORK ON NEXT|No waiting work was found|this folder is empty|what do you want here|nothing is running|claim|\+---/,
    );
    assert.equal(fs.existsSync(path.join(dir, 'atris')), false);
    assert.equal(fs.existsSync(path.join(dir, '.atris')), false);
    ...
    assert.doesNotMatch(jsonPad.stdout, /folder not found|init|claim|atris\.launchpad\.v1|bootstrap|what do you want here/);
    assert.equal(fs.existsSync(path.join(dir, 'atris')), false);
    assert.equal(fs.existsSync(path.join(dir, '.atris')), false);
...
test('atris launchpad after minting a file folder talks keep-working, not factory', () => {
  const dir = makeTempDir();
  const home = path.join(dir, 'home');
  fs.mkdirSync(home, { recursive: true });
  fs.writeFileSync(path.join(dir, 'notes.md'), 'already writing\n', 'utf8');
  ...
  try {
    ...
    assert.equal(pad.status, 0, pad.stderr || pad.stdout);
    assert.equal(pad.stdout.trim(), minute.stdout.trim());
    assert.equal(pad.stdout.trim(), again.stdout.trim());
    assert.equal(pad.stdout.trim(), status.stdout.trim());
    assert.equal(pad.stdout.trim(), now.stdout.trim());
    assert.match(pad.stdout, /^hey keshav, "notes\.md" is already yours\.$/m);
    assert.equal(nextLine(pad.stdout), keepWorking);
    assert.notEqual(nextLine(pad.stdout), 'atris do');
    assert.equal(spokenLineCount(pad.stdout), 2);
    assert.doesNotMatch(
      pad.stdout + pad.stderr,
      /Where we are|Decision: let it run|Generate MAP\.md|TASK BOARD|Set up this workspace|WHAT HAPPENED|RUN THIS|atris init|nothing is running|what do you want here|ready to claim|# now|\+---/,
    );

    const jsonPad = runCli(['launchpad', '--json'], { cwd: dir, env });
    assert.equal(jsonPad.status, 0, jsonPad.stderr || jsonPad.stdout);
    const payload = JSON.parse(jsonPad.stdout);
    assert.equal(payload.next_action, keepWorking);
    assert.equal(payload.reason, '"notes.md" is already yours');
    assert.doesNotMatch(jsonPad.stdout, /Where we are|let it run|Generate MAP|atris\.launchpad\.v1|bootstrap|atris init/);
  } finally {
    cleanupTempDir(dir);
  }
});

test('atris launchpad after init --yes --minimal talks the claim, not factory init', () => {
  const dir = makeTempDir();
  const home = path.join(dir, 'home');
  fs.mkdirSync(home, { recursive: true });
  fs.writeFileSync(path.join(dir, 'notes.txt'), 'already writing\n', 'utf8');
  ...
  try {
    ...
    assert.equal(pad.stdout.trim(), now.stdout.trim());
    assert.equal(pad.stdout.trim(), status.stdout.trim());
    assert.match(pad.stdout, /generate map\.md/i);
    assert.match(pad.stdout, /ready to claim/);
    assert.match(pad.stdout, /^next: atris task claim /m);
    assert.equal(nextLine(pad.stdout), nextLine(minute.stdout));
    assert.match(nextLine(pad.stdout), /^atris task claim \S+ --as \S+$/);
    assert.equal(spokenLineCount(pad.stdout), spokenLineCount(minute.stdout));
    assert.equal(spokenLineCount(pad.stdout), 2);
    assert.doesNotMatch(
      pad.stdout + pad.stderr,
      /Where we are|Decision: let it run|TASK BOARD|What is queued|What is blocking|Set up this workspace|WHAT HAPPENED|RUN THIS|atris init|nothing is running|what do you want here|already yours|# now|\+---/,
    );
    assert.ok(fs.existsSync(path.join(dir, 'atris', 'MAP.md')));
    assert.equal(fs.existsSync(path.join(dir, '.atris', 'business.json')), false);
    ...
    const live = runCli(['launchpad'], { cwd: dir, env });
    assert.equal(live.status, 0, live.stderr || live.stdout);
    assert.match(live.stdout, /WHAT HAPPENED|RUN THIS|Live job|New change ready to start|No clear next move/);
    assert.doesNotMatch(live.stdout, /ready to claim|Set up this workspace|atris init/);
  } finally {
    cleanupTempDir(dir);
  }
});
...
test('atris status after minting a file folder talks keep-working, not factory let-it-run', () => {
  ...
  try {
    ...
    assert.equal(recapPayload.next_action, keepWorking);
    ...
    assert.equal(nowPayload.next, keepWorking);
...
test('atris stop after init --yes --minimal talks the claim, not cloud-computer', () => {
  const dir = makeTempDir();
  const home = path.join(dir, 'home');
  fs.mkdirSync(home, { recursive: true });
  fs.writeFileSync(path.join(dir, 'notes.txt'), 'already writing\n', 'utf8');
  ...
  try {
    ...
    assert.equal(stopped.stdout.trim(), status.stdout.trim());
    assert.match(stopped.stdout, /generate map\.md/i);
    assert.match(stopped.stdout, /ready to claim/);
    assert.match(stopped.stdout, /^next: atris task claim /m);
    assert.equal(nextLine(stopped.stdout), nextLine(minute.stdout));
    assert.match(nextLine(stopped.stdout), /^atris task claim \S+ --as \S+$/);
    assert.equal(spokenLineCount(stopped.stdout), spokenLineCount(minute.stdout));
    assert.equal(spokenLineCount(stopped.stdout), 2);
    assert.doesNotMatch(
      stopped.stdout + stopped.stderr,
      /cloud-computer|business\.json|Pass --mission|Atris left your work unchanged|Where we are|Decision: let it run|TASK BOARD|nothing is running|what do you want here|already yours|# now|Current operating truth|spaceship|autopilot|hours/,
    );
    assert.equal(fs.existsSync(path.join(dir, '.atris', 'business.json')), false);
    assert.ok(fs.existsSync(path.join(dir, 'atris', 'MAP.md')));
    ...
    const jsonStop = runCli(['stop', '--json'], { cwd: dir, env });
    assert.equal(jsonStop.status, 0, jsonStop.stderr || jsonStop.stdout);
    assert.match(jsonStop.stdout, /ready to claim/);
    assert.match(jsonStop.stdout, /^next: atris task claim /m);
    assert.doesNotMatch(jsonStop.stdout + jsonStop.stderr, /cloud-computer|business\.json|Pass --mission|Atris left your work unchanged/);
    assert.equal(fs.existsSync(path.join(dir, '.atris', 'business.json')), false);

    const help = runCli(['stop', '--help'], { cwd: dir, env });
    assert.equal(help.status, 0, help.stderr || help.stdout);
    assert.match(help.stdout, /Usage: atris stop/);
    assert.doesNotMatch(help.stdout, /ready to claim|cloud-computer|nothing is running|clean start/);
  } finally {
    cleanupTempDir(dir);
  }
});
...
test('atris test after init --minimal talks like first-minute, not bootstrap', () => {
  ...
  try {
    ...
    assertKeepWorkingNext(afterVerb.stdout);
    assert.notEqual(nextLine(afterVerb.stdout), 'atris do');
    ...
    assert.equal(nextLine(missing.stdout), nextLine(afterVerb.stdout));
    assertKeepWorkingNext(missing.stdout);
    ...
    assert.equal(json.status, 2, json.stderr || json.stdout);
    const payload = JSON.parse(json.stdout);
    assert.equal(payload.next_action, nextLine(afterVerb.stdout));
    assert.notEqual(payload.next_action, 'atris do');
...
test('named empty folder next command starts a first task when pasted', () => {
  ...
  try {
    ...
    const keepWorking = assertKeepWorkingNext(pasted.stdout);
    assert.doesNotMatch(pasted.stdout.split('\n')[0], /[A-Z0-9]+-\d+/);
    ...
    const shown = runCli(['do'], { cwd: dir, env });
    assert.equal(shown.status, 0, shown.stderr || shown.stdout);
    assert.match(shown.stdout, /already yours/);
    assert.equal(nextLine(shown.stdout), keepWorking);
    ...
    assert.equal(nextLine(afterTask.stdout), keepWorking);
