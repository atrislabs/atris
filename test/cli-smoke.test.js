...
test('atris.md boot visualization does not create a task DB just to count tasks', () => {
  ...
  try {
    ...
    assert.equal(res.status, 0, res.stderr);
    assert.match(res.stdout, /tasks\s+1 waiting to start/);
...
test('atris.md boot visualization prefers task DB counts over stale TODO rows', () => {
  ...
  try {
    ...
    const res = runCli(['atris.md'], { cwd: dir, env: { ATRIS_TASKS_DB: dbPath } });
    assert.equal(res.status, 0, res.stderr);
    assert.match(res.stdout, /tasks\s+1 being worked on/);
    assert.doesNotMatch(res.stdout, /1 waiting to start/);
  } finally {
    taskDb.close();
...
test('runner-profile help lists canonical profile names only', () => {
  ...
  try {
    ...
    assert.match(run.stdout, /--runner-profile NAME\s+Runner profile for this run \(one of: atris-fast, claude, codex, cursor, fable, composer, haiku, devin, grok, hermes, droid\)/);
    ...
    assert.match(autopilot.stdout, /--runner-profile NAME\s+Runner profile for this run \(one of: atris-fast, claude, codex, cursor, fable, composer, haiku, devin, grok, hermes, droid\)/);
...
test('runner-profile validation fails fast while aliases stay compatible', () => {
  ...
  try {
    ...
    for (const profile of ['atris-fast', 'atris2-fast', 'atris-2-fast']) {
      const good = runCli(['run', '--legacy', '--dry-run', '--runner-profile', profile], { cwd: dir });
