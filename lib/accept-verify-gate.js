...
function evaluateAcceptVerify(task, workspaceRoot, { cache = null } = {}) {
  ...
  if (!parsed.ok) {
    return {
      ok: false,
      reason: parsed.reason || 'verify_command_not_allowed',
      detail: `the stored verify command did not parse as a runnable, allow-listed command (${parsed.reason || 'unknown'})`,
      command,
      ran: false,
    };
  }
