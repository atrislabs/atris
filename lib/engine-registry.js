...
function rosterPickFromValue(value, role, options = {}) {
  ...
  for (const part of extras) {
    ...
    const maxMatch = /^max\b/i.test(part);
...
// The words a written line uses for one engine, model, and effort: the model
// alone when it already says the engine ("opus 5.5"), else engine then model,
// then the effort word.
function rosterWordsFor(engine, model, effort = '') {
  const tail = effort ? ` ${effort}` : '';
  if (!model) return `${engine}${tail}`;
  const label = rosterModelLabel(model);
  try {
    const read = resolveEngineModelWords(label);
    if (read.engine === engine && read.model === model) return `${label}${tail}`;
  } catch {}
  return `${engine} ${label}${tail}`;
}
...
function rosterLineText(key, pick) {
  ...
  const parts = [rosterWordsFor(pick.engine, pick.model || '', pick.effort || '')];
  if (pick.backup) parts.push(`backup ${rosterWordsFor(pick.backup, pick.backup_model || '', pick.backup_effort || '')}`);
  if (pick.max_seconds) parts.push(rosterMaxText(pick.max_seconds));
...
function setRosterPick(jobName, engineName, options = {}, root = process.cwd()) {
  ...
  const file = everywhere ? machineRosterMarkdownFile(options) : projectRosterFile(projectRosterRoot(root));
  ...
  if (!options.clear) {
    ...
    if (backupName) {
      try {
        ...
        throw new Error(canonicalEngineName(backupName.split(' ')[0]) ? `the backup ${error.message}` : `unknown backup engine "${backupName}"`);
      }
    }
    ...
    const maxSeconds = options.max === undefined ? null : parseRosterMax(`max ${options.max}`);
    ...
    line = rosterLineText(key, {
      ...
      ...(backupRead && backupRead.model ? { backup_model: backupRead.model } : {}),
      ...(backupRead && backupRead.effort ? { backup_effort: backupRead.effort } : {}),
      ...(maxSeconds ? { max_seconds: maxSeconds } : {}),
...
function renewRosterMarkdown(file, date, options = {}) {
  ...
  const next = rewriteRosterValues(text, (entry) => {
    ...
    return entry.raw.replace(/(,\s*until\s+)(\d{4}-\d{2}-\d{2})/i, `$1${until}`);
  });
...
function rosterChoiceForPick(pick, role, registry, { key, label, source, scopeText = '', now } = {}) {
  ...
  // A backup runs its own model and effort, never the primary's. The time
  // cap belongs to the line, so it rides whichever engine runs.
  const backupModel = backup && pick.backup_model ? savedRosterModel(backup.id, pick.backup_model) : '';
  const chosenModel = chosen === primary ? model : backupModel;
  const chosenEffort = chosen === primary ? savedRosterEffort(chosen.id, pick.effort) : savedRosterEffort(chosen.id, pick.backup_effort);
  const maxSeconds = Number(pick.max_seconds) > 0 ? Number(pick.max_seconds) : 0;
  const engine = chosenModel || chosenEffort || maxSeconds
    ? {
      ...chosen,
      ...(chosenModel ? { roster_model: chosenModel } : {}),
      ...(chosenEffort ? { roster_effort: chosenEffort } : {}),
...
function rosterModelFor(role, engineId, root = process.cwd(), options = {}) {
  return rosterPinFor(role, engineId, root, options).roster_model || '';
}
