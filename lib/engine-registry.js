...
const ENGINE_HEALTH_STATUSES = Object.freeze(['ready', 'not_installed', 'credit_out']);
...
function normalizeEngineEntry(id, saved = {}) {
  ...
  const creditOut = savedStatus === 'credit_out';
  const status = creditOut ? 'credit_out' : (installed ? 'ready' : 'not_installed');
  const health = { status };
  if (creditOut && savedHealth.last_failure_ts) health.last_failure_ts = String(savedHealth.last_failure_ts);
  return {
    id,
    name: id,
    bin: def.bin,
...
function setEngineHealth(name, status, root = process.cwd()) {
  ...
  const engines = registry.engines.map((engine) => {
    ...
    if (normalizedStatus === 'credit_out') health.last_failure_ts = new Date().toISOString();
