...
function assignedWorkerLines(key, engineName, options, context) {
  ...
  // its settings as written, keeping only the first line for each tool. The
  // old lead stays when it is a different tool, and then its note
  // stays on its own line.
  const taken = new Set([engine.id, backup.id]);
  const others = currentLines.filter((text, index) => {
    const tool = currentWorkers[index].engine;
    if (taken.has(tool)) return false;
    taken.add(tool);
    return true;
  });
  const keptOldLead = currentLines.length > 0 && ![engine.id, backup.id].includes(currentWorkers[0].engine);
...
function setRosterPick(jobName, engineName, options = {}, root = process.cwd()) {
  ...
  const next = replaceJobSection(base, {
    ...
    // Only an assign names a heading; remove and clear keep the one written.
    heading,
    lines,
  });
