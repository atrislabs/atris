...
function projectRosterRoot(root = process.cwd()) {
  ...
  try {
    ...
    if (path.basename(path.dirname(gitdir)) !== 'worktrees') return root;
    const main = path.dirname(path.dirname(path.dirname(gitdir)));
    return fs.existsSync(projectRosterFile(main)) ? main : root;
  } catch {
    return root;
  }
}
...
// is the nearest one not in the past as of the anchor, the moment the line
// was written (the file's last change). Anchoring to the read time instead
// would roll "oct 24" into next year the day after it passed, so it could
// never expire. null means the words are not a date.
...
function rosterPickFromValue(value, role, options = {}) {
  ...
  for (const part of extras) {
    ...
    if (backupMatch && !backup) {
      ...
      until = parseRosterUntilWords(untilMatch[1], options.anchor || options.now);
...
function rosterFileAnchor(file) {
  try {
...
function readRosterMarkdownLayer(scope, file, options = {}) {
  ...
  const anchor = rosterFileAnchor(file);
  return { scope, format: 'markdown', path: file, ...parseRosterMarkdown(text, file, { ...options, anchor }) };
}

// Before atris rewrites a ROSTER.md, every "until oct 24" becomes the full
// date it meant when it was written. The rewrite moves the file's anchor to
// today, so a date that already passed would otherwise come back as next
// year's. Only the date words change; the rest of each line stays.
function pinRosterDates(text, anchor) {
  const { rewriteRosterValues } = require('./roster-markdown');
  return rewriteRosterValues(text, (entry) => {
    let changed = false;
    const next = entry.raw.replace(/(,\s*until\s+)([^,<]+?)(\s*(?:,\s*\d{4}\b)?)(?=\s*(?:,|<!--|$))/gi, (match, lead, words, year) => {
      const typed = `${words}${year || ''}`.trim();
      if (parseRosterUntil(typed) || /\d{4}/.test(typed)) return match;
      const pinned = parseRosterUntilWords(typed, anchor);
      if (!pinned) return match;
      changed = true;
      return `${lead}${pinned}`;
    });
...
function setRosterPick(jobName, engineName, options = {}, root = process.cwd()) {
  ...
  const existing = layer.format === 'markdown' ? pinRosterDates(fs.readFileSync(file, 'utf8'), rosterFileAnchor(file)) : null;
  let line = null;
  if (!options.clear) {
...
function renewRosterMarkdown(file, date, options = {}) {
  ...
  const text = pinRosterDates(original, rosterFileAnchor(file));
  ...
  if (next !== original) writeAtomic(file, next);
}
