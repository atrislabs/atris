...
// (first seed, schema drift). Settled registries stay untouched, so concurrent
// readers can never stomp a mutation another process just landed.
function registryNeedsPersist(root, registry) {
  const raw = readRawRegistry(engineRegistryFile(root));
  return JSON.stringify(raw.engines || []) !== JSON.stringify(registry.engines);
}

function readEngineRegistry(root = process.cwd(), options = {}) {
  const registry = seededRegistry(root);
  if (options.persist !== false && registryNeedsPersist(root, registry)) {
