...
const ENGINE_SEED_META = Object.freeze({
  ...
  claude: Object.freeze({ tier: 'max', roles: Object.freeze(['validator', 'executor']), models: Object.freeze(['opus 5.5', 'opus 5', 'opus 4.8', 'fable', 'haiku']), fallback_order: 20 }),
...
function setRosterPick(jobName, engineName, options = {}, root = process.cwd()) {
  ...
  if (options.clear) {
    ...
    if (!engine.roles.includes(role)) throw new Error(`${engine.id} cannot do ${job}`);
    ...
    if (backup && !backup.roles.includes(role)) throw new Error(`${backup.id} cannot do ${job}`);
  ...
  writeEngineRegistry(root, { ...registry, updated_at: new Date().toISOString(), roster });
  return roster;
}

function confirmRoster(root = process.cwd(), now = new Date()) {
  const registry = readEngineRegistry(root, { persist: false });
  const date = rosterDate(now);
  const roster = { ...(registry.roster || {}) };
  for (const role of ENGINE_ROLES) {
    if (roster[role]) roster[role] = { ...roster[role], until: rosterUntil(date, 30), set_at: date.toISOString() };
  }
...
function resolveEngineForRoleRanked(role, root = process.cwd(), options = {}) {
  ...
  const pick = registry.roster && registry.roster[normalizedRole];
  const job = Object.keys(ENGINE_JOBS).find((name) => ENGINE_JOBS[name] === normalizedRole);
  const expired = pick && String(pick.until || '') < rosterDate(options.now).toISOString().slice(0, 10);
  const primary = pick && engines.find((engine) => engine.id === pick.engine);
  const backup = pick && engines.find((engine) => engine.id === pick.backup);
  const chosen = pick && !expired && primary ? primary : backup;
  if (chosen) {
    const engine = chosen === primary
      ? { ...chosen, ...(pick.model ? { roster_model: pick.model } : {}) }
      : chosen;
    return {
      engine,
      reason: chosen === primary
        ? `roster pick for ${job}: ${chosen.id}`
        : `roster pick for ${job} ${expired ? 'expired' : 'is not ready'}, using backup: ${chosen.id}`,
      ranked: [engine, ...(chosen === primary && backup ? [backup] : []), ...ranked.candidates.filter((candidate) => candidate.id !== chosen.id && (!backup || candidate.id !== backup.id))],
    };
  }
  return { engine: ranked.candidates[0] || null, reason: routerPickExplanation(ranked), ranked: ranked.candidates };
}
