...
const ENGINE_SEED_META = Object.freeze({
  ...
  claude: Object.freeze({ tier: 'max', roles: Object.freeze(['validator', 'executor', 'navigator']), models: Object.freeze(['opus 5.5', 'opus 5', 'opus 4.8', 'fable', 'haiku']), fallback_order: 20 }),
  ...
  haiku: Object.freeze({ tier: 'fast', roles: Object.freeze(['validator', 'navigator']), models: Object.freeze(['haiku']), fallback_order: 70 }),
  agy: Object.freeze({
    tier: 'pro',
...
// spellings like "opus-5.5". Save the id the CLI accepts, once, at assign.
const CLAUDE_FAMILY_ENGINES = Object.freeze(['claude', 'fable', 'haiku']);
const CLAUDE_MODEL_ALIASES = Object.freeze(['opus', 'sonnet', 'haiku']);
...
function normalizeRosterModel(engineId, value) {
  ...
  if (CLAUDE_MODEL_ALIASES.includes(lower)) return lower;
  const match = CLAUDE_MODEL_SPELLING.exec(lower.replace(/^claude[\s_-]*/, ''));
  if (match) {
    const id = `claude-${match[1]}-${match[2]}${match[3] ? `-${match[3]}` : ''}`;
    return CLAUDE_MODEL_DATED_IDS[id] || id;
  }
  if (lower.startsWith('claude-')) return raw;
  throw new Error(`${engineId} does not know the model "${raw}". use opus, sonnet, haiku, a version like opus 5.5, sonnet 5, haiku 4.5, or fable 5.1, or a full claude- id`);
}
...
// the roster still honors the job the engine can do today.
function engineHasRole(engine, role) {
  ...
  return Boolean(seed && seed.roles.includes(role));
}
...
function setRosterPick(jobName, engineName, options = {}, root = process.cwd()) {
  const job = rosterJob(jobName);
  if (!job) throw new Error(`unknown job "${jobName}". known jobs: search, build, review`);
  ...
  if (options.clear) {
    delete roster[ENGINE_JOBS[job]];
  } else {
    const role = ENGINE_JOBS[job];
    ...
    if (!engineHasRole(engine, role)) throw new Error(`${engine.id} cannot do ${job}`);
    ...
    if (backup && !engineHasRole(backup, role)) throw new Error(`${backup.id} cannot do ${job}`);
...
function renewRoster(roster, date) {
  ...
  for (const role of ENGINE_ROLES) {
    if (next[role]) next[role] = { ...next[role], until: rosterUntil(date, 30), set_at: date.toISOString() };
  }
  return next;
}
...
// Walk the roster layers in order: this project's pick, its backup, the
// all-projects pick, its backup. The first ready engine wins; null means the
// router decides.
function rosterChoice(role, registry, options = {}) {
  const job = Object.keys(ENGINE_JOBS).find((name) => ENGINE_JOBS[name] === role);
  ...
  const layers = [
    { source: 'project', pick: registry.roster && registry.roster[role] },
    { source: 'machine', pick: machine && machine[role] },
  ];
  for (const { source, pick } of layers) {
    if (!pick || !pick.engine) continue;
    const expired = rosterPickExpired(pick, options.now);
    const primary = expired ? null : ready.find((engine) => engine.id === pick.engine) || null;
    ...
    const engine = chosen === primary && pick.model ? { ...chosen, roster_model: pick.model } : chosen;
    return {
      engine,
      ...
      reason: chosen === primary
        ? `roster pick for ${job}${scope}: ${chosen.id}`
        : `roster pick for ${job}${scope} ${expired ? 'expired' : 'is not ready'}, using backup: ${chosen.id}`,
    };
  }
  return null;
}
