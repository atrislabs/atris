 ...
 */
// Blocked means a task whose status is failed: the DB has no separate
// blocked status, doneTask logs a failed task as a "blocked" event, and the
// board renders failed tasks under "## Blocked". Once this workspace has rows
// in the task DB, the DB is the only source. The DB file is shared by every
// workspace on the machine, so a workspace with no rows there falls back to
// the Blocked section of its own TODO.md, never a word elsewhere in a title.
function hasBlockedTasks(atrisDir, todoFile, legacyTaskContextFile) {
  try {
    const taskDb = require('./task-db');
    if (fs.existsSync(taskDb.getDbPath())) {
      const db = taskDb.open();
      const workspaceRoot = taskDb.workspaceRoot(path.dirname(path.resolve(atrisDir)));
      if (taskDb.listTasks(db, { workspaceRoot, limit: 1 }).length) {
        return taskDb.listTasks(db, { workspaceRoot, status: 'failed', limit: 1 }).length > 0;
      }
    }
  } catch (e) {
    if (process.env.ATRIS_DEBUG) console.error('[state-detection] db blocked read failed:', e.message);
  }
  const taskFilePath = fs.existsSync(todoFile)
    ? todoFile
    : (fs.existsSync(legacyTaskContextFile) ? legacyTaskContextFile : null);
  if (!taskFilePath) return false;
  return getTasksFromTodoSection(fs.readFileSync(taskFilePath, 'utf8'), 'Blocked').length > 0;
}
...
function detectWorkspaceState(workspaceDir) {
  ...
  if (hasBlockedTasks(atrisDir, todoFile, legacyTaskContextFile)) {
    return { state: 'blocked', reason: 'Tasks marked as blocked' };
  }
