...
function readRoadmapOpenItems(root) {
  ...
  if (!text) return [];
  // Tolerate a header suffix (e.g. "(priority)") and stop only at a sibling/parent
  // heading (## or #), not a child ### inside the section, and also at a malformed
  // "##Next" with no space after the hashes.
...
// Mark an open ROADMAP item handled by flipping its `- [ ]` to `- [x]`. This is
// the single source of truth for "the loop took this on": readRoadmapOpenItems
// then excludes it, so both the work gate and the seed picker agree it is done.
// Returns true if it flipped a line. Idempotent for an already-checked item.
function checkRoadmapItem(root, title) {
  ...
  const out = text.split(/\r?\n/).map((line) => {
    ...
    if (m && norm(m[2]) === target) {
      changed = true;
      return `${m[1]}- [x] ${m[2].trim()}`;
    }
    return line;
  }).join('\n');
  if (changed) fs.writeFileSync(p, out, 'utf8');
  return changed;
}

// The top open ROADMAP item the loop has not already handled. Handled items are
// marked `- [x]` (so readRoadmapOpenItems drops them); this also skips anything
// already in the inbox or killed/approved, so an idle loop advances to the next
// item each cycle. Root-explicit and pure of cwd, so it is testable without a
// live runner.
