...
function resolveFreshOpenWork({ root, files, commit, dirty } = {}) {
  ...
  if (!listed) return '';
  return spokenOpenWork(listed);
}

function freshWinLine({ folder = 'this folder', files, root, commit, dirty } = {}) {
  const visible = spokenVisibleNames(files);
...
function renderFresh({ person = '', folder = 'this folder', files, root, commit, dirty } = {}) {
  const room = folder || 'this folder';
  return [
    `${greet(person)}${freshWinLine({ folder: room, files, root, commit, dirty })}`,
    '',
...
function freshMinuteJson(folder = 'this folder', files, extra = {}) {
  ...
  return {
    ...
    ok: false,
    status: 'stuck',
    reason: freshWinLine({ folder, files, root, commit, dirty }).replace(/\.$/, ''),
...
function buildFirstMinute({
  ...
  if (fresh) {
    ...
    return {
      kind: 'fresh',
      text: renderFresh({ person: who, folder: room, files: visible, root, commit, dirty }),
