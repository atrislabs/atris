...
function headCommitSubject(root = process.cwd()) {
  const resolved = path.resolve(root || '.');
  try {
    // Only this folder. A parent repo is not their last words here.
...
function freshWinLine({ folder = 'this folder', files, root, commit } = {}) {
  const visible = spokenVisibleNames(files);
...
function renderFresh({ person = '', folder = 'this folder', files, root, commit } = {}) {
  const room = folder || 'this folder';
  return [
    `${greet(person)}${freshWinLine({ folder: room, files, root, commit })}`,
    '',
...
function freshMinuteJson(folder = 'this folder', files, extra = {}) {
  ...
  return {
    ...
    ok: false,
    status: 'stuck',
    reason: freshWinLine({ folder, files, root, commit }).replace(/\.$/, ''),
...
function buildFirstMinute({
  ...
  if (fresh) {
    ...
    return {
      kind: 'fresh',
      text: renderFresh({ person: who, folder: room, files: visible, root, commit }),
