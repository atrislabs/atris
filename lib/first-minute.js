...
function listUserVisibleWork(root = process.cwd()) {
  ...
  return names.filter(isUserVisibleName).sort((a, b) => a.localeCompare(b));
}
...
function buildFirstMinute({
  ...
  if (fresh) {
    ...
    return {
      kind: 'fresh',
      text: renderFresh({ person: who, folder: room, files }),
...
function speakFirstMinute({
  ...
  if (asJson && isFresh) {
    log(JSON.stringify(freshMinuteJson(folderName(root), listUserVisibleWork(root)), null, 2));
    return 2;
  }
