...
function freshWinLine({ folder = 'this folder', files } = {}) {
...
function renderFresh({ person = '', folder = 'this folder', files } = {}) {
  const room = folder || 'this folder';
  return [
    `${greet(person)}${freshWinLine({ folder: room, files })}`,
    '',
...
function firstTalkNext({ starter = null, person = '', folder = 'this folder' } = {}) {
  ...
  if (starter && starter.display_id) return `atris task claim ${starter.display_id} --as ${claimAs}`;
...
function freshMinuteJson(folder = 'this folder', files) {
  return {
    schema: 'atris.one_lap.v1',
    ok: false,
    status: 'stuck',
    reason: freshWinLine({ folder, files }).replace(/\.$/, ''),
...
function buildFirstMinute({
  ...
  if (fresh) {
    ...
    return {
      kind: 'fresh',
      text: renderFresh({ person: who, folder: room, files: visible }),
...
function speakFirstMinute({
  ...
  const visible = files != null ? spokenVisibleNames(files) : listUserVisibleWork(root);
  if (asJson && isFresh) {
    log(JSON.stringify(freshMinuteJson(folderName(root), visible), null, 2));
    return 2;
  }
