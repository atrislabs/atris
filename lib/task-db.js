...
// Stale-claim reaper (OBL-2214): dead loop ticks leave status='claimed' rows
// held by the shared 'atris' identity, and owner-scoped releaseTask refuses
// them with 'held_by_other'. This releases claimed rows idle past a threshold
// back to 'open' and notes the dead claim on each. Default scope is the
// shared 'atris' loop identity only; rows held by a named member or person
// are reported as skipped unless includePersons is set. apply=false previews
// without writing.
function reapStaleClaims(db, { workspaceRoot: ws, olderThanDays = 14, apply = false, actor, includePersons = false, limit = 5000 } = {}) {
  const days = Number(olderThanDays);
  if (!Number.isFinite(days) || days <= 0) throw new Error('olderThanDays must be a positive number of days');
  const idleMs = days * 86400000;
  const now = Date.now();
  const stale = listTasks(db, { workspaceRoot: ws || null, status: 'claimed', limit })
    .filter(row => Number.isFinite(Number(row.updated_at)) && now - Number(row.updated_at) >= idleMs);
  const isSharedLoopClaim = row => String(row.claimed_by || '') === 'atris';
  const candidates = stale.filter(row => includePersons || isSharedLoopClaim(row));
  const skippedPersons = stale.filter(row => !includePersons && !isSharedLoopClaim(row));
  const idleDays = row => Math.floor((now - Number(row.updated_at)) / 86400000);
  const sample = candidates.slice(0, 10).map(row => ({
    id: row.id,
    title: row.title,
    claimed_by: row.claimed_by || null,
    idle_days: idleDays(row),
  }));
  const skippedSample = skippedPersons.slice(0, 10).map(row => ({
    id: row.id,
    title: row.title,
    claimed_by: row.claimed_by || null,
    idle_days: idleDays(row),
  ...
  if (!apply) {
    return {
      ...
      sample,
      skipped_person_count: skippedPersons.length,
      skipped_person_sample: skippedSample,
    };
  }
  ...
  for (const row of candidates) {
    const result = withBusyRetry(() => db.prepare(`
      UPDATE tasks
       ...
       WHERE id = ?
         ...
         AND claimed_by = ?
         AND updated_at = ?
    `).run(now, row.id, row.claimed_by, row.updated_at));
    ...
    noteTask(db, {
      id: row.id,
      actor: actorText,
      content: `stale claim reaped: held by ${row.claimed_by || 'unknown'} for ${idleDays(row)} day(s) with no update; released back to open by reap-stale-claims`,
    });
