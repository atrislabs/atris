...
function reviewTask(db, { id, actor, reward, lesson, nextTask, proof, careerXpEligible = false, clearedFields = [] }) {
  ...
  const updatingPendingReviewProof = row.status === 'review'
    && metadata.approval_status === 'pending'
    && numericReward <= 0
    && Boolean(proofText || lessonText || nextTaskText);
  ...
  if (reviewingPendingProof || updatingPendingReviewProof) {
    ...
    if (lessonText) metadata.latest_agent_lesson = lessonText;
    if (nextTaskText) metadata.latest_agent_next_task = nextTaskText;
  }
    ...
    tasks: visibleRows.map(row => {
      ...
      const allMessages = taskEvents
        ...
        .map(e => ({
          version: e.version,
          actor: e.actor,
          content: clipProjectionText(e.event_type === 'message'
