...
const TODO_RENDER_FAILED_LIMIT = 8;
...
function readAgentWorktreeWorkspaceRoot(root) {
  const metaPath = path.join(root, '.atris', 'agent-worktree.json');
  ...
  const raw = meta && (meta.workspace_root || meta.launcher_root || meta.source_root || meta.primary_root);
  if (!raw) return null;
  let candidate = path.resolve(root, String(raw));
...
function workspaceRoot(cwd) {
  ...
  return readAgentWorktreeWorkspaceRoot(root) || root;
}
...
function renderLimit(value, fallback) {
  const fallbackLimit = Math.max(0, Number(fallback) || 0);
  if (value === undefined || value === null || value === '') return fallbackLimit;
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) return fallbackLimit;
...
function renderTodoMarkdown(rows, { title = 'TODO.md', doneLimit = TODO_RENDER_DONE_LIMIT, failedLimit = TODO_RENDER_FAILED_LIMIT, refRows = rows, preservedSections = [] } = {}) {
  ...
  const renderedFailed = buckets.failed.slice(0, renderLimit(failedLimit, TODO_RENDER_FAILED_LIMIT));
  appendSection(lines, 'Failed / No-go', renderedFailed);
  const archivedFailed = Math.max(0, buckets.failed.length - renderedFailed.length);
  if (archivedFailed > 0) {
    lines.push(`(${archivedFailed} older failed/no-go task${archivedFailed === 1 ? '' : 's'} archived in \`atris task list --status failed\` and \`atris task events\`.)`, '');
  }
  const renderedDone = buckets.done.slice(0, renderLimit(doneLimit, TODO_RENDER_DONE_LIMIT));
