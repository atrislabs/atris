...
function appendSection(lines, name, rows) {
  ...
  for (const row of rows) {
    ...
    if (process.env.ATRIS_TODO_RENDER !== 'full') {
      ...
      const verify = meta.verify ? ` · verify: ${singleLine(meta.verify).slice(0, 60)}` : '';
      lines.push(`- **[${displayRef}]** ${singleLine(row.title)}${tag}${decision} · ${owner}${verify}`);
      if (name === 'In Progress' || name === 'Review') {
