...
function interpretationCandidates(kind, ctx) {
  ...
  if (kind === 'missing-first-slice' && ctx.topic) candidates[0] = `${ctx.topic} capture path`;
  if (kind === 'missing-first-slice' && ctx.action) candidates[0] = `${ctx.action} path`;
...
function interpretationLabel(kind, ctx) {
  if (kind === 'vague-superlative') return capitalizeFirst(ctx.superlative || ctx.topic || 'this');
