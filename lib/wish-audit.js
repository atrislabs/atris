...
const DEICTIC_STAND_INS = new Set(['thing', 'stuff', 'one', 'it', 'that', 'those', 'these']);
...
function clarityAudit(text) {
  ...
  const missing = [];
  const questions = [];
  const vague = words.length < 4 || !hasVerb(text) || !hasMeaningfulContent(text);
...
function splitWishText(text) {
  const parts = [];
  for (const segment of String(text || '').split(';')) {
    for (const part of splitSegmentOnJoiners(segment)) parts.push(part);
  }
  return parts.map(cleanPartText).filter(Boolean);
}

function partLooksDeliverable(text, root) {
  if (outOfScopeReason(text, root)) return true;
  return hasVerb(text) && meaningfulInstructionWordCount(text) >= 2;
}

function analyzeWishParts(text, root) {
  const clean = cleanPartText(text);
  if (!clean) return null;
  const rough = splitWishText(clean);
  if (rough.length > 1 && !rough.every((part) => partLooksDeliverable(part, root))) return null;
