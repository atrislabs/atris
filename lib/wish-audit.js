...
const KEYWORD_VALUE_SKIP = new Set(['a', 'an', 'at', 'for', 'from', 'in', 'is', 'on', 'the', 'to']);
...
function rowAskedPathSlot(row) {
  return Array.isArray(row && row.questions)
    && row.questions.some((question) => String(question || '').trim() === PATH_SLOT_QUESTION);
}
...
function priorPathQuestionRowForAuditText(text, root) {
  return readWishAuditEvents(root).find((row) => rowAskedPathSlot(row) && rowMatchesAuditText(row, text)) || null;
}
...
function shouldAcceptPathAnswer(text, root) {
  ...
  const askedRow = priorPathQuestionRowForAuditText(text, root);
  if (!askedRow) return false;
  context.filled_slot = {
    kind: 'path',
    question: PATH_SLOT_QUESTION,
    value: tokens[0],
  };
