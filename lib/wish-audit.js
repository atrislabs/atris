...
// A wish already sized quick (a small fix, tweak, or patch) asks the roster
// for the owner's small build pick first; with none set, it routes as any
// build. The router's own ranking is unchanged either way.
function resolveExecutor(root, engineOverride = '', options = {}) {
  if (engineOverride) {
    return engineRegistry.resolveEngineForRoleWithPreference('executor', root, engineOverride, options);
  }
  return {
    engine: resolveRole('executor', root, options),
...
function auditWish(text, root = process.cwd(), options = {}) {
  ...
  try {
    const small = inferBudgetTier(text) === 'quick' ? { job: engineRegistry.SMALL_BUILD_JOB } : {};
    const resolved = resolveExecutor(root, engineOverride, small);
