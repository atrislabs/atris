...
function acceptedInLastDay(tasks, now = Date.now()) {
  ...
  for (const t of tasks || []) {
    ...
    const entry = { ref: t.display_id || t.legacy_ref || t.id, title: String(t.title || '').slice(0, 90) };
...
// Short enough for one iMessage, plain enough to read over coffee.
function composeDigest({ accepted, waiting, landed, project }) {
  ...
  if (autoCount + humanCount > 0) {
    const parts = [];
    if (autoCount) parts.push(`${plur(autoCount, 'piece')} landed on their own`);
    if (humanCount) parts.push(`${plur(humanCount, 'piece')} you approved`);
    lines.push(parts.join(', '));
  } else {
    lines.push('nothing landed in the last day');
  }
  if (waiting.length > 0) {
