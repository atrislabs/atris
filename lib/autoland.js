...
const OPERATOR_WHY = /\b(sav(?:e|es|ing)|burn(?:s|ing)?|wast(?:e|es|ing)|cost(?:s|ing)?|prevent\w*|reduc\w*|risk|stop(?:s|ping)?|break(?:s|ing)?|fail(?:s|ing)?|slow(?:s|er)?|faster|cheaper|easier|clearer|safer|simpler|trust|revenue|so(?: that)?|because)\b/i;
...
function digestMoveLine(move) {
  if (move.kind === 'mission_ready') {
    ...
    const clause = title.includes(':') ? title.split(':', 1)[0].trim() : '';
    const completeClause = clause && clause.length <= 60
      ? clause.charAt(0).toLowerCase() + clause.slice(1)
      : '';
    const label = finishThought(completeClause || String(move.label || '').replace(/^#\d+\s+/, ''), 60);
    return label
      ? `review proof for ${label} so finished work can leave the queue`
...
function unexplainedSummaryRows(items, label) {
  ...
  return [...groups.entries()].map(([member, count]) => {
    ...
    return `${count} more ${label}${who} that could not explain ${reflexive(count)} yet`;
  });
}
...
function composeDigest({ accepted, waiting, waitingWishes = [], landed, project, nextMoves = [], acceptAll = false, reapError = null, janitor = null, landingSweep = null, fullStory = false }) {
  const lines = [];
  ...
  if (waiting.length > 0) {
    ...
    lines.push(`waiting on you (${explainedWaiting.length} clear next actions/${waiting.length} total; approve or bounce: atris task reviews):`);
  ...
  if (moves.length > 0) {
    ...
    if (unexplained > 0) lines.push(`- ${plur(unexplained, 'more move')} need${unexplained === 1 ? 's' : ''} a clearer next action (atris now)`);
  } else if (unexplained > 0) {
    lines.push(`${plur(unexplained, 'move')} in the queue need${unexplained === 1 ? 's' : ''} a clearer next action (atris now)`);
  }
