...
const AGENT_JARGON = /[a-z0-9]_[a-z0-9]|\b[a-z]+[A-Z]\w*|--[a-z]|\bCLI-\d+|\b[Aa]tris\s+[a-z][a-z-]*/;
...
function composeDigest({ accepted, waiting, landed, project, nextMoves = [] }) {
  ...
  if (autoCount > 0) {
    ...
    // Operator-ready sentences earn the limited digest surface. Rough landed
    // text is counted, not shown; the source needs a better landing sentence.
    ...
    if (visibleWork.length > 0) {
      ...
      lines.push(`${plur(held, 'landed result')} need clearer landing sentences before they get text space: atris autoland digest`);
    }
  }
  ...
  const displayMoves = [];
  let unexplained = Number(nextInfo.unexplained || 0);
  for (const move of (nextInfo.moves || []).filter((candidate) => candidate && candidate.title)) {
    const line = finishThought(move.title, 140);
    if (operatorReady(line)) displayMoves.push({ move, line });
    else unexplained += 1;
    if (displayMoves.length >= 3) break;
  }
  if (displayMoves.length > 0) {
    lines.push('next, if you agree:');
    for (const { move, line } of displayMoves) {
      lines.push(`- ${line}${move.owner ? ` (best fit: ${move.owner})` : ''}`);
    }
