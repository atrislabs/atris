...
function composeDigest({ accepted, waiting, landed, project, nextMoves = [] }) {
  ...
  if (autoCount > 0) {
    ...
    if (visibleWork.length > 0) {
      ...
      // Everything landed, but nobody wrote a sentence fit to read. Say that
      // plainly: the gap is in the writing, and hiding it would hide the bug.
      lines.push(`${plur(work.length + (stops.length > 0 ? 1 : 0), 'piece')} landed and verified, but their summaries aren't written for you yet: atris autoland digest`);
    }
  }
