...
function composeDigest({ accepted, waiting, waitingWishes = [], landed, project, nextMoves = [], acceptAll = false, reapError = null, janitor = null, landingSweep = null }) {
  ...
  if (moves.length > 0) {
    ...
    if (unexplained > 0) lines.push(`- ${plur(unexplained, 'more idea')} that can't explain ${reflexive(unexplained)} yet (atris now)`);
  } else if (unexplained > 0) {
    lines.push(`${plur(unexplained, 'idea')} in the queue can't explain ${reflexive(unexplained)} yet (atris now)`);
  }
