...
function digestLine(item) {
  ...
  const line = cutAtWord(source.replace(/[.\s]+$/, ''), 150);
...
function composeDigest({ accepted, waiting, landed, project, nextMoves = [] }) {
  ...
  if (waiting.length > 0) {
    ...
    for (const item of waiting.slice(0, 2)) {
      lines.push(`- ${cutAtWord(dejargon(item.title), 130)}${item.hours >= 12 ? ` (${item.hours}h)` : ''}`);
    }
  ...
  if (moves.length > 0) {
    ...
    for (const move of moves) {
      lines.push(`- ${cutAtWord(move.title, 130)}${move.owner ? ` (best fit: ${move.owner})` : ''}`);
    }
