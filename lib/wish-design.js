...
const BACKEND_CONTEXT_RE = /\b(?:api|json|endpoint|terminal|log|logs|deploy|release|script|pipeline|server|cache|config|webhook|payloads?|secrets?)\b/i;
...
function isFrontendWish(text) {
  ...
  const backendWeakOnly = weakMatches.length > 0
    && (BACKEND_CONTEXT_RE.test(raw) || PATH_CONTEXT_RE.test(raw))
    && !STRONG_FRONTEND_RE.test(raw)
    && !visualPositive
