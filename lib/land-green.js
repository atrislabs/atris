...
function landOneBranch(root, base, branch, {
  ...
  if (dryRun) return verdictFor(branch, base, { action: 'skipped', reason: 'dry_run' });
  ...
  try {
    ...
    const merged = gitOut(['merge', '--no-edit', '--no-ff', base, '-m', `land: ${branch} onto ${base} after a green verify`], scratch.dir);
    ...
    const advanced = advanceBase(root, base, sha.out);
    if (!advanced.ok) return verdictFor(branch, base, { action: 'skipped', reason: advanced.reason, detail: advanced.detail, verify });
    const pushed = push ? pushBase(root, base) : { attempted: false, ok: false, reason: 'push_disabled' };
