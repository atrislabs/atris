...
function prepareMissionGitGuard({
  ...
  const source = [
    ...
    // Inlined copies of this module's gitSubcommand/defaultGit: keeping them
    // inline lets the module stop exporting its internal helpers (the static
    // orphan-export ratchet cannot see this dynamic require).
    `const consumesValue = new Set(['-C', '-c', '--git-dir', '--work-tree', '--namespace', '--config-env']);`,
    `function gitSubcommand(argv) {`,
    `  for (let index = 0; index < argv.length; index += 1) {`,
    `    const arg = String(argv[index] || '');`,
    `    if (consumesValue.has(arg)) { index += 1; continue; }`,
    `    if (arg.startsWith('-')) continue;`,
    `    return arg;`,
    `  }`,
    `  return '';`,
    `}`,
    `const git = (gitArgs, cwd) => spawnSync(config.gitBin, gitArgs, { cwd, encoding: 'utf8', timeout: 10000 });`,
    `if (gitSubcommand(args) === 'commit') {`,
    `  const result = guard.inspectMissionProtectedDiff({ root: config.root, tags: config.tags, includeUnstaged: false, git });`,
    `  if (!result.allowed) {`,
