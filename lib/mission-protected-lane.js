...
function isProtectedDiffCandidate(file) {
  return isCodeFile(file)
    || PATH_SURFACES.some(([, pattern]) => pattern.test(file));
}
...
function enobufsDetail({ nameOnly = false, matchedPaths = [] } = {}) {
  if (nameOnly) {
    return 'git diff name-only output was too large (enobufs, 64mb limit); matched paths could not be read';
  }
  const shown = matchedPaths.slice(0, 20);
  const more = matchedPaths.length > shown.length ? ` (+${matchedPaths.length - shown.length} more)` : '';
  return `git diff output was too large after name-only filtering (enobufs, 64mb limit); matched paths: ${shown.join(', ') || 'none'}${more}`;
}

function runGitDiff(git, args, root, { nameOnly = false, matchedPaths = [] } = {}) {
  const result = git(args, root);
  if (!result || result.error || result.status !== 0) {
    const errorText = result?.error?.message || '';
    if (result?.error?.code === 'ENOBUFS' || /\bENOBUFS\b/i.test(errorText)) {
      return { ok: false, detail: enobufsDetail({ nameOnly, matchedPaths }) };
    }
...
function readFilteredGitDiff(git, targetArgs, root) {
  const commonArgs = ['diff', '--no-ext-diff', '--no-color', '--no-renames'];
  const names = runGitDiff(
    git,
    [...commonArgs, '--name-only', '-z', ...targetArgs, '--'],
    root,
    { nameOnly: true },
  );
  if (!names.ok) return names;

  const matchedPaths = changedPathsFromNameOnly(names.diff).filter(isProtectedDiffCandidate);
  if (matchedPaths.length === 0) return { ok: true, diff: '' };

  return runGitDiff(
    git,
    [...commonArgs, '--unified=0', ...targetArgs, '--', ...matchedPaths],
    root,
    { matchedPaths },
  );
}
...
function readMissionDiff({
  ...
  const parts = [];
  const staged = readFilteredGitDiff(git, ['--cached'], root);
  if (!staged.ok) return staged;
  parts.push(staged.diff);

  if (includeUnstaged) {
    const unstaged = readFilteredGitDiff(git, [], root);
    if (!unstaged.ok) return unstaged;
    parts.push(unstaged.diff);
  }

  if (baseRef) {
    const committed = readFilteredGitDiff(git, [baseRef, 'HEAD'], root);
...
function prepareMissionGitGuard({
  ...
  const source = [
    ...
    `  }`,
    `}`,
    `const child = spawnSync(config.gitBin, args, { cwd: process.cwd(), stdio: 'inherit', maxBuffer: ${GIT_MAX_BUFFER} });`,
