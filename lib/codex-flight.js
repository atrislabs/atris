...
// Worktree flights need full file access to edit and ship the requested repo.
const FLIGHT_SANDBOX = 'danger-full-access';
...
function resolveCodexCompanion() {
  if (process.env.ATRIS_CODEX_COMPANION) {
    const override = path.resolve(process.env.ATRIS_CODEX_COMPANION);
    return fs.existsSync(override) && fs.statSync(override).isFile() ? override : null;
  }
  const cache = path.join(os.homedir(), '.claude', 'plugins', 'cache', 'openai-codex', 'codex');
  ...
  for (const version of versions) {
    const script = path.join(cache, version, 'scripts', 'codex-companion.mjs');
    if (fs.existsSync(script) && fs.statSync(script).isFile()) return script;
  }
  return null;
}

function spawnCodex(worktreePath, prompt, companion) {
  return new Promise((resolve, reject) => {
    const argv = [companion, 'task', '--background', '--write', prompt];
    const child = spawn('node', argv, {
      cwd: worktreePath,
      env: { ...process.env, CODEX_COMPANION_SANDBOX: FLIGHT_SANDBOX },
...
async function dispatchCodexFlight({ repoPath, slug, brief, verifyCmd, dryRun = false } = {}) {
  ...
  if (!companion) throw new Error('codex companion not found. set ATRIS_CODEX_COMPANION to its script path.');
  runWorktreeAdd(repo.repoPath, branch, worktreePath);
  const task = await spawnCodex(worktreePath, prompt, companion);
