...
const { MAP_REF_DOCS, existingMapDocs } = require('./map-refs');
const {
...
function seedScoutContext({ task, worktreePath }) {
  ...
  const mapPath = MAP_REF_DOCS[0];
  const mapTexts = readMapTexts(worktreePath);
  ...
  // The boot map routes; the notes file holds the deep file:line refs. Read both.
  for (const [doc, mapText] of mapTexts) {
    ...
    for (let index = 0; index < mapLines.length; index += 1) {
      ...
      if (matches.length && line.trim() && line.trim().length <= 300 && gotchaCandidates.length < 16) {
        gotchaCandidates.push({ ...(doc === mapPath ? {} : { file: doc }), line: index + 1, text: line.trim() });
      }
    }
  }
...
// Map text by doc, boot map first. A missing doc is simply absent.
function readMapTexts(root) {
  return new Map(existingMapDocs(root).map((doc) => [doc, fs.readFileSync(path.join(root, doc), 'utf8')]));
}

function verifyMapGotcha(gotcha, mapTexts) {
  ...
  const file = gotcha.file === undefined ? MAP_REF_DOCS[0] : String(gotcha.file);
  if (!text || text.length > 300 || !Number.isInteger(citedLine) || citedLine < 1) return null;
  if (!mapTexts.has(file)) return null;
  const lines = mapTexts.get(file).split(/\r?\n/);
  const actualLine = lines.findIndex((line) => line.trim() === text) + 1;
  if (!actualLine) return null;
  return { ...(file === MAP_REF_DOCS[0] ? {} : { file }), line: actualLine, text };
}
...
function verifyScoutPack(rawPack, { task, worktreePath, allowedFiles, expectedCommit = '' }) {
  ...
  const mapGotchas = (Array.isArray(rawPack.map_gotchas) ? rawPack.map_gotchas : [])
    ...
    .map((gotcha) => verifyMapGotcha(gotcha, mapTexts))
    .filter(Boolean);
...
function renderVerifiedScoutBlock(pack, { worktreePath }) {
  ...
  if (pack.map_gotchas.length) {
    ...
    lines.push(...pack.map_gotchas.map((gotcha) => `- ${gotcha.file || 'MAP'} line ${gotcha.line}: ${gotcha.text}`));
  }
