...
function orbFailureCandidate(run = {}) {
  ...
  return {
    ...
    exit_code: exitCode,
    log_path: logPath,
    next_task: `investigate failed orb job "${label}" from ${engine} exit ${exitCode}; open ${logPath}`,
  };
}

function buildOrbFailureScorecardRow(run = {}) {
  const candidate = orbFailureCandidate(run);
  ...
  return {
    ...
    verify_passed: false,
    credits_deducted: 0,
    what_shipped: `orb job failed: ${candidate.label}`,
    files_written: [],
...
function ingestOrbScorecard(root, options = {}) {
  ...
  for (const failure of scorecard.failures) {
    const row = buildOrbFailureScorecardRow(failure);
  ...
  return {
    ...
    improvement_candidates: scorecard.failures.map(orbFailureCandidate),
    scorecards_written: written.length,
    written,
    historical_failures: existing.filter((row) => row && row.source === 'orb' && row.mode === 'orb_failure').length + written.length,
  };
}
