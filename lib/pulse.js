...
function modelForRunnerProfile(name) {
  const key = String(name || '').trim();
  if (!key) return '';
  const profile = RUNNER_PROFILES[key];
  if (!profile) {
    throw new Error(`Unknown runner profile "${key}". Known profiles: ${Object.keys(RUNNER_PROFILES).join(', ')}`);
  }
...
function buildTickScript(opts = {}) {
  ...
  const runnerModel = model || modelForRunnerProfile(runnerProfile) || DEFAULT_CLAUDE_RUNNER_MODEL;
