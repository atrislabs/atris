...
function buildTickScript(opts = {}) {
  ...
  const runnerBinExport = runnerBin
    ? `[ -n "\${ATRIS_CLAUDE_BIN:-}" ] || export ATRIS_CLAUDE_BIN=${shellSingleQuote(runnerBin)}`
    : '';
  const runnerCommandTemplateExport = runnerCommandTemplate
    ? `[ -n "\${ATRIS_CLAUDE_COMMAND_TEMPLATE:-}" ] || export ATRIS_CLAUDE_COMMAND_TEMPLATE=${shellSingleQuote(runnerCommandTemplate)}`
    : '';
...
[ -n "\${ATRIS_CLAUDE_MODEL:-}" ] || export ATRIS_CLAUDE_MODEL="${model}"
${runnerBinExport}
