...
function normalizeRunnerIdentity(input = {}) {
  const runner = input && typeof input === 'object' ? input : {};
  return {
    model: runner.model || null,
    profile: runner.profile || null,
    bin: runner.bin || null,
    template_configured: runner.template_configured != null ? Boolean(runner.template_configured) : null,
  };
}
...
function buildPulseReceipt(input = {}) {
  return {
    ...
    runner: input.runner ? normalizeRunnerIdentity(input.runner) : null,
  };
}
...
function summarizePulse(receipts, now = Date.now()) {
  ...
  return {
    ...
    last_runner: last ? (last.runner || null) : null,
