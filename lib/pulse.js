...
function detectStaleTick(receipts, now = Date.now(), staleMs = LIVENESS_STALE_MS) {
  ...
  if (!Number.isFinite(lastMs)) {
    // A finished tick whose timestamp is missing or unparseable: we cannot
    // confirm it is recent, so we must not silently report the loop as alive.
    return { stale: true, reason: 'last_tick_unreadable_ts' };
  }
