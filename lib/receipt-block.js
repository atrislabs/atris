...
function proofText(receipt) {
  ...
  if (land.checked) return lowerLine(land.checked);
  if (land.tested) return lowerLine(land.tested);
  if (verifier) {
...
function renderEmailLine(receipt) {
  const what = truncateLine(trimPunctuation(changedText(receipt)), 110);
  const scale = scaleText(receipt);
  const proof = truncateLine(trimPunctuation(proofText(receipt)), 90);
