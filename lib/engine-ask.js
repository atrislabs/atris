...
function askUsage() {
  return [
    ...
    '',
    `jobs files contain up to ${MAX_ASK_JOBS} objects: [{"engine":"codex","prompt":"question","label":"optional"}]`,
  ].join('\n');
}
...
function parseEngineAskArgs(args, { root = process.cwd(), readFile = fs.readFileSync } = {}) {
  ...
  let jobs;
  if (jobsFile) {
    if (commonPrompt || requestedEngines.length) {
      throw new Error('--jobs cannot be combined with a shared prompt or --engine');
    }
...
function buildReadOnlyEngineInvocation(engineName, prompt) {
  ...
  if (engine === 'claude' || engine === 'fable' || engine === 'haiku') {
    return {
      ...
      args: [
        '-p', request,
        '--model', profile.model || DEFAULT_CLAUDE_RUNNER_MODEL,
  ...
  if (engine === 'codex') {
    return {
      engine,
      bin: profile.bin,
      args: ['exec', '--sandbox', 'read-only', '--ephemeral', '--ignore-user-config', '--ignore-rules', '--color', 'never', request],
    };
  }
  if (engine === 'cursor') {
    return { engine, bin: profile.bin, args: ['--trust', '-p', '--mode', 'ask', '--sandbox', 'enabled', request] };
  }
  if (engine === 'devin') {
    return { engine, bin: profile.bin, args: ['-p', request, '--permission-mode', 'auto', '--sandbox', '--respect-workspace-trust', 'false'] };
  }
  if (engine === 'grok') {
    return {
      engine,
      bin: profile.bin,
      args: ['--no-memory', '--no-subagents', '--permission-mode', 'plan', '--sandbox', 'read-only', '-p', request],
    };
  }
  if (engine === 'droid') {
...
async function runEngineAskJobs(jobs, {
  ...
  let nextIndex = 0;
  const execute = executeAskJob || (async (job) => runAskProcess(
    buildReadOnlyEngineInvocation(job.engine, job.prompt),
    { cwd: root, timeoutMs }
  ));
