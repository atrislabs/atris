...
const { createEngineTerminalEmitter, engineTerminalStatus } = require('./engine-job-lifecycle');
...
function runAskProcess(invocation, {
  ...
  return new Promise((resolve) => {
    ...
    const finish = (code, closeSig, spawnError) => {
      if (finished) return;
      finished = true;
...
async function runEngineAskJobs(jobs, {
  ...
  let nextIndex = 0;
  const emit = createEngineTerminalEmitter(onTerminal);
  ...
  function recordAnswer(index, job, result) {
    const answer = { ...job, ...result };
    answers[index] = answer;
    const event = emit(`${index}:${job.label || job.engine}`, answer);
    if (event) answers[index] = { ...answer, status: event.status };
  ...
  async function work() {
    while (true) {
      ...
      if (signal && signal.aborted) {
        recordAnswer(index, job, cancelledAskResult(job));
        continue;
      }
      try {
        assertAskModelSupported(job.engine, job.model);
        recordAnswer(index, job, await execute(job, { root, timeoutMs, index, signal }));
      } catch (error) {
        recordAnswer(index, job, {
          ok: false,
...
function atomicWriteEngineAskReceipt(receiptPath, receipt) {
  const tmpPath = path.join(
    ...
    `.${path.basename(receiptPath)}.${process.pid}.${crypto.randomBytes(4).toString('hex')}.tmp`
  );
  try {
...
function printAskAnswer(answer, output = console.log) {
  output(`\n${answer.label} (${answer.engine})`);
  const body = String(answer.stdout || '').trim() || String(answer.stderr || '').trim();
  if (answer.ok) output(body);
  else output(`failed: ${compactFailure(answer)}`);
}
...
async function runEngineAskCommand(args, root = process.cwd(), deps = {}) {
  ...
  let answers;
  try {
    answers = await runEngineAskJobs(parsed.jobs, {
      root,
      ...
      signal: abort.signal,
      onTerminal: (answer) => {
        if (typeof deps.onTerminal === 'function') deps.onTerminal(answer);
        if (!parsed.json) printAskAnswer(answer);
      },
    });
    ...
    const status = interruptedSignal || receipt.summary.cancelled ? 'cancelled' : 'completed';
    writeFinal({
      ...receipt,
      status,
    ...
    if (parsed.json) {
      console.log(JSON.stringify({ ...receipt, status, receipt_path: relativeReceiptPath }, null, 2));
    } else {
      console.log('\nsummary:');
