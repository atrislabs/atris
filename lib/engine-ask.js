...
function buildReadOnlyEngineInvocation(engineName, prompt, modelName = '') {
  ...
  if (engine === 'cursor') {
    return { engine, bin: profile.bin, args: ['--trust', ...(model ? ['--model', model] : []), '-p', '--mode', 'ask', '--sandbox', 'enabled', request] };
  }
  if (engine === 'devin') {
...
async function runEngineAskJobs(jobs, {
  ...
  const execute = executeAskJob || (async (job) => runAskProcess(
    buildReadOnlyEngineInvocation(job.engine, job.prompt, job.model),
    { cwd: root, timeoutMs }
  ));
