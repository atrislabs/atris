...
const { engineTerminalStatus } = require('./engine-job-lifecycle');
...
async function runEngineAskJobs(jobs, {
  ...
  const execute = executeAskJob || (async (job) => {
    ...
    return runAskProcess(invocation, { cwd: invocation.cwd || root, timeoutMs, signal });
  });
  ...
  async function work() {
    while (true) {
      ...
      try {
        ...
        answers[index] = { ...job, ...(await execute(job, { root, timeoutMs, index, signal })) };
      } catch (error) {
        answers[index] = {
          ...job,
          ok: false,
