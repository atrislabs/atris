...
function wishLabel(wish) {
  ...
  return shortRecordLabel(wish, text, { wordLimit: wishLabelWordLimit(text) });
}
...
function delegateWish(wish, audit, root, asJson, options = {}) {
  ...
  if (asJson) console.log(JSON.stringify(payload, null, 2));
    ...
    printGranted(options.taskText || wish.text, audit, { ...options, verifyPlan, engine: record.engine, wish });
    if (record.attached_to_existing_flight) console.log(`attached to existing flight ${record.mission_id}`);
  }
  return 0;
}
...
function printDecomposed(wish, parts, delegatedParts, waitingParts) {
  ...
  delegatedParts.forEach((part) => {
    ...
    if (part.attached_to_existing_flight) console.log(`attached to existing flight ${part.mission_id}`);
  });
