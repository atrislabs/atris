...
function machineRecord(wish, status, audit, extra = {}) {
  const record = {
    ...
    validator: extra.validator || (audit && audit.validator ? audit.validator.id : null),
...
function runCapturedWish(text, root = process.cwd(), options = {}) {
  ...
  return delegateWish(wish, audit, root, asJson, {
    ...
    ...(options.verifyPlan ? {
      verifyPlan: options.verifyPlan,
      userVerifyProvided: options.userVerifyProvided === true,
    } : {}),
  });
}
