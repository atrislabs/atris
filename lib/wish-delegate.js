...
function printList(root = process.cwd()) {
  ...
  wishes.forEach((wish, index) => {
    console.log(`${index + 1}. ${wishLabel(wish)} - ${operatorStatus(wish, root)}${reviewedMarker(wish)}`);
  });
  return 0;
}
...
function waitingAnswerWishes(root = process.cwd()) {
  return readWishes(root).filter((wish) => wish && wish.id
    && WAITING_INPUT_STATUSES.has(String(wish.status || ''))
    && !wish.dispatched_at);
}
...
function answerWish(positionals, root = process.cwd(), options = {}) {
  ...
  if (parts.length > 1 && explicitAnswerRef(parts[0])) {
    wish = waiting.find((row) => wishMatchesRef(row, parts[0]));
    if (!wish) {
  ...
  if (!wish) {
    wish = waiting.reduce((latest, row) => (String(row.ts || '') >= String(latest.ts || '') ? row : latest), waiting[0]);
  }
