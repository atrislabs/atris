...
function metricVerifierCommand(parsed, workspace) {
  const backend = resolveBackendRoot(workspace);
  if (!backend) throw new Error(BACKEND_ROOT_HINT);
  const quoted = `'${backend.replace(/'/g, `'\\''`)}'`;
  return `cd ${quoted} && venv/bin/python backend/scripts/metric_verify.py ${parsed.metric} ${parsed.flag} ${parsed.target}`;
}
...
function readWishReviewRewardRows(root = process.cwd()) {
  ...
  if (!backend) return [];
  const file = path.join(backend, '.atris', 'state', 'scorecards.jsonl');
...
function printRewards(root = process.cwd()) {
  ...
  if (!resolveBackendRoot(root)) console.error(BACKEND_ROOT_HINT);
...
function startWishDelegation(wish, audit, root, options = {}) {
  ...
  let verifyPlan = metricPlan
    ? { command: metricVerifierCommand(metricPlan, root), outcome: `metric ${metricPlan.expression} is met`, status: 'metric' }
  ...
  // The build roster can pin a model for this engine; carry it onto the
  // mission so the runner does not fall back to the engine default.
  // Its effort and time cap ride along the same way.
  const rosterPicked = Boolean(audit.executor && audit.executor.id === engine);
  const rosterModel = audit.executor && audit.executor.id === engine && audit.executor.roster_model
    ? audit.executor.roster_model
    : '';
  const now = stampIso();
  ...
  const missionPayload = withRoot(root, () => {
    ...
    return startMission(missionArgs, {
      ...
      missionPatch: {
          ...
          ...(rosterPicked && audit.executor.roster_effort ? { roster_effort: audit.executor.roster_effort } : {}),
          ...(rosterPicked && audit.executor.roster_max_seconds ? { roster_max_seconds: audit.executor.roster_max_seconds } : {}),
...
function runCapturedWish(text, root = process.cwd(), options = {}) {
  ...
  if (options.metric?.ok && !resolveBackendRoot(root)) {
...
function applyAnswer(wish, answer, root, options = {}) {
  const asJson = !!options.asJson;
  const noMission = !!options.noMission;
  if (wish.metric && !resolveBackendRoot(root)) {
    console.error(BACKEND_ROOT_HINT);
    return 2;
  }
  let engineOverride = '';
