...
function evaluateAutoAccept(task, options = {}) {
  // strict by default (master); executeVerify=false lets dry-run previews
  // skip running recorded verify commands (this branch's status-hang fix).
  const { strictVerify = true, minPasses = AGENT_CERTIFICATION_REVIEW_PASSES, executeVerify = true } = options;
