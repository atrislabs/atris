...
function evaluateAutoAccept(task, options = {}) {
  // strict by default; executeVerify=false lets dry-run previews skip running
  // recorded verify commands (status-hang fix); acceptAll widens the sweep.
  const { strictVerify = true, minPasses = AGENT_CERTIFICATION_REVIEW_PASSES, executeVerify = true, acceptAll = false } = options;
  ...
  let verifyExecuted = null;
  if (strictVerify) {
    const verify = metadata.verify;
    if (!verify) return strictVerifyMissingResult(ref);
    // Previews (dry-run, autoland status) must stay fast: validate the command
    // against the allowlist but do not execute it. Only live accepts pay the
    // execution cost - running dozens of recorded checks during a status render
    // is what made `autoland --json` hang on a large review queue.
    const parsed = parseVerifyCommand(verify);
    if (!parsed.ok) {
      return { eligible: false, ref, reason: parsed.reason, verify };
    }
    if (executeVerify) {
  ...
  return {
    ...
    ...(verifyExecuted === null ? {} : { verify_executed: verifyExecuted }),
  };
}
