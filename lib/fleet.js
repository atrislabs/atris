...
function wrapCodexWithWatchdog(cmd, watchdogPath = CODEX_WATCHDOG_SOURCE) {
  ...
  return `${shellSingleQuote(process.execPath)} ${shellSingleQuote(watchdogPath)} --startup-deadline 90 --max-runtime 3600 -- sh -c ${shellSingleQuote(cmd)} </dev/null`;
}

function buildEngineCommand(engineName, promptFile, { yolo = false, sealed = false, allowedTools = FLEET_ALLOWED_TOOLS, watchdogPath = CODEX_WATCHDOG_SOURCE } = {}) {
  ...
  try {
    ...
    if (engineName === 'codex') cmd = wrapCodexWithWatchdog(cmd, watchdogPath);
    return cmd;
  } finally {
...
function dispatchToEngine({ task, engine, worktreePath, root = process.cwd(), timeoutMs = null, runner = null, prompt: promptOverride = '', yolo = false, sealed = false, briefId = '', skipBriefCapture = false, environment = null, allowedTools = FLEET_ALLOWED_TOOLS }) {
  ...
  const command = buildEngineCommand(engine, promptFile, { yolo, sealed, allowedTools, watchdogPath });
