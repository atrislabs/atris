...
function wrapCodexWithWatchdog(cmd) {
  ...
  return `node ${watchdog} --startup-deadline 90 --max-runtime 3600 -- sh -c ${quoted} </dev/null`;
}
...
function buildEngineCommand(engineName, promptFile, { yolo = false, sealed = false, allowedTools = FLEET_ALLOWED_TOOLS } = {}) {
  ...
  try {
    ...
    if (engineName === 'codex') cmd = wrapCodexWithWatchdog(cmd);
    return cmd;
  } finally {
