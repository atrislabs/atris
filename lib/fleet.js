...
const { mapNotesPath } = require('./map-refs');
const {
...
function buildFleetPrompt(task, { worktreePath, yolo = false } = {}) {
  ...
  lines.push(
    'Rules:',
    mapNotesPath(worktreePath || process.cwd())
      ? '- Read atris/MAP.md first to locate the code, then atris/refs/MAP-NOTES.md for exact file:line refs; never guess file locations.'
...
// A roster time cap, in whole seconds, or 0 when none is set.
function capSeconds(value) {
  const seconds = Math.floor(Number(value));
  return Number.isFinite(seconds) && seconds > 0 ? seconds : 0;
}

// The pin a validator runs with: a model string (older callers) or
// { model, effort, max_seconds } from the roster.
function validatorPin(validatorModels, engine) {
  const pin = validatorModels && validatorModels[engine];
  if (!pin) return { model: '', effort: '', maxSeconds: 0 };
  if (typeof pin === 'string') return { model: pin, effort: '', maxSeconds: 0 };
  return { model: String(pin.model || ''), effort: String(pin.effort || ''), maxSeconds: capSeconds(pin.max_seconds) };
}

function wrapCodexWithWatchdog(cmd, watchdogPath = CODEX_WATCHDOG_SOURCE, receiptPath = '', maxSeconds = 0) {
  ...
  const maxRuntime = capSeconds(maxSeconds) || 3600;
  return `${shellSingleQuote(process.execPath)} ${shellSingleQuote(watchdogPath)} --startup-deadline 90 --max-runtime ${maxRuntime}${receiptArg} -- sh -c ${shellSingleQuote(cmd)} </dev/null`;
}

function buildEngineCommand(engineName, promptFile, { yolo = false, sealed = false, allowedTools = FLEET_ALLOWED_TOOLS, watchdogPath = CODEX_WATCHDOG_SOURCE, watchdogReceiptPath = '', model = '', effort = '', maxSeconds = 0 } = {}) {
  ...
  try {
    let cmd = buildRunnerCommand({ promptFile, allowedTools, ...(model ? { model } : {}), ...(effort ? { effort } : {}) });
    ...
    if (engineName === 'codex') cmd = wrapCodexWithWatchdog(cmd, watchdogPath, watchdogReceiptPath, maxSeconds);
    return cmd;
  } finally {
...
// Every parent signal guard a dispatch installs, so one dispatch can tell
// another dispatch's guard from a handler someone else added.
const DISPATCH_PARENT_GUARDS = new WeakSet();
...
function spawnWithOutputChunks(executable, args, options = {}) {
  ...
  // detached makes the child its own process-group leader, so a time cap can
  // stop the engine the shell launched and not just the shell. Windows has no
  // process groups and keeps the direct child kill.
  ...
  // If this process dies while the child runs, take the child's group down
  // with it so an interrupted dispatch does not orphan the engine. A detached
  // child leads its own group, so the terminal's Ctrl-C never reaches it, and
  // Node's default signal exit skips 'exit' listeners. Guard the termination
  // signals too: kill the group, drop every listener this dispatch added,
  // then re-raise the same signal so the parent still dies the way it would
  // have. Re-raise only when nothing but dispatch guards listened when this
  // one installed, and nothing listens now: a handler added with
  // process.once has already removed itself by the time this runs, and its
  // async cleanup owns exiting. Other dispatches' guards do not count at
  // install, so the last of several running dispatches still re-raises.
  const killGroupOnParentExit = () => {
    try { process.kill(-child.pid, 'SIGKILL'); } catch {}
  };
  const parentGuards = [['exit', killGroupOnParentExit]];
  const othersAtInstall = new Map();
  const setParentGuards = (install) => {
    for (const [event, handler] of parentGuards) {
      if (install) {
        othersAtInstall.set(event, process.listeners(event).filter((fn) => !DISPATCH_PARENT_GUARDS.has(fn)).length);
        DISPATCH_PARENT_GUARDS.add(handler);
  ...
  for (const signal of ['SIGINT', 'SIGTERM', 'SIGHUP']) {
    parentGuards.push([signal, () => {
      killGroupOnParentExit();
      setParentGuards(false);
      if (othersAtInstall.get(signal) === 0 && process.listenerCount(signal) === 0) {
        process.kill(process.pid, signal);
      }
    }]);
  }
  if (processGroups && typeof child.pid === 'number') {
    setParentGuards(true);
  }
...
function dispatchToEngine({ task, engine, worktreePath, root = process.cwd(), timeoutMs = null, runner = null, prompt: promptOverride = '', yolo = false, sealed = false, briefId = '', skipBriefCapture = false, environment = null, allowedTools = FLEET_ALLOWED_TOOLS, liveLogPath = '', model = '', effort = '', maxSeconds = 0 }) {
  ...
  // A roster time cap replaces the default backstop. Codex keeps a minute of
  // slack past its watchdog so the watchdog's own receipt gets written.
  const cap = capSeconds(maxSeconds);
  const defaultTimeoutMs = cap
    ? cap * 1000 + (engine === 'codex' ? 60000 : 0)
    : (engine === 'codex' ? CODEX_DISPATCH_TIMEOUT_MS : DEFAULT_DISPATCH_TIMEOUT_MS);
  const dispatchTimeoutMs = timeoutMs === null || timeoutMs === undefined ? defaultTimeoutMs : timeoutMs;
...
function reviewOnlyEngineEnvironment(worktreePath, options = {}) {
  ...
  const environment = {
    ATRIS_OWNER_NAMES: [...new Set(ownerIdentity().names)].join(','),
...
async function runIndependentValidator({
  ...
  const dispatch = validatorDispatcher || ((entry) => Promise.resolve(dispatchToEngine({
    ...validatorPin(validatorModels, entry.engine),
    task,
    engine: entry.engine,
    worktreePath,
    root,
...
async function runDispatchFlight({
  ...
  scoutAsk = null,
  // Pinned model, effort, and time cap for the requested engine only; a
  // restaffed fallback engine runs on its own defaults. validatorModels maps
  // a validator engine to its model, or to { model, effort, max_seconds }.
  model = '',
  effort = '',
  maxSeconds = 0,
  validatorModels = null,
  ...
  const dispatch = dispatcher || ((entry) => new Promise((resolve) => {
    resolve(dispatchToEngine({
      ...
      skipBriefCapture: true,
      liveLogPath,
      model: entry.engine === engine ? model : '',
      effort: entry.engine === engine ? effort : '',
      maxSeconds: entry.engine === engine ? maxSeconds : 0,
    }));
  }));
  const restaffState = { used: false };
