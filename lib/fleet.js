...
function runInReapedProcessGroup(executable, args, options, controlDir, statusFile) {
  ...
  fs.writeFileSync(supervisor, [
    ...
    "const child = spawn(executable, args, { cwd: process.cwd(), env: process.env, detached: true, stdio: ['ignore', 'inherit', 'inherit', leaseFd] });",
    ...
    "  try {",
    "    const saved = Number(fs.readFileSync(statusFile, 'utf8').trim());",
    "    if (Number.isInteger(saved) && saved >= 0 && saved <= 255) exitCode = saved;",
    "  } catch {}",
...
function sandboxLifecycleWrapper(runtimeDir) {
  ...
  const exec = runner || ((cmd) => {
    ...
    try {
      ...
      const args = sandboxProfile
        ? ['-p', sandboxProfile, lifecycleWrapper, statusFile, '/bin/sh', '-c', cmd]
        : ['-c', cmd];
      const spawnOptions = {
        cwd: worktreePath,
        env: childEnv,
...
function reviewOnlyEngineEnvironment(worktreePath, options = {}) {
  ...
  const lifecycle = sandboxLifecycleWrapper(runtimeTmp);
  let sandboxProfile;
  try {
...
function defaultTrustedVerifyRunner(command, cwd) {
  ...
  const args = sandboxProfile
    ? ['-p', sandboxProfile, lifecycleWrapper, statusFile, parsed.argv[0], ...parsed.argv.slice(1)]
    : parsed.argv.slice(1);
  let result;
  try {
    const spawnOptions = {
