  ...
  for (const { entry, result } of results) {
    ...
    if (landed.ok) {
      ...
      cli([
        ...
        '--proof', `Built by ${activeEngine} engine in fleet flight.${restaffProof} Landed via worktree ship gate (rebase-before-ship, verify re-run). Receipt saved at ${path.relative(root, receiptPath)}. Report tail: ${clipHeadTail(String(result.report || ''), { head: 400, tail: 300 }).replace(/\n/g, ' ')}`,
