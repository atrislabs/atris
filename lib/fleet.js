...
function buildFleetPrompt(task, { worktreePath, yolo = false } = {}) {
  ...
  const commitRule = yolo
    ? 'Commit on the current branch with a plain-English message, then land it yourself: run atris worktree ship --message "<msg>" --verify "<your focused node --test command>" --merge and report the PR URL. If ship reports a rebase conflict or the verify fails, stop and report; never resolve conflicts yourself.'
...
function buildEngineCommand(engineName, promptFile, { yolo = false } = {}) {
  ...
  try {
    let cmd = buildRunnerCommand({ promptFile, allowedTools: FLEET_ALLOWED_TOOLS });
...
async function dispatchEntryWithRestaff({
  ...
  const runOnce = async (engineName) => {
    try {
      return normalizeDispatchResult(await dispatch({ ...entry, engine: engineName }), engineName);
    } catch (err) {
...
function dispatchToEngine({ task, engine, worktreePath, root = process.cwd(), timeoutMs = 900000, runner = null, prompt: promptOverride = '', yolo = false }) {
  ...
  const command = buildEngineCommand(engine, promptFile, { yolo });
...
async function runFleetFlight({
  ...
  for (const { entry, result } of results) {
    ...
    if (landed.ok) {
      ...
      cli(['task', 'ready', String(entry.task.display_id), '--proof', `Built by ${activeEngine} engine in fleet flight.${restaffProof} Landed via worktree ship gate (rebase-before-ship, verify re-run). Receipt saved at ${path.relative(root, receiptPath)}. Report tail: ${String(result.report || '').slice(-300).replace(/\n/g, ' ')}`, '--as', `fleet-${activeEngine}`]);
...
async function runDispatchFlight({
  ...
  for (const taskId of ids) {
    ...
    prepared.push({ task, taskId, worktreePath: wt.trim(), engine });
  ...
  for (const { entry, result } of results) {
    ...
    if (result.exitCode !== 0) {
      const paused = { task: entry.taskId, engine: activeEngine, stage: 'build', detail: String(result.stderr || '').slice(0, 300) };
      if (result.restaffed) paused.restaffed = result.restaffed;
      if (result.deadEngine) paused.deadEngine = result.deadEngine;
      flight.paused.push(paused);
    ...
    if (yolo) {
      ...
      if (selfLanded.ok) {
        flight.landed.push({ task: entry.taskId, engine, landing: 'self', target: selfLanded.target || DISPATCH_SELF_LAND_TARGET });
    ...
    if (landed.ok) {
      ...
      flight.paused.push({ task: entry.taskId, engine: activeEngine, ...(result.restaffed ? { restaffed: result.restaffed } : {}), ...landed });
