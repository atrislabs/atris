...
// binary a dangling symlink, and every cron, loop, and mission dies with
// 'command not found'. The failure is silent and total and reads like a
// shell/PATH problem, not a worktree-cleanup one (found live 2026-07-26,
// CLI-1193; same shape as the cron PATH wedge that cost 20 days of autoland).
...
async function runFleetFlight({
  ...
  const land = lander || (({ entry }) => {
    ...
    if (shipped.status !== 0 || !/done: worktree shipped/.test(shipped.stdout)) {
      return { ok: false, stage: 'ship', detail: clipHeadTail(shipped.stderr || shipped.stdout, { head: 400, tail: 300 }) };
    }
...
async function runDispatchFlight({
  ...
  const land = lander || (({ entry }) => {
    ...
    if (shipped.status !== 0 || !/done: worktree shipped/.test(shipped.stdout)) {
      return { ok: false, stage: 'ship', detail: clipHeadTail(shipped.stderr || shipped.stdout, { head: 400, tail: 500 }) };
    }
    return {
      ok: true,
      stage: 'shipped',
