...
const { createEngineTerminalEmitter, engineTerminalReason } = require('./engine-job-lifecycle');
...
function runInReapedProcessGroup(executable, args, options, controlDir, statusFile) {
  ...
  const spawnOptions = { ...options, env: spawnEnv };
...
function detectDeadEngineDispatch(result) {
  ...
  const output = dispatchResultOutput(result).trim();
  if (exitCode == null) return { reason: 'unknown', exitCode: null };
  if (exitCode === 0) {
    return output ? null : { reason: 'no_output', exitCode: 0 };
  }
  const loweredOutput = output.toLowerCase();
  const pattern = DEAD_ENGINE_OUTPUT_PATTERNS.find((p) => loweredOutput.includes(p));
...
function dispatchToEngine({ task, engine, worktreePath, root = process.cwd(), timeoutMs = null, runner = null, prompt: promptOverride = '', yolo = false, sealed = false, briefId = '', skipBriefCapture = false, environment = null, allowedTools = FLEET_ALLOWED_TOOLS }) {
...
function dispatchResultToTerminal(entry, result) {
  const timedOut = Boolean(result && result.timed_out);
  const cancelled = Boolean(result && result.cancelled);
  const exitCode = dispatchResultExitCode(result);
  const payload = {
    engine: (result && result.engine) || entry.engine,
    task: entry.taskId || (entry.task && entry.task.display_id) || '',
    exit_code: exitCode,
    timed_out: timedOut,
    cancelled,
    stdout: (result && result.report) || '',
...
function attachDispatchEngineTerminal(log) {
  const emit = createEngineTerminalEmitter();
  return function logDispatchEngineTerminal(entry, result) {
    const payload = dispatchResultToTerminal(entry, result);
    const key = `${payload.engine}:${payload.task}:${(result && (result.brief_id || result.promptFile)) || ''}`;
    const event = emit(key, payload);
    if (!event || typeof log !== 'function') return event;
    const name = String(event.engine || 'engine').padEnd(8);
    if (event.status === 'answered') {
      log(`    ${name} answered ${event.task}`);
      const output = String(event.stdout || '').trim();
      if (output) log(clipHeadTail(output, { head: 800, tail: 400 }));
    } else if (event.status === 'timed out') {
      log(`    ${name} timed out ${event.task}`);
    } else if (event.status === 'cancelled') {
      log(`    ${name} cancelled ${event.task}`);
    } else {
      const detail = String(event.stderr || '').trim().split(/\r?\n/).find(Boolean);
      log(`    ${name} failed ${event.task}${detail ? `: ${detail}` : ''}`);
    }
    return event;
  };
}
...
function defaultSelfLandCheck({ worktreePath, targetRef = DISPATCH_SELF_LAND_TARGET, startCommit = '', entry = null, git = null } = {}) {
  ...
  if (!recordedStart) {
    return {
      ...
      reason: 'unknown_start_commit',
      target: targetRef,
      detail: 'dispatch start commit was not recorded',
    };
  }
...
async function runFleetFlight({
  ...
  const logEngineTerminal = attachDispatchEngineTerminal(log);
  ...
  const results = await Promise.all(prepared.map((entry) =>
    dispatchEntryWithRestaff({
      ...
      logEngineTerminal(entry, result);
      return { entry, result };
    })
  ));
    ...
    if (yolo) {
      ...
      if (selfLanded.ok) {
        ...
        flight.landed.push({
          ...
          target,
          verifier_result: {
            command: `HEAD differs from dispatch start commit; git merge-base --is-ancestor HEAD ${target}`,
            passed: true,
            status: 0,
