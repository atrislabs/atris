...
function buildEngineCommand(engineName, promptFile, { yolo = false, sealed = false, allowedTools = FLEET_ALLOWED_TOOLS, watchdogPath = CODEX_WATCHDOG_SOURCE, watchdogReceiptPath = '', model = '' } = {}) {
  ...
  try {
    let cmd = buildRunnerCommand({ promptFile, allowedTools, ...(model ? { model } : {}) });
...
function dispatchToEngine({ task, engine, worktreePath, root = process.cwd(), timeoutMs = null, runner = null, prompt: promptOverride = '', yolo = false, sealed = false, briefId = '', skipBriefCapture = false, environment = null, allowedTools = FLEET_ALLOWED_TOOLS, liveLogPath = '', model = '' }) {
...
async function runIndependentValidator({
  ...
  const dispatch = validatorDispatcher || ((entry) => Promise.resolve(dispatchToEngine({
    ...
    model: validatorModels && validatorModels[entry.engine] || '',
  })));
...
async function runDispatchFlight({
  ...
  scoutAsk = null,
  // Pinned model for the requested engine only; a restaffed fallback engine
  // runs on its own default. validatorModels maps validator engine to model.
  model = '',
  validatorModels = null,
} = {}) {
