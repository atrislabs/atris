...
const RUNNER_COMMAND_TEMPLATE_KEYS = Object.freeze([
  'bin',
  'prompt',
  'promptFile',
  'model',
...
function validateRunnerCommandTemplate(template) {
  const seen = new Set();
  const unknown = [];
  for (const match of String(template || '').matchAll(/\{([A-Za-z][A-Za-z0-9_]*)\}/g)) {
    const key = match[1];
    seen.add(key);
    if (!RUNNER_COMMAND_TEMPLATE_KEYS.includes(key)) unknown.push(key);
  }
  if (unknown.length) {
    throw new Error(`Unknown runner command template placeholder "{${unknown[0]}}". Supported placeholders: ${RUNNER_COMMAND_TEMPLATE_KEYS.map((k) => `{${k}}`).join(', ')}`);
  }
  if (!seen.has('prompt') && !seen.has('promptFile')) {
    throw new Error('Runner command template must include {prompt} or {promptFile} so the autonomous runner receives the task prompt.');
  }
}

function renderRunnerCommandTemplate(template, { promptFile, allowedTools, model }) {
  validateRunnerCommandTemplate(template);
