...
const RUNNER_PROFILE_DEFS = Object.freeze({
  ...
  // devin and grok take --model only when one is pinned (a roster pick or a
  // mission model); otherwise each rides its own CLI default. {pinnedModelFlag} renders to nothing when nothing is pinned.
  devin: Object.freeze({
    bin: 'devin',
    model: '',
  ...
  grok: Object.freeze({
    bin: 'grok',
    model: '',
    commandTemplate: '{bin} --always-approve {pinnedModelFlag} -p {prompt}',
  }),
  agy: Object.freeze({
    bin: 'agy',
...
function renderRunnerCommandTemplate(template, { promptFile, allowedTools, model, pinnedModel = '' }) {
  const allowedToolsFlag = allowedTools ? `--allowedTools ${shellWord(allowedTools)}` : '';
  // The pinned flag takes its leading space with it, so an unpinned launch
  // reads exactly like the template without the placeholder.
...
function buildRunnerCommand({ promptFile, allowedTools, model } = {}) {
  ...
  if (template) {
    return renderRunnerCommandTemplate(template, { promptFile, allowedTools, model: resolved, pinnedModel: resolvePinnedRunnerModel({ model }) });
  }
