...
function trustedAllowRules(policy) {
  ...
  if (tools.has('Read') || tools.has('Glob') || tools.has('Grep')) rules.push('Read(/**)');
  if (tools.has('Edit') || tools.has('Write')) rules.push('Edit(/**)');
  for (const tool of ['Skill', 'WebFetch', 'WebSearch', 'Bash']) {
    if (tools.has(tool)) rules.push(tool);
  }
  return rules;
}
...
function beginPackRunReceipt(packDir, manifest, policy, options = {}) {
  ...
  const launch = {
    ...
    approvalMode: options.trust ? 'auto-approved-within-declared-ceiling' : 'prompt-within-declared-ceiling',
