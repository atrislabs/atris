...
const ARCHE = {
  ...
  statement(ctx, slide, s, spec) {
    ...
    ctx.box(slide, M, 150, 580, 118, s.text || s.headline || '',
      { family: ctx.F.display, size: 38, color: ctx.C.ink, line: 96 });
    if (s.sub) {
      ctx.box(slide, M, 276, 560, 96, s.sub,
        { family: ctx.F.body, size: 12.5, color: ctx.C.soft, line: 120 });
    }
  },
  columns(ctx, slide, s, spec) {
    ...
    cols.forEach((c, i) => {
      ...
      ctx.box(slide, cx, cy + 32, colW - 6, 148, c.b || c.body || '',
        { family: ctx.F.body, size: 11, color: ctx.C.soft, line: 128 });
    });
  },
  ...
  prose(ctx, slide, s, spec) {
    ...
    paras.forEach((p) => {
      ctx.box(slide, M, y, 580, 88, p,
        { family: ctx.F.body, size: 13.5, color: ctx.C.soft, line: 132 });
      y += 96;
    });
  },
