...
function mergeBuckets(dbBuck, mdBuck, todoPath) {
  ...
  // Callers render "completed" differently by provenance: DB done rows are
  // recent wins the renderer keeps on purpose; markdown-only completed rows
  // are legacy cleanup debt the old contract said to clear.
  out.source = 'db';
  return out;
}
