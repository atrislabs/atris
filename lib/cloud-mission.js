...
function parseCloudRunArgs(args) {
  ...
  return { text, lane, asJson: args.includes('--json') };
}
...
function requestError(result) {
  ...
  return new CloudMissionError(`cloud mission request failed${status}${detail}`, 1);
}

async function enqueueCloudMission({ text, lane = 'fast' }, deps = {}) {
  if (!VALID_CLOUD_LANES.has(lane)) {
    throw new CloudMissionError(`invalid --lane "${lane}". expected fast, pro, or max`, 2);
  }
