...
function checkMapDocs(root, mapText) {
  const refs = existingMapDocs(root).flatMap(doc => {
...
function fixMapDocs(root) {
  const changes = [];
  const left = [];
  for (const doc of existingMapDocs(root)) {
...
// Repo-relative path of the notes file when this workspace has one, else null.
function mapNotesPath(root) {
  return existingMapDocs(root).includes(MAP_REF_DOCS[1]) ? MAP_REF_DOCS[1] : null;
}

module.exports = { MAP_REF_DOCS, existingMapDocs, mapNotesPath, checkMapRefs, checkMapDocs, fixMapRefs, fixMapDocs };
