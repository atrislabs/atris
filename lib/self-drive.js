...
function handleMissionBlocker({ mission, stopReason, workspaceRoot, appendEvent }, injected = {}) {
  ...
  try {
    ...
    deps.dispatchToEngine({
      task,
      engine: engine.id,
      worktreePath: worktree.path,
      root: scopedRoot,
      ...(engine.roster_model ? { model: engine.roster_model } : {}),
      ...(engine.roster_effort ? { effort: engine.roster_effort } : {}),
      ...(engine.roster_max_seconds ? { maxSeconds: engine.roster_max_seconds } : {}),
    });
    projectTasks(deps.taskDb, db, scopedRoot);
