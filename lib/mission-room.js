...
function missionName(input) {
  ...
  if (terms.includes('thinking') && terms.includes('approval')) return 'Thinking Approval Mission Room';
...
function buildPendingResultLanding(name, owner) {
  return {
    schema: 'atris.mission_room_result.v1',
    status: 'pending_goal_run',
    landing: {
      status: 'pending_goal_run',
      changed: `Pending: ${owner} has not run ${name} yet.`,
      checked: 'Pending: the next goal must attach verifier output, receipt path, screenshot, customer signal, or human accept gate.',
      proof: null,
      decision: 'After proof: accept, revise, or continue to the next task-first preview.',
    },
  };
}
...
function buildMissionRoom(input, options = {}) {
  ...
  const result = buildPendingResultLanding(name, owner);
...
function missionRoomLines(room, receiptPath) {
  return [
    ...
    `  Suggested next mission: ${room.proactive_next_mission?.objective || room.goal_chain.first_goal}`,
    `  Result landing: ${room.result?.landing?.status || 'pending_goal_run'} -> changed / checked / proof / accept-or-revise after the goal runs`,
    `  Verifier: ${room.verifier}`,
    `  Share: ${room.share_line}`,
    `  Receipt: ${receiptPath}`,
    `  Next: ${room.next_command}`,
  ];
}
