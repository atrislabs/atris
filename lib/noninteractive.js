...
/**
 * The one place that reads the two env names. Both mean the same thing; any
 * value other than empty, 0, or false turns prompts off.
 */
const NON_INTERACTIVE_ENV_NAMES = ['ATRIS_NONINTERACTIVE', 'ATRIS_NO_INTERACTIVE'];
function envForcesNonInteractive(env = process.env) {
  return NON_INTERACTIVE_ENV_NAMES.some((name) => {
...
function isForcedNonInteractive(args = process.argv.slice(2)) {
  ...
  if (envForcesNonInteractive()) return true;
