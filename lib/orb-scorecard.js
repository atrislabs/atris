...
function readOrbScorecard(root, { days = DEFAULT_DAYS, now = Date.now() } = {}) {
  ...
  const runs = readJsonl(indexPath).filter((row) => {
  ...
  scorecard.ok = runs.filter((row) => Number(row.exitCode) === 0).length;
  scorecard.fail = runs.length - scorecard.ok;
  scorecard.completion_rate = runs.length ? scorecard.ok / runs.length : null;
  scorecard.median_duration_ms = median(runs.map((row) => Number(row.durationMs)));
  scorecard.failures = runs
