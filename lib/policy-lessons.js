...
function syncLessonsMd(root, mined) {
  ...
  const lines = (mined.lessons || []).map((lesson) => ({
    id: lesson.id,
    line: `- **[${today}] policy-${lesson.id}** - ${lesson.status || 'pass'} - ${String(lesson.lesson).replace(/\s*\u2014\s*/g, ', ')} (mined from ${mined.sources.career_xp_receipts} receipts / ${mined.sources.task_episodes} episodes / ${mined.sources.scorecards} scorecards)`,
  }));
