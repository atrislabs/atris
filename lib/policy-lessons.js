...
const { RECEIPT_PATH_PATTERN } = require('./receipt-evidence');
...
const VERIFY_COMMAND_PATTERN = /\b(npm (run )?test|node --test|node --check|node bin\/|pytest|cargo test|go test|make test|atris verify)\b|--verify\b/;
...
function policyHintsForProof(proofText, mined) {
  ...
  for (const lesson of lessons) {
    ...
    } else if (lesson.hint_when === 'proof_missing_receipt_path' && !hasReceiptPath(proof)) {
      hints.push({ id: lesson.id, hint: lesson.lesson });
    }
  }
  return hints;
