...
//
// Cheap local sources only: never the network, never the tool itself. Codex
// keeps a cache of the models it can run; claude takes the names atris knows;
// every other engine lists its seed models.

const KNOWN_CLAUDE_MODELS = Object.freeze(['opus 5.5', 'opus 5', 'sonnet 5', 'haiku 4.5', 'fable 5.1']);

// ~/.codex/models_cache.json unless a path is given. Test runs never read the
// real file unless they point at one on purpose.
function codexModelsCacheFile(options = {}) {
  if (options.codexModelsCachePath) return options.codexModelsCachePath;
  if (process.env.ATRIS_CODEX_MODELS_CACHE_PATH) return process.env.ATRIS_CODEX_MODELS_CACHE_PATH;
  if (process.env.NODE_TEST_CONTEXT) return '';
  return path.join(process.env.CODEX_HOME || path.join(os.homedir(), '.codex'), 'models_cache.json');
}

// Every "slug" in the cache, in order, once each. [] when there is no cache
// or it does not parse.
function readCodexModelSlugs(options = {}) {
  const file = codexModelsCacheFile(options);
  if (!file) return [];
  let data;
  try {
  ...
  const walk = (value, depth) => {
    ...
    if (typeof value.slug === 'string' && value.slug.trim() && !slugs.includes(value.slug.trim())) slugs.push(value.slug.trim());
...
function homeLabel(file) {
  const home = os.homedir();
  const rel = path.relative(home, file);
  return rel && !rel.startsWith('..') && !path.isAbsolute(rel) ? `~/${rel}` : file;
}

// One row per installed engine: the models it offers and where that list
// came from ("from <file>" or "known names").
function availableModels(engines, options = {}) {
  return (engines || []).map((engine) => {
    const id = engine.id;
    if (id === 'codex') {
      const slugs = readCodexModelSlugs(options);
      if (slugs.length) return { engine: id, models: slugs, source: `from ${homeLabel(codexModelsCacheFile(options))}` };
    }
    if (id === 'claude') return { engine: id, models: [...KNOWN_CLAUDE_MODELS], source: 'known names' };
    const models = Array.isArray(engine.models) ? engine.models.filter(Boolean) : [];
    return { engine: id, models, source: 'known names' };
  });
}
