...
- [x] M4: Wired `proposeCandidateHorizons` into `suggestNextTask` as the imagined fallback — `suggestNextTask` is now async, the empty-suggestions branch tries the helper, picks highest-confidence via `reduce`, returns `{ task, why, kind: 'imagined', priority: 99 }`, falls back to `null` on throw; only call site at line 895 awaits. `suggestNextTask.constructor.name === 'AsyncFunction'`; tests 121/123 (matches pre-existing baseline; 2 fails are the unrelated `getContextFiles-dead-return` regression in lessons.md). MAP.md autopilot block reline-numbered for the +13-line shift. (2026-04-08)
