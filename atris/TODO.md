...
- **T21a:** Enumerate the surfaces in scope for T21 by reading `atris/features/human-output/idea.md` (Surface audit) and `examples.md`. List each surface with its capture command (e.g. `atris autopilot --auto --iterations=1`, `atris status`, `atris review`, `atris run --once`). Write the list to the top of `atris/features/human-output/validate.md` under `## Surfaces under test`. Exit: validate.md exists with a surfaces list of ≥3 entries, each with command + expected shape ref. [explore]
- **T21b:** Capture autopilot tick output — run `atris autopilot --auto --iterations=1` from this repo, paste the full default (non-verbose) stdout block into `atris/features/human-output/validate.md` under `## Surface: autopilot tick`. Measure line count + max line width, mark pass/fail against (i) ≤20 lines, (ii) ≤80 chars wide, (iii) plain-language (no box art in default mode), (iv) non-technical reader can state horizon + next step. Exit: section has raw block + 4 check results + overall pass/fail. [explore]
