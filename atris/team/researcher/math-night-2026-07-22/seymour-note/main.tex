...
for oriented graphs on at most 16 vertices\thanks{Working note, not
...
verification, fully machine-verified for all oriented graphs on at most 16
vertices: no counterexample exists on $n \le 16$ vertices. The result
rests on 47 unsatisfiable SAT instances solved by CaDiCaL 3.0.1 with DRAT
...
(The full artifact set spans $n \le 18$ and 63 CNF instances; all 59
completed refutations are drat-trim-verified, including the two largest
proofs, $0.60$\,GB checked in $1{,}689$\,s and $4.23$\,GB checked in
$9{,}671$\,s; the 47 form the $n \le 16$ claim.)
...
and zero counterexamples. At $n = 17, 18$ the six lowest-degree shards are
likewise refuted and verified; the near-tournament top shards hit the
solver cap with no verdict and remain open as machine results, except that
the $n = 17$ tournament shard is closed by Fisher's theorem, so those
orders are reported separately and not claimed. We found no previously published
...
No oriented graph on at most $16$ vertices is a counterexample to
Conjecture~\ref{conj:ssnc}.
\end{theorem}

Kaneko and Locke~\cite{KanekoLocke01} (any counterexample has minimum
out-degree at least $7$) together with the arc-counting bound of
Section~\ref{sec:shards} (minimum out-degree at most
$\lfloor (n-1)/2 \rfloor$) already force $n \ge 15$ for a hypothetical
counterexample; combined with Theorem~\ref{thm:main} it must have
$n \ge 17$. Theorem~\ref{thm:main} itself is the unconditional statement
that does not rely on~\cite{KanekoLocke01}: our shards re-prove the
low-degree cases for $n \le 16$ from scratch rather than citing them.
...
Neither item is load-bearing for Theorem~\ref{thm:main}: the only
tournament-forced shard in range, $n15\_s7$, finished UNSAT and its DRAT
proof is drat-trim-verified, so at $n = 15$ Fisher's theorem and Lemma~T
are independent confirmations, not dependencies. They are load-bearing
only for the $n = 17$ tournament shard $n17\_s8$ discussed in
Section~\ref{sec:beyond}, whose SAT run hit the solver cap with no
verdict; there the closure comes from the published
literature~\cite{Fisher96}, with Lemma~T as the unreviewed in-house
alternative. We keep Lemma~T documented because it is short enough to
audit in one sitting, while noting it is now redundant for the headline
claim.
...
\texttt{2e3b2dc}, upstream \texttt{marijnheule/drat-trim}). All 59
completed refutations have \texttt{s VERIFIED} receipts, including the two
giants: $n15\_s7$ ($0.60$\,GB proof, verified in $1{,}689$\,s) and
$n16\_s7$ ($4.23$\,GB proof, verified in $9{,}671$\,s). No receipt has
ever failed. The receipts were \emph{regenerated from scratch in the write-up session}
...
$n = 9,\dots,16$ it rests on Pipeline A plus the written arguments of
Section~\ref{sec:method}.

\subsection{Auxiliary lane: Cayley digraphs of small groups}

As a targeted probe outside the SAT chain (not part of the headline
claim), all
...
For $n \le 16$: every listed instance is UNSAT with a drat-trim-verified
...
\subsection{Per-order status}\label{sec:beyond}

\begin{itemize}
\item $\mathbf{n = 15}$: all $7$ shards UNSAT by CaDiCaL (top shard
$s_7$: $4{,}643$\,s wall, $0.60$\,GB proof), all $7$ DRAT proofs
drat-trim-verified ($s_7$ receipt: \texttt{s VERIFIED}, $1{,}689$\,s).
Shard $s_7$ forces a $7$-regular tournament, so $n = 15$ is closed twice
over: by the verified DRAT proof and, independently, by Fisher's
theorem~\cite{Fisher96} (or the unreviewed Lemma~\ref{lem:t}). Part of
the headline claim.
\item $\mathbf{n = 16}$: all $7$ shards UNSAT by CaDiCaL ($s_7$:
$19{,}080$\,s wall, $4.23$\,GB proof), all $7$ DRAT proofs
drat-trim-verified ($s_7$ receipt: \texttt{s VERIFIED}, $9{,}671$\,s).
No tournament argument applies at even $n$; the certificate chain alone
carries $n = 16$, and it is now fully proof-verified. Part of the
headline claim.
\item $\mathbf{n = 17, 18}$: shards $s_1$ through $s_6$ UNSAT and
verified. The near-tournament shards ($n17\_s7$, $n17\_s8$, $n18\_s7$,
$n18\_s8$) were attempted and hit the solver cap with verdict
\texttt{c UNKNOWN}: as machine results they are open. $n17\_s8$ forces
an $8$-regular tournament and is closed by Fisher~\cite{Fisher96} (or
Lemma~\ref{lem:t}) regardless of its SAT run; that closure comes from the
literature, not from a certificate. So $n = 17$ is open only at shard
$s_7$, while $n = 18$ is open at shards $s_7$ and $s_8$; neither order
is claimed.
\end{itemize}
...
\item \textbf{The $n = 15, 16$ upgrade.} The two large proof
verifications completed with \texttt{s VERIFIED} receipts
($n15\_s7$: $1{,}689$\,s; $n16\_s7$: $9{,}671$\,s), and the headline of
this note was moved to $n \le 16$ on that basis, by the automated
pipeline. A human should re-read those two receipts
(\texttt{receipts/n15\_s7.drattrim.txt},
\texttt{receipts/n16\_s7.drattrim.txt}) and confirm the wording of the
upgraded claim.
\end{enumerate}
