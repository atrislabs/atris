"""Keep a candidate only when the SessionStart plant score moves from 0 to 1."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the sessionstart-plant keep/revert loop.")
    ...
    if not args.proposal:
        append_result(
            results_path,
            {
                ...
                "proposal": "measure.py",
                "description": "current live pack hook score",
            },
        )
    ...
    for trial_index, proposal in enumerate(args.proposal, start=1):
        ...
        try:
            ...
            # Keep only a measured 0 -> 1 win. A already-guarded tree stays put.
