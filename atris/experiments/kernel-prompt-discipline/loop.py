"""Keep/revert loop for the kernel-prompt-discipline experiment.

Bounded target is system_prompt.txt. Each proposal mutates it; the metric
re-runs; we keep only strict improvements and revert everything else.
"""
...
DEFAULT_TARGET = EXPERIMENT_DIR / "system_prompt.txt"
...
def append_result(results_path: Path, row: dict) -> None:
    ...
    with results_path.open("a", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=[
                "timestamp", "trial", "status",
                "old_score", "new_score", "proposal", "description",
            ],
            delimiter="\t",
        )
        if write_header:
...
def main() -> int:
    ...
    print(f"BASELINE {current_score:.4f}  coverage={baseline['spine_coverage']} words={baseline['words']}")
    ...
    print(f"FINAL {final_measure['score']:.4f}  coverage={final_measure['spine_coverage']} words={final_measure['words']}")
    return 0
