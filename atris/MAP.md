...
- **atris youtube search cache:** `commands/youtube.js:1734` writes the last successful free local search to `~/.atris/youtube-search-cache.json`; `commands/youtube.js:1753` reads it after the existing 429 retry when the normalized query matches and the row is younger than one hour; miss, expiry, or a corrupt file keep `LOCAL_SEARCH_RATE_LIMIT_MESSAGE` and a nonzero exit; paid search hard-refuses on a fresh same-query hit before any paid network call and does not write this file. `commands/youtube.js:1718` `printSearchTeachNext` prints one `next: atris youtube teach <first-url>` after human rows (quoted via `quoteYoutubeUrl` at `commands/youtube.js:2346`); free search calls it from `printSearchRows` at `commands/youtube.js:1725`; paid search with rows calls the same helper after `formatPaidSearchResults` at `commands/youtube.js:1928`; `--json` stays quiet; empty rows print no next-line. Agent path: `atris/skills/youtube/SKILL.md` Feature map (429 is cache or stop, never `--paid`; `--paid` also hard-refuses on a fresh free-cache hit). Regression: `test/youtube-search.test.js`.
- **atris youtube notes:** `commands/youtube.js:1297` runs the bundled `scripts/det/ytnotes` rail with inherited stdio; default `atris youtube notes URL` is stdout only (`commands/youtube.js:1447`); rich ephemeral prints one apply next-step via `lib/apply-gate.js:103` `hintEphemeralApply` (`commands/youtube.js:1513`) and writes no files; thin or failed ephemeral prints none; `--save` of thin notes (no number-with-units and no named mechanism, same bar as teach via `extractTeachNumbers` / `extractTeachMechanisms`) prints `thin: no number or named mechanism. no brief.`, writes no brief/journal/apply/experiment, and exits 2 (`TEACH_THIN_REFUSE` at `commands/youtube.js:2020`). A rich `--save` files `atris/wiki/briefs/youtube-<id>.md` plus a claimable journal line (`commands/youtube.js:599`), mints `atris/experiments/notes-<videoid>/` through `fileTeachExperiment` (`commands/youtube.js:676` `saveRichNotes`, slug at `commands/youtube.js:648`), then `commands/youtube.js:694` `ensureNotesApply` writes one apply sidecar plus a journal Inbox claim that name that pack path and the keep rule (`keep only if measure.py moves 0→1`). The sidecar omits the notes check tokens so a fresh `measure.py` score stays 0. `commands/youtube.js:761` `unsaveYoutubeNotes` deletes the brief, apply sidecar, notes-<id> pack, and every teach-<id>-s* pack if present and stays quiet when they are already gone; `commands/youtube.js:818` `ensureProcessApply` (`required: true`) refuses missing or stub-only Apply with one sentence and exit 2 before any `/agent/process_youtube` call; `commands/youtube.js:28` lists notes as ephemeral unless `--save`; `commands/youtube.js:56` says rich notes/teach mint a keep/revert experiment. Regression: `test/youtube.test.js` and `test/youtube-notes-save.test.js`.
- **atris youtube teach:** `commands/youtube.js:2112` `parseTeachArgs` takes `--section N` (1-based, default 1), `--save`, `--recap TEXT`, `--skip`, `--owed`/`owed`, `next`, and `--json`, plus `recap`/`skip`/`next` subcommands; empty argv (no url, no `--section`) sets `owed`+`resume` instead of help; `commands/youtube.js:2629` `applyTeachResume` reprints unpaid owed (no captions), else `applyTeachNext` teaches the continue-cursor, else `nothing owed` plus the start command; `commands/youtube.js:2271` `extractTeachNumbers` keeps a number only with its unit or nearby claim; `commands/youtube.js:2301` `extractTeachMechanisms` keeps a named head (window/model/cycle) and drops quotes, swears, and caption crumbs; `commands/youtube.js:2340` `teachLessonFromCues` feeds those lists to `formatTeachLesson` (`commands/youtube.js:2372`) and `isThinTeachLesson` (`commands/youtube.js:2376`); printed lesson still has one fail-able check and, when the section is not last, the same `TEACH_RESUME_NEXT` handoff as owed/resume (`next: atris youtube teach recap TEXT or atris youtube teach skip`); last section still prints `next: last section`; `commands/youtube.js:2457` `rememberTeachOwed` writes an ephemeral `.atris/youtube-teach-owed.json` sidecar (cwd, not a wiki brief) after a successful section; recap/skip keep a continue cursor (url + next section) via `commands/youtube.js:2473` `writeTeachContinue` instead of wiping the video; `commands/youtube.js:2547` `printTeachOwed` reads unpaid rows only and prints url-or-id, owed section, and unpaid check (`nothing owed` when empty; `--json` is structured); resume adds one next line (`recap TEXT` / `skip`, or `atris youtube teach <url>` when empty); `commands/youtube.js:2568` `applyTeachRecap` unlocks when recap text contains that chapter's number-with-units or named mechanism, and `--skip` unlocks without claiming they answered; `commands/youtube.js:2528` `printTeachUnlockNext` prints one `next: atris youtube teach next` after a successful unlock when a url is known (`--json` stays quiet); `commands/youtube.js:2607` `applyTeachNext` teaches the next section of the last unlocked video from that sidecar, reprints an unpaid check and exits 2 with no captions, or prints `nothing owed` plus the same start command as bare teach; `commands/youtube.js:2975` `runYoutubeTeach` routes bare resume through `applyTeachResume`, then refuses `--section N` (N>1) with the unpaid check and exit 2 until recap/skip (`--json` stays quiet on that refuse). First section never owes a recap. Recap/skip write no brief, apply, journal, or experiment pack. `commands/youtube.js:2939` `extractTeachSource` pulls yt-dlp chapters plus English captions with no network in tests. Rich ephemeral prints one apply next-step via `lib/apply-gate.js:103` `hintEphemeralApply` (`commands/youtube.js:3051`) and writes no files; thin, no-caption, or failed teach prints none. `--save` of a thin chapter (no number-with-units and no named mechanism) prints `thin: no number or named mechanism. no brief.`, writes no brief/journal/apply sidecar, and exits 2 (`TEACH_THIN_REFUSE` at `commands/youtube.js:2085`). A rich chapter files the brief, mints `atris/experiments/teach-<videoid>-s<N>/`, then `commands/youtube.js:2920` `ensureTeachApply` writes one apply sidecar plus a journal Inbox claim that name that pack path and the keep rule (`keep only if measure.py moves 0→1`; scores 1 only when the fixture contains the check tokens). The sidecar omits the chapter check tokens so a fresh `measure.py` score stays 0. `commands/youtube.js:2395` `teachExperimentSlug` lowercases the video id; `commands/youtube.js:2639` `fileTeachExperiment` mints the pack with `measure.py` derived from the printed fail-able check, plus `program.md` under 1200 chars, `loop.py` keep-0-to-1, `reset.py`, and `results.tsv` (notes `--save` reuses this helper with `notesExperimentSlug`). Thin `--save` still writes no experiment. Teach without `--save` still writes no `atris/` tree. No `/agent/process_youtube`, no `--paid`, no credits. Does not collide with `atris teach`. Agent path: `atris/skills/youtube/SKILL.md` Feature map (bare teach resumes unpaid owed, else the continue-cursor after recap/skip; `teach next` still continues; next `--section` refuses until recap/skip). Regression: `test/youtube-teach.test.js`. Keep/revert pack: `atris/experiments/teach-thin-save/`.
...
- `atris/experiments/x-search-<slug>/` - minted by rich `atris x-search --save` through the same `fileTeachExperiment` helper (`commands/youtube.js:2296`, called from `commands/x-search.js:511` `saveRichXSearch`, slug at `commands/x-search.js:409`); same keep-0-to-1 rule, apply sidecar `x-search-<slug>.apply.md`, thin `--save` writes no pack. Default x-search without `--save` writes no pack (`commands/x-search.js:635`). `atris x-search unsave <query-or-source>` (`commands/x-search.js:488`) deletes this pack with the brief and apply sidecar; leftover pack-only still removes the pack.
...
**Search:** `rg "experimentsCommand|experimentsKeep|experimentsRevert|runMeasureJson|runResetScript|experimentsRun|experimentsCompare|experimentsReplay|buildBenchmarkArtifact|ensureExperimentsFramework|experimentsDaily|experimentsQueue|sessionstart-plant|teach-thin-save|fileTeachExperiment|ensureTeachApply|teachExperimentSlug|notesExperimentSlug|saveRichNotes|xSearchExperimentSlug|saveRichXSearch|unsaveXSearch" commands/experiments.js commands/init.js lib/experiments/daily.js commands/autoland.js commands/youtube.js commands/x-search.js atris/experiments/sessionstart-plant atris/experiments/teach-thin-save`
...
- `lib/engine-ask.js:205-227` (`buildReadOnlyEngineInvocation`) - claude/fable/haiku ask args are `-p --model --tools Read,Glob,Grep,WebSearch,WebFetch --permission-mode plan --no-session-persistence`; current Claude Code rejects `--safe-mode`, so that flag stays off this path. Fable still pins `--model claude-fable-5`. Scout then narrows tools to `Read,Glob,Grep` in `lib/dispatch-scout.js:156-161`. Regressions: `test/engine-ask.test.js:167-179`, `test/dispatch-scout.test.js:112-120`.
- `lib/engine-ask.js:296-312,314-438,536-670` (`osAccountName`, `buildAskSpawnEnv`, `runAskProcess`) - parallel read-only engine questions atomically write a running receipt with pid/pgid before workers start, name an adjacent append-only `live_log`, write each stdout/stderr chunk before exit, finalize every terminal path without dropping the log pointer, fail closed when output or exit metadata is missing, persist each successful or failed engine run so auth/subscription lapses bench the engine until a later success restores it, and pin spawn `USER`/`LOGNAME` to `os.userInfo().username` so Claude/Fable Keychain lookup still finds the login when a host app passed a display-name `USER`; regressions: `test/engine-ask.test.js:109-165,354-412`, `test/engine-job-lifecycle.test.js:23-35`.
