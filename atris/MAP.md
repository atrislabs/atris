...
rg "runCloudWish|cloudWishOptionError|--cloud|findActiveWishFlight|attached_to_existing_flight|resolveDefaultVerifier|deriveVerifyPlan|namedTestFiles|verifierBudgetWarning|wishBodyText|buildClarityQuestions|printQuestion|oneLap|lapOwnsDispatch|dispatchWishHeadlessly|printWishReceptionist" commands/wish.js lib/cloud-mission.js lib/wish-audit.js lib/wish-delegate.js lib/first-minute.js lib/default-verifier.js test/wish-cloud.test.js test/wish.test.js test/first-minute.test.js test/one-lap-router.test.js  # Wish dispatch: empty-folder and post-init bare wish reuse buildFirstMinute like bare atris; help stays usage; a plain sentence still files; vague intake repeats the complete wish and asks one short question grounded in its words; cloud wishes, Check lines, scoped verifiers, suite-window warnings, duplicate local slices, and one-lap ownership stay intact
...
rg "function reviewAtris|renderReviewMinute" commands/workflow.js test/workflow-command.test.js  # Review command: empty folder reuses speakFirstMinute / isFreshWorkspace (same two spoken lines and --json next as bare atris); after init, certified -> accept and empty room stays "nothing is waiting"; --all/--limit/--group-by keep the queue; --verbose legacy validator prompt; headless never prompts
...
- First-minute helpers: `lib/first-minute.js` `pickNext` / `taskCommand` / `deskNextCommand` / `taskNextCommand` / `freshMinuteJson` / `speakFirstMinute`; `commands/wish.js` `printWishReceptionist` reuses `buildFirstMinute` for bare wish; `commands/human-missions.js` `askCommand` and `commands/mission.js` `missionCommand` reuse `speakFirstMinute` for empty-folder and post-init bare ask, and for empty-folder bare mission; `commands/task.js` `cmdNext` reuses `buildFirstMinute` when there is no `atris/` folder and no leftover projection; desk points idle history at `task next`, and `task next` itself names `task new` when the room is already there and nothing is active
...
- Entry: `commands/workflow.js:443-828` (planAtris function)
...
- Human head after init: `buildFirstMinute` when a claimed/open/review task exists (same win + next as bare `atris`)
- Default stays PROMPT ONLY plus the first-minute lines when live work exists; `--verbose`/`--full` or leftover words after `plan` print the navigator paste and file dump
- Missing `team/navigator` after `init --minimal` is optional context, not "run init". Regression: `test/workflow-command.test.js` (`plan in an uninitialized folder talks like first-minute`, `plan after init --yes --minimal does not send you back to init`, `plan names a claimed task the same way first-minute does`), `test/first-minute.test.js` (`atris plan in an empty folder talks like first-minute`)
...
- After init: stay in the room. Certified leads with the win and `atris task accept <id>`. Uncertified stays "still being checked", not needs-you. An empty live room says `nothing is waiting on you.`
...
- `--help` stays usage
- Regression: `test/workflow-command.test.js` (`review in an uninitialized folder talks like first-minute`, `review after init --yes --minimal does not send you back to init`, `review talks like first-minute`), `test/first-minute.test.js` (`atris review in an empty folder talks like first-minute`, `atris review after init --minimal stays in the room`)
...
- **Search:** `rg "shouldGatherContext|saveContextProfile|createStarterTask|buildFirstMinute|freshMinuteJson|speakFirstMinute|isFreshWorkspace|printWishReceptionist|cmdFirstMinute|isUninitializedTaskFolder|deskNextCommand|taskNextCommand|cmdNext|cmdNextTruth|reviewAtris|Context gatherer" bin/atris.js lib/context-gatherer.js lib/first-minute.js commands/task.js commands/workflow.js commands/wish.js commands/human-missions.js commands/mission.js test
...
- `reviewAtris()` / `renderReviewMinute()` → `commands/workflow.js` (empty folder uses speakFirstMinute; after init, first-minute review minute; --verbose validator prompt)
