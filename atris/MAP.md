...
- `lib/engine-ask.js:85`, `lib/engine-ask.js:185`, `lib/engine-ask.js:337`, `lib/engine-ask.js:386`, `lib/engine-ask.js:425` - parallel read-only engine questions: shared and per-job model pins, honest `model_not_supported` failures, engine-specific safe command shapes, capped output and process-group timeouts, bounded concurrency, labeled receipts under `atris/runs/`, and plain completion summaries; routed at `commands/engine.js:1416`; fake-process regressions cover model arguments, fan-out, limits, cleanup, silent output, receipts, and no worktree or lock writes in `test/engine-ask.test.js:57`, `test/engine-ask.test.js:113`, `test/engine-ask.test.js:151`, and `test/engine-ask.test.js:256`.
