...
- **atris youtube teach:** `commands/youtube.js:1962` `parseTeachArgs` takes `--section N` (1-based, default 1) and `--save`; `commands/youtube.js:2072` `extractTeachNumbers` keeps a number only with its unit or nearby claim; `commands/youtube.js:2102` `extractTeachMechanisms` keeps a named head (window/model/cycle) and drops quotes, swears, and caption crumbs; `commands/youtube.js:2141` `teachLessonFromCues` feeds those lists to `formatTeachLesson` (`commands/youtube.js:2173`) and `isThinTeachLesson` (`commands/youtube.js:2177`); printed lesson still has one fail-able check and `next: atris youtube teach "<url>" --section N+1`; `commands/youtube.js:2501` `extractTeachSource` pulls yt-dlp chapters plus English captions with no network in tests; `commands/youtube.js:2537` `runYoutubeTeach` prints stdout only unless `--save`. `--save` of a thin chapter (no number-with-units and no named mechanism) prints `thin: no number or named mechanism. no brief.`, writes no brief/journal/apply sidecar, and exits 2 (`TEACH_THIN_REFUSE` at `commands/youtube.js:1941`). A rich chapter still files the brief + apply stub and exits 0 even without Apply. `commands/youtube.js:2187` `teachExperimentSlug` lowercases the video id; `commands/youtube.js:2209` `fileTeachExperiment` then mints `atris/experiments/teach-<videoid>-s<N>/` with `measure.py` derived from the printed fail-able check (score 1 only when the fixture contains that number-with-units or named mechanism), plus `program.md` under 1200 chars, `loop.py` keep-0-to-1, `reset.py`, and `results.tsv`. Thin `--save` still writes no experiment. Teach without `--save` still writes nothing. No `/agent/process_youtube`, no `--paid`, no credits. Does not collide with `atris teach`. Regression: `test/youtube-teach.test.js`. Keep/revert pack: `atris/experiments/teach-thin-save/`.
...
- `atris/experiments/teach-<videoid>-s<n>/` - minted by rich `atris youtube teach --save`; `measure.py` is derived from that chapter's printed fail-able check and scores 1 only when the fixture contains the number-with-units or named mechanism (`loop.py` keep-0-to-1)
...
**Search:** `rg "experimentsCommand|experimentsRun|experimentsCompare|experimentsReplay|buildBenchmarkArtifact|ensureExperimentsFramework|experimentsDaily|experimentsQueue|sessionstart-plant|teach-thin-save|fileTeachExperiment|teachExperimentSlug" commands/experiments.js commands/init.js lib/experiments/daily.js commands/autoland.js commands/youtube.js atris/experiments/sessionstart-plant atris/experiments/teach-thin-save`
