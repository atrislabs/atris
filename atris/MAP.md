...
```bash
# Core CLI logic
rg "async function interactiveEntry|shouldGatherContext|renderContextGathererPrompt|buildFirstMinute|listUserVisibleWork|freshWinLine|shouldAutoInitFresh|isTaskNextLook|isLeftoverVerbLook|isFirstTalkLine" bin/atris.js lib/context-gatherer.js lib/first-minute.js    # Main entry points: cold-start dispatcher, first-minute bare atris screen, first-contact context gatherer, leftover task next and leftover words after brainstorm/wish/log/plan/do are a look not first-talk, pasted talk line is first-talk not overview, a folder with user-visible files is named not empty, legacy natural-language mode
...
- **Empty folder:** default spoken `atris recap` and `atris recap --json` reuse `speakFirstMinute` / `freshMinuteJson` only when there is no task history and no `atris/` room (same two spoken lines and talk next as bare `atris`, including a named file already here); a folder with task history keeps recap; `--verbose` / `--share` keep the empty report; `--help` stays usage
