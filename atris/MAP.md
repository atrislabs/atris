...
rg "youtubeCommand|processYoutube|runYoutubeNotes|parseNotesArgs|runYoutubeNotesBatch|fileBriefFromNotes|runYoutubeDigest|collectVideoBriefs|tickWatch|watch add|parseSearchArgs|parseSearchStdout|runPaidYoutubeSearch|/youtube/search|--paid|writeLocalSearchCache|readFreshLocalSearchCache|LOCAL_SEARCH_CACHE|Usage: atris youtube|process_youtube|ytnotes|ytsearch" commands/youtube.js bin/atris.js test/youtube.test.js test/youtube-batch.test.js test/youtube-watch.test.js test/youtube-digest.test.js test/youtube-search.test.js test/billed-command-auth.test.js scripts/det/ytnotes scripts/det/ytsearch atris/skills/youtube/SKILL.md  # YouTube CLI: `commands/youtube.js:27` help lists search (free), search --paid (5-credit watch permalinks), notes, process, digest, and watch; `commands/youtube.js:1299` parseSearchArgs accepts --paid; `commands/youtube.js:1414` writeLocalSearchCache stores the last free local rows as {query, savedAt, rows} under ~/.atris/youtube-search-cache.json; `commands/youtube.js:1433` readFreshLocalSearchCache serves that file after the 429 retry if the normalized query matches and age is under one hour; `commands/youtube.js:1557` runPaidYoutubeSearch posts /youtube/search via ensureBilledCommandAuth (scope youtube), mints/retries like x-search, never /auth/cli, and never reads the free cache; `commands/youtube.js:1652` free runYoutubeSearch still prefers PATH ytsearch, then bundled `scripts/det/ytsearch`, then yt-dlp ytsearchN, retries once on LOCAL_SEARCH_RATE_LIMIT_RE, then cache-reads or prints LOCAL_SEARCH_RATE_LIMIT_MESSAGE; `commands/youtube.js:1736` routes search before notes/digest/watch/process; `bin/atris.js` --all help names paid search; `test/youtube-search.test.js` mocks runner, fs, and HTTP (no live credits); process remains the 5-credit cloud knowledge path. Skill Feature map in `atris/skills/youtube/SKILL.md` is the ordered agent path: free search, 429 cache-or-stop, `--paid` only if the user asked to buy, notes, then process; never paste tokens or hit `/auth/cli`.
...
- **atris youtube search cache:** `commands/youtube.js:1414` writes the last successful free local search to `~/.atris/youtube-search-cache.json`; `commands/youtube.js:1433` reads it after the existing 429 retry when the normalized query matches and the row is younger than one hour; miss, expiry, or a corrupt file keep `LOCAL_SEARCH_RATE_LIMIT_MESSAGE` and a nonzero exit; paid search never reads or writes this file. Agent path: `atris/skills/youtube/SKILL.md` Feature map (429 is cache or stop, never `--paid`). Regression: `test/youtube-search.test.js`.
