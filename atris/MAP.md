...
- **atris youtube teach:** `commands/youtube.js:2104` `parseTeachArgs` takes `--section N` (1-based, default 1), `--save`, `--recap TEXT`, `--skip`, and `--json`, plus `recap`/`skip` subcommands; `commands/youtube.js:2246` `extractTeachNumbers` keeps a number only with its unit or nearby claim; `commands/youtube.js:2276` `extractTeachMechanisms` keeps a named head (window/model/cycle) and drops quotes, swears, and caption crumbs; `commands/youtube.js:2315` `teachLessonFromCues` feeds those lists to `formatTeachLesson` (`commands/youtube.js:2347`) and `isThinTeachLesson` (`commands/youtube.js:2351`); printed lesson still has one fail-able check and `next: atris youtube teach "<url>" --section N+1`; `commands/youtube.js:2432` `rememberTeachOwed` writes an ephemeral `.atris/youtube-teach-owed.json` sidecar (cwd, not a wiki brief) after a successful section; `commands/youtube.js:2448` `applyTeachRecap` unlocks when recap text contains that chapter's number-with-units or named mechanism, and `--skip` clears without claiming they answered; `commands/youtube.js:2820` `runYoutubeTeach` refuses `--section N` (N>1) with the unpaid check and exit 2 until recap/skip (`--json` stays quiet on that refuse). First section never owes a recap. Recap/skip write no brief, apply, journal, or experiment pack. `commands/youtube.js:2784` `extractTeachSource` pulls yt-dlp chapters plus English captions with no network in tests. Rich ephemeral prints one apply next-step via `lib/apply-gate.js:103` `hintEphemeralApply` (`commands/youtube.js:2880`) and writes no files; thin, no-caption, or failed teach prints none. `--save` of a thin chapter (no number-with-units and no named mechanism) prints `thin: no number or named mechanism. no brief.`, writes no brief/journal/apply sidecar, and exits 2 (`TEACH_THIN_REFUSE` at `commands/youtube.js:2080`). A rich chapter files the brief, mints `atris/experiments/teach-<videoid>-s<N>/`, then `commands/youtube.js:2765` `ensureTeachApply` writes one apply sidecar plus a journal Inbox claim that name that pack path and the keep rule (`keep only if measure.py moves 0→1`; scores 1 only when the fixture contains the check tokens). The sidecar omits the chapter check tokens so a fresh `measure.py` score stays 0. `commands/youtube.js:2370` `teachExperimentSlug` lowercases the video id; `commands/youtube.js:2484` `fileTeachExperiment` mints the pack with `measure.py` derived from the printed fail-able check, plus `program.md` under 1200 chars, `loop.py` keep-0-to-1, `reset.py`, and `results.tsv` (notes `--save` reuses this helper with `notesExperimentSlug`). Thin `--save` still writes no experiment. Teach without `--save` still writes no `atris/` tree. No `/agent/process_youtube`, no `--paid`, no credits. Does not collide with `atris teach`. Agent path: `atris/skills/youtube/SKILL.md` Feature map (next `--section` refuses until recap/skip). Regression: `test/youtube-teach.test.js`. Keep/revert pack: `atris/experiments/teach-thin-save/`.
