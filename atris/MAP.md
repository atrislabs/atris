...
- **atris pack:** `commands/pack.js:155` publishes an offline Atris brain zip with `pack.json` versioning and denylist filtering, `commands/pack.js:257` installs local zip/url/slug payloads with zip-slip protection, `lib/zip.js:65` writes standard deflated PKZIP archives, `lib/zip.js:158` reads them, `bin/atris.js:517` adds the help line, `bin/atris.js:2635` routes the modular command, and `test/pack.test.js:41` covers round-trip, denylist, zip-slip, and version bump behavior.
