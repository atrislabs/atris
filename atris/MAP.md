...
rg "youtubeCommand|processYoutube|runYoutubeNotes|fileBriefFromNotes|runYoutubeDigest|collectVideoBriefs|tickWatch|watch add|Usage: atris youtube|process_youtube|ytnotes" commands/youtube.js bin/atris.js test/youtube.test.js test/youtube-watch.test.js test/youtube-digest.test.js scripts/det/ytnotes atris/skills/youtube/SKILL.md  # YouTube CLI: `commands/youtube.js:25` help lists notes, process, digest, and watch add/list/remove/tick; `commands/youtube.js:481` files a wiki brief after notes exit 0; `commands/youtube.js:552` parses digest --days (default 7); `commands/youtube.js:582` collects video briefs in the date window and skips digest- plus non-http sources; `commands/youtube.js:666` writes atris/wiki/briefs/digest-YYYY-MM-DD.md and a claimable journal line; `commands/youtube.js:861` watch add writes `{channel, added}` to `.atris/state/youtube_watch.json`; `commands/youtube.js:923` tick fetches three latest ids, seeds a fresh channel, and briefs unseen via ytnotes; `commands/youtube.js:1018` notes wraps bundled `scripts/det/ytnotes`; `commands/youtube.js:1051` routes notes, digest, and watch before process; `bin/atris.js:575` help lists notes, digest, and process; `test/youtube.test.js:246` covers missing-url and non-youtube exit 2; `test/youtube.test.js:265` covers brief + journal filing; `test/youtube-watch.test.js:1` covers add/list/remove, unseen-only briefs, fresh-channel newest-only seed, and failed-fetch skip; `test/youtube-digest.test.js:1` covers window filtering, digest- and non-video skips, filed sources, journal append, and empty-window no-op. Process remains the 5-credit cloud knowledge path.
