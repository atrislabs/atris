...
- **Regression:** `test/first-minute.test.js` (`atris status in an empty folder talks first-talk, not init`; `atris status after minting a file folder talks keep-working, not factory let-it-run`); `test/dogfood-p0-safety.test.js` (`26c`, `26d`, `26e`); `test/commands.test.js` covers `status --help` without `atris/` or `$HOME/.atris`
...
- **Regression:** `test/recap.test.js` (buckets, jargon ban, share format, projection fallback, empty workspace, empty-folder first-minute, certified-only needs-you, spoken default, verbose report, headless mixed board, post-mint claimed file not factory map.md, claimed non-seed still recaps); `test/first-minute.test.js` (`atris status after minting a file folder talks keep-working, not factory let-it-run` also locks recap); `test/dogfood-p0-safety.test.js` (`26e`)
