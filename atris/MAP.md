...
rg "printUsage|spaceship plan|planLines|hasYesFlag|wantsJson|pass --yes to start|spaceship --help|NO_COLOR|isUnboundScratchFolder|refuseUnboundScratch|applyRunnerFlags" commands/spaceship.js commands/autopilot-front.js bin/atris.js lib/scratch-root.js scripts/spaceship.sh test/dogfood-pass2-t1-t5.test.js test/dogfood-p0-safety.test.js test/dogfood-spaceship-scratch.test.js  # spaceship --help is short usage; bare invoke is a plan; --json without --yes is JSON refuse exit 2; --yes starts only in a room; unbound scratch --yes exits 2 with no email, overnight process, or atris/ mint; applyRunnerFlags skips house-engine seed on unbound scratch so autopilot does not write engines.json before the refuse; no color off TTY
...
rg "addTask|claimTask|doneTask|appendTaskCompletionLogs|listTasks|workspaceRoot|findWorkspaceRoot|isGenericScratchRoot|isUnboundScratchFolder|isUnderGenericScratchRoot|applyRunnerFlags" lib/task-db.js lib/scratch-root.js lib/mission-root.js commands/spaceship.js commands/autopilot-front.js bin/atris.js test/scratch-root-room.test.js test/dogfood-spaceship-scratch.test.js  # SQLite task store + workspace walk-up; scratch roots (/tmp, /private/tmp, /var/tmp) are never inherited by child folders; an empty child under those roots is not a room, so spaceship/autopilot --yes refuse (exit 2) instead of minting atris/ or .atris/state/engines.json; applyRunnerFlags skips the house-engine registry seed on unbound scratch
...
- Headless autopilot: `commands/autopilot-front.js:223` refuses bare headless invoke and `--json`/`--once` unless `--yes` or `--auto`; unbound scratch `--yes`/`--auto` still refuse via `isUnboundScratchFolder` before `writeState`; `bin/atris.js` `applyRunnerFlags` skips `resolveDefaultEngine` on unbound scratch so the refuse does not mint `.atris/state/engines.json`; `autopilot stop` kills `child_pid` then the wrapper. Regression: `test/dogfood-p0-safety.test.js` (`32:`), `test/dogfood-pass4.test.js` (`30:`), and `test/dogfood-spaceship-scratch.test.js`.
...
- **atris youtube notes:** `commands/youtube.js:509` runs the bundled `scripts/det/ytnotes` rail with inherited stdio; `commands/youtube.js:470` files `atris/wiki/briefs/youtube-<id>.md` plus a claimable journal line after exit 0; `commands/youtube.js:27` lists notes next to process; `bin/atris.js:575` names the help line; `test/youtube.test.js:246` and `test/youtube.test.js:257` cover usage and non-youtube exit 2; `test/youtube.test.js:265` and `test/youtube.test.js:294` cover brief filing and the silent no-wiki path.
...
- **Autopilot front door:** `commands/autopilot-front.js` (`autopilotFront` main loop; `legPlan` selection — member legs force `--runner atris2`; `driveLeg` records `child_pid` and polls the stop file every 2s so `atris autopilot stop` kills the running leg, not only the wrapper; any leg under 30s counts toward the 5-in-a-row no-progress/failure halt). Bare headless / `--json` / `--once` at `commands/autopilot-front.js:223` exits 2 unless `--yes` or `--auto` is passed. Unbound scratch `--yes`/`--auto` still exit 2 via `isUnboundScratchFolder` before `writeState` (no mission, no `.atris/` mint). `bin/atris.js` `applyRunnerFlags` skips `resolveDefaultEngine` on unbound scratch so that refuse does not seed `.atris/state/engines.json`. A folder that already has `atris/` can still start.
...
- **Regression:** `test/first-minute.test.js`, `test/task-first-minute.test.js`, `test/task-desk-next.test.js`, `test/task-next-truth.test.js`, `test/context-gatherer.test.js`, `test/cli-smoke.test.js`, `test/workflow-command.test.js`, `test/wish.test.js`, `test/human-missions.test.js`, `test/log-inbox.test.js`, `test/scratch-root-room.test.js`, `test/task-workspace-root.test.js`
- **Search:** `rg "shouldGatherContext|saveContextProfile|createStarterTask|buildFirstMinute|freshMinuteJson|speakFirstMinute|isFreshWorkspace|printWishReceptionist|isWishDeskVerb|cmdFirstMinute|isUninitializedTaskFolder|cmdAdd|deskNextCommand|taskNextCommand|cmdNext|cmdNextTruth|reviewAtris|logAtris|Context gatherer" bin/atris.js lib/context-gatherer.js lib/first-minute.js commands/task.js commands/workflow.js commands/wish.js commands/human-missions.js commands/mission.js commands/log.js test
