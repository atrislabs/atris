...
- `commands/slop.js` (`slopCommand`, `RULES` `:28-91`, `PAIR_RULES` `:95-104` — windowed multi-line composites, e.g. `hero-kicker` dot+eyebrow) — deterministic frontend/prose slop detector (HOW copy reads); `detect --fix/--diff/--staged`, project rules in `.atris/slop.rules.json`, `installHook` pre-commit gate; `slop dead` (`findDeadCode` `:364`) — require-graph reachability scan for dead JS files (unreachable from package entrypoints, unreferenced by any string mention) plus test-only orphans
