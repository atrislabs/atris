...
- **Freshness check:** `atris 0-shot --check` and no-request `atris next --check` compare durable latest files, global prompt, per-model prompts, and current source fingerprints, then report fresh, stale, or missing
