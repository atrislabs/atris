...
- `lib/router-brain.js` (`loadRouterHistory`, `computeEngineTaskStats`, `rankEngines`) - receipt-backed engine routing by task type: reads task receipts plus mission events, requires three verified outcomes per ready candidate, scores pass rate, median duration, and recency, then preserves engine registry fallback order whenever evidence is thin; wired through `lib/engine-registry.js` `resolveEngineForRole`; `test/router-brain.test.js`
