...
- **atris youtube search cache:** `commands/youtube.js:1743` writes the last successful free local search to `~/.atris/youtube-search-cache.json`; `commands/youtube.js:1762` reads it after the existing 429 retry when the normalized query matches and the row is younger than one hour; miss, expiry, or a corrupt file keep `LOCAL_SEARCH_RATE_LIMIT_MESSAGE` and a nonzero exit; paid search hard-refuses on a fresh same-query hit before any paid network call and does not write this file. `commands/youtube.js:1723` `printYoutubeTeachNext` prints one `next: atris youtube teach <url>` (quoted via `quoteYoutubeUrl` at `commands/youtube.js:2355`); `commands/youtube.js:1729` `printSearchTeachNext` hands the first row URL to that helper; free search calls it from `printSearchRows` at `commands/youtube.js:1734`; paid search with rows calls the same helper after `formatPaidSearchResults` at `commands/youtube.js:1937`; `--json` stays quiet; empty rows print no next-line. Agent path: `atris/skills/youtube/SKILL.md` Feature map (429 is cache or stop, never `--paid`; `--paid` also hard-refuses on a fresh free-cache hit). Regression: `test/youtube-search.test.js`.
