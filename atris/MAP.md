...
**Search:** `rg "experimentsCommand|experimentsRun|experimentsCompare|experimentsReplay|buildBenchmarkArtifact|ensureExperimentsFramework|experimentsDaily|experimentsQueue|sessionstart-plant|teach-thin-save" commands/experiments.js commands/init.js lib/experiments/daily.js commands/autoland.js atris/experiments/sessionstart-plant atris/experiments/teach-thin-save`
...
- **Dispatch:** `bin/atris.js` `command === 'run'` and `command === 'autopilot'` branches — default routes to the fronts, `--legacy` routes to the old loops below; run skips `applyRunnerFlags` on first-talk and unbound scratch, and autopilot skips it on help and unbound scratch, so engine registry seed cannot mint `.atris/state`
