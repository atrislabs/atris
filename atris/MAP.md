...
- **No-progress judgment:** `mission run` takes the existing honest-stop path after two structurally idle ticks, preserves the stop receipt, and appends one plain decision card to `atris/status/for-you.md` via `appendMissionJudgmentCard()`. Regression: `test/mission-idle-stop.test.js`.
