...
- **atris youtube search cache:** `commands/youtube.js:1729` writes the last successful free local search to `~/.atris/youtube-search-cache.json`; `commands/youtube.js:1748` reads it after the existing 429 retry when the normalized query matches and the row is younger than one hour; miss, expiry, or a corrupt file keep `LOCAL_SEARCH_RATE_LIMIT_MESSAGE` and a nonzero exit; paid search hard-refuses on a fresh same-query hit before any paid network call and does not write this file. `commands/youtube.js:1717` `printSearchRows` prints one `next: atris youtube teach <first-url>` after human rows (quoted via `quoteYoutubeUrl` at `commands/youtube.js:2340`); `--json` stays quiet; empty rows print no next-line. Agent path: `atris/skills/youtube/SKILL.md` Feature map (429 is cache or stop, never `--paid`; `--paid` also hard-refuses on a fresh free-cache hit). Regression: `test/youtube-search.test.js`.
