...
- **atris youtube notes:** `commands/youtube.js:1166` runs the bundled `scripts/det/ytnotes` rail with inherited stdio; `commands/youtube.js:535` files `atris/wiki/briefs/youtube-<id>.md` plus a claimable journal line after exit 0; `commands/youtube.js:639` `ensureNotesApply` then requires one Apply sidecar (`youtube-<id>.apply.md`) with change + receipt, or writes a claimable fill-this stub and exits 2; `commands/youtube.js:27` lists notes next to process; `bin/atris.js:575` names the help line; `test/youtube.test.js:246` and `test/youtube.test.js:257` cover usage and non-youtube exit 2; `test/youtube.test.js:267` and `test/youtube.test.js:296` cover brief filing and the silent no-wiki path; `test/youtube.test.js:332` and `test/youtube.test.js:355` cover notes without Apply (open stub, exit 2) and notes + Apply receipt (complete).
