...
- task next help: `commands/task.js:13269` prints one usage line (`atris task next [--tag <tag>] [--create-next]`) for `--help`/`-h`/`help`/`-?` and does not list, claim, or print the desk. `atris task --help` stays the encyclopedia. Regression: `test/task-next-help.test.js`.
...
- **Handler:** `commands/task.js:13240` (`run` dispatch); default `atris task` is `cmdFirstMinute` at `commands/task.js:6471`; full desk is `cmdHome` at `commands/task.js:6480`
...
- First-minute helpers: `lib/first-minute.js` `pickNext` / `taskCommand` / `deskNextCommand` / `taskNextCommand` / `freshMinuteJson`; `commands/wish.js` `printWishReceptionist` reuses `buildFirstMinute` for bare wish; desk points idle history at `task next`, and `task next` itself names `task new` when nothing is active
...
- **Search:** `rg "shouldGatherContext|saveContextProfile|createStarterTask|buildFirstMinute|freshMinuteJson|printWishReceptionist|cmdFirstMinute|deskNextCommand|taskNextCommand|cmdNextTruth|Context gatherer" bin/atris.js lib/context-gatherer.js lib/first-minute.js commands/task.js commands/workflow.js commands/wish.js test
