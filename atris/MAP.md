...
- **First-message boot:** `bin/atris.js` `interactiveEntry()` and `showWelcomeVisualization()` point initialized workspaces at the current 0-shot route, reason, durable freshness, first command, `atris 0-shot --all`, direct model-tier prompts, direct horizon prompts, and refresh the ambient latest files
