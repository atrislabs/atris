...
rg "ensureNowFile|renderDefaultNow|renderPortfolioNow|currentMissionMoveLine|compactMissionCommand|countOpenWorkItems|countOpenTodoItems|countJournalCompletedReceipts|todayTaskReceiptLines|storeTaskReceipt|now --help" bin/atris.js commands/now.js test/now.test.js test/commands.test.js  # now.md front door: local date, complete current-mission clause, compact untruncated next command, projection-backed open work counts, deduplicated task receipts preserve complete landed results, their why clauses, and human proof states, portfolio signals, non-mutating help
...
- **Honest hidden-move state:** unclear queued moves stay hidden until their owners write clear actions; the digest does not send operators to `atris now`, which cannot reveal them. Regression: `test/autoland.test.js` (`digest and alarm compose in plain language`)
