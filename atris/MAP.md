...
- **Runners & models:** `mission start --runner manual|claude|atris2|codex_goal [--model <id>]`. `claude` spawns the configured local Claude binary (`ATRIS_CLAUDE_BIN`, default `claude`) per tick via `spawnClaudeTick()`; model resolves from `mission.model` -> `ATRIS_CLAUDE_MODEL` -> `opus` in `lib/runner-command.js`; shell worker paths (`autopilot`/`run`/horizon checks) use the same shared runner command and expose per-run `--runner-bin`, `--runner-template`, and `--runner-model` flags for runner swaps/model changes. `atris2` runs each tick as one `/atris2/turn` on the AtrisOS backend via `runAtris2Turn()` (exported from `commands/probe.js`; default model `atris:fast`, stateless per tick — continuity lives on disk; no creds → mission pauses `auth-required`; receipt carries `result.atris2` with engine/tools_run/receipt_text). `codex_goal` publishes the goal for a live Codex session to pull. Regression: `test/mission-atris2-runner.test.js`, `test/mission-model-resolution.test.js`, `test/runner-command.test.js`, `test/autopilot-runner-model.test.js`
...
**Purpose:** Auto-chain plan → do → review cycles autonomously using the shared runner command (`ATRIS_CLAUDE_BIN`, `ATRIS_CLAUDE_COMMAND_TEMPLATE`, `ATRIS_CLAUDE_MODEL`; default Claude-compatible command)
