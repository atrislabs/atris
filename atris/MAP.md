...
rg "function doAtris" commands/workflow.js     # Do command (line 868): empty folder reuses buildFirstMinute / freshMinuteJson like bare atris; files already here start first-talk via startFirstTalk / visibleWorkTitle (ready + already claimed as the speaker, next is the claimed desk show, no next: atris do); PERSONA head stays first-minute after init; executor paste stays on --prompt/--verbose/--full; missing executor spec after init --minimal does not send you back to init
...
- Files already here, no `atris/`: bare `atris` still names the files and `next: atris do`. `atris do` starts from those files (`lib/context-gatherer.js` `startFirstTalk`, title from `visibleWorkTitle`): `{file} is ready` / already claimed as the speaker / `next: atris task show <id>` (same claimed desk next as bare `atris` after a manual claim). A second `do` and bare `atris` after that match that show next. It does not print `task claim` or `next: atris do`, and it does not dump a factory banner. Empty or hidden-only folders still name the talk command and do not mint.
...
- **Task desk next:** `lib/first-minute.js` (`deskNextCommand`) reuses `pickNext`/`taskCommand` for `commands/task.js` `renderTaskDesk` on `atris task desk` and `atris task --all`; claimed desk copy is `atris task show <id>`
- **Task next:** `commands/task.js` (`cmdNext`, `cmdNextTruth`) reuses `speakFirstMinute` in an uninitialized folder (same two spoken lines as bare `atris`, talk command, no `atris task new`, no `.atris/` or `atris/` mint); `bin/atris.js` `interactiveEntry` uses `isLeftoverVerbLook` so quoted leftover after `task next` / `brainstorm` / `wish` / `log` / `plan` / `do` is also a look, not first-talk; after a real room (`init --minimal` or the pasted talk line) it reuses `pickNext`/`taskCommand`/`taskNextCommand` so `atris task next` names the same accept/show/claim/new command as bare atris and the desk; a claimed row never prints `No open tasks`; `--create-next` still seeds Endgame fallback; `--help` stays usage. Regression: `test/task-next-scratch-nomint.test.js`
