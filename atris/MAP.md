...
- **atris youtube watch:** `commands/youtube.js:1186` `addWatchChannel` prints `watching` or `already watching`, then one `next: atris youtube watch tick` via `printWatchTickNext` (`commands/youtube.js:1182`); usage and bad input stay quiet. `commands/youtube.js:1250` `tickWatch` prints per-channel counts then `total: N new, M briefed`; when M>0 it prints one `next: atris youtube teach <first-briefed-url>` via `printYoutubeTeachNext` (`commands/youtube.js:1734`, quoted by `quoteYoutubeUrl` at `commands/youtube.js:2366`). First-briefed is the first video this tick ran through runner/briefFiler (`toBrief` order, channels in state order) using `https://www.youtube.com/watch?v=ID`. 0 briefed (empty list, fetch-fail only, no new videos) prints no next-step. Help: `commands/youtube.js:52` says add hands off to tick and tick hands off to teach when it briefed. Agent path: `atris/skills/youtube/SKILL.md` Feature map. Regression: `test/youtube-watch.test.js`.
