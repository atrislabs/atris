...
- **Radar surface:** `atris radar` / `atris radar --json` includes the current computed 0-shot route, owner-gate context, `atris 0-shot --all` menu command, horizon/model bucket counts, and durable prompt/menu/model/horizon freshness status; when the selected 0-shot route is owner-gated, the top `Next:` line leads with the read-only first command and stop-at-owner-gate boundary so the command-center view shows what any newly activated model should do next even if the latest file is stale
