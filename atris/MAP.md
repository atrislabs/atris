...
- **atris youtube teach:** `commands/youtube.js:1952` `parseTeachArgs` takes `--section N` (1-based, default 1) and `--save`; `commands/youtube.js:2189` `extractTeachSource` pulls yt-dlp chapters plus English captions with no network in tests; `commands/youtube.js:2105` `formatTeachLesson` prints numbers, named mechanisms, one check, and `next: atris youtube teach "<url>" --section N+1`; `commands/youtube.js:2225` `runYoutubeTeach` prints stdout only unless `--save`. Exit 0 even without Apply. No `/agent/process_youtube`, no `--paid`, no credits. Does not collide with `atris teach`. Regression: `test/youtube-teach.test.js`.
