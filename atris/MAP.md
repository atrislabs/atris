...
rg "doAtris|isWorkflowOwnerGatedZeroShot|printWorkflowOwnerGateStop" commands/workflow.js test/cli-smoke.test.js     # Do command + 0-shot owner-gate stop
...
- **Workflow activation:** `commands/workflow.js` `atris plan`, `atris do`, and legacy `atris review --verbose/--execute` stop on owner-gated 0-shot routes before Navigator/Executor/Validator prompts, auth, or agent execution; they print the current route, first read-only command, prompt/menu commands, owner-gate boundary, and `atris 0-shot --all`
