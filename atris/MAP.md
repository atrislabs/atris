...
- **Ambient latest files:** `atris 0-shot --write` and no-request `atris next --write` refresh `.atris/state/zero-shot.latest.json`, `.atris/state/zero-shot.prompt.txt`, and `.atris/state/zero-shot.<fast|pro|validator|human>.prompt.txt` without mutating tasks or calling external systems
- **Freshness check:** `atris 0-shot --check` and no-request `atris next --check` compare durable latest files, global prompt content, per-model prompt content, and current source fingerprints, then report fresh, stale, or missing
