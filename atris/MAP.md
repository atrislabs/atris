...
```bash
# Core CLI logic
rg "async function interactiveEntry|shouldGatherContext|renderContextGathererPrompt|buildFirstMinute|shouldAutoInitFresh|isTaskNextLook" bin/atris.js lib/context-gatherer.js lib/first-minute.js    # Main entry points: cold-start dispatcher, first-minute bare atris screen, first-contact context gatherer, leftover task next is a look not first-talk, legacy natural-language mode
...
- **First-minute screen:** `lib/first-minute.js` (`buildFirstMinute`, `freshMinuteJson`, `speakFirstMinute`) prints one win and one next command for bare `atris`, default `atris task` (`commands/task.js` `cmdFirstMinute`), `atris task next` (`commands/task.js` `cmdNext`), `atris task new` (`commands/task.js` `cmdAdd`), `atris plan`, `atris do`, and `atris review` (`commands/workflow.js` `planAtris` / `doAtris` / `reviewAtris`), bare `atris wish` (`commands/wish.js` `printWishReceptionist`), `atris log` (`commands/log.js` `logAtris`), and bare `atris ask` / `atris mission` (`commands/human-missions.js` `askCommand`, `commands/mission.js` `missionCommand` / `isLiveInFlightMission`); a running mission or a recent tick with a live driver shows one card; a stalled ready row with no live driver speaks the same claim/ready/accept next as bare `atris`; empty folders say the room is empty and name `atris "what should <folder> be?"`; pasting that talk line through bare atris runs `init --minimal` backstage, saves the first direction, and names `atris task claim <id>`; leftover command words (`task next`, `task new`, `wish`, `log`, `brainstorm`, `plan`, `do`, `review`, `ask`, `mission`) match that two-line screen and do not mint work, including quoted `task next` (`isTaskNextLook`); after `init --minimal` those commands still file or capture; `wish --help`, `ask --help`, `mission --help`, `task next --help`, `task new --help`, `review --help`, `log --help`, and `brainstorm --help` stay usage; `shouldAutoInitFresh` runs `init --minimal` on `--yes`/`-y` even under `ATRIS_NO_INTERACTIVE`; `--json` without a request names the talk command; claimed work names `atris task show <id>` so the pasted line works; any review names `atris task accept <id>` and says one finished thing is waiting (never shipped, never `review-chat`); throwaway names (`tmp`, `temp`, `atris-*`, hashes) stay "this folder"; a real basename like `launch-day` stays even under `/tmp`; person name prefers the saved account first name
...
- **Search:** `rg "shouldGatherContext|saveContextProfile|createStarterTask|buildFirstMinute|freshMinuteJson|speakFirstMinute|isFreshWorkspace|isTaskNextLook|printWishReceptionist|isWishDeskVerb|cmdFirstMinute|isUninitializedTaskFolder|cmdAdd|deskNextCommand|taskNextCommand|cmdNext|cmdNextTruth|reviewAtris|logAtris|brainstormAtris|isLiveInFlightMission|pickLiveLocalMission|Context gatherer" bin/atris.js lib/context-gatherer.js lib/first-minute.js commands/task.js commands/workflow.js commands/wish.js commands/human-missions.js commands/mission.js commands/log.js commands/brainstorm.js test
