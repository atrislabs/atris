...
- **File folder, just minted, no live mission:** `speakKeepWorkingMinute` when first-minute next is claimed keep-working (`"notes.md" is already yours` / `next: atris task ready <id> --verify "git diff --check"`). Not factory `let it run`. A live running mission (`pickLiveLocalMission`) still gets the board. `--verbose` keeps the factory dump.
...
- **Next:** one certified accept names `atris task accept <id>`; recap does not send humans to `atris task reviews` when that accept is the next move; keep-working recap names `atris task ready <id> --verify "git diff --check"`
