...
- **atris pack:** `commands/pack-craft.js:1` scaffolds a research pack with `atris pack craft "<topic>" [--dir <target>] [--force]`; `commands/pack.js` publishes and installs packs, records successful registry observations in `.atris/state/pack.json`, stages pull reviews in `.upstream/`, and reports installed, origin, remote-check, and staged-review truth; `lib/zip.js` provides zero-dependency zip reads and writes; `bin/atris.js` routes the command; `test/pack*.test.js` covers publishing, install/update/pull/status, craft/run/share, and packet safety.
