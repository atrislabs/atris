...
rg "isNonInteractive|isForcedNonInteractive|ATRIS_NONINTERACTIVE|refuseHeadlessUnless|isHelpToken|argsWantHelp|rejectUnsupportedJson|whoami --json|PROMPT ONLY|ACTION TAKEN|seedInitTaskPlane|importJournalFile|join --help|signup help|avail --help|helpOnly|write start help|showStartHelp|start --help|task accept --help|task claim --help|task ready --help|task step --help|task next --help" lib/noninteractive.js lib/workspace-scaffold.js commands/auth.js commands/log.js commands/now.js commands/skill.js commands/member.js commands/version.js commands/workflow.js commands/wiki.js commands/task.js commands/social.js commands/signup.js commands/avail.js commands/spaceship.js commands/dream.js commands/scout.js commands/interview.js commands/autopilot-front.js commands/write.js bin/atris.js test/dogfood-p0.test.js test/dogfood-p0-safety.test.js test/signup.test.js test/write.test.js test/dogfood-pass4.test.js test/task-accept-help.test.js test/task-claim-help.test.js test/task-ready-help.test.js test/task-step-help.test.js test/task-next-help.test.js  # Dogfood P0 + pass-4 safety: help on start/voice words is usage before NL; noninteractive guard; help-before-network; headless autopilot --json status; JSON contracts; task accept/claim/ready/step/next --help is usage only
rg "voiceTriggers|Unknown command|Did you mean|SINGLE_WORD_NATURAL_INTENTS|unknownCommandToken|isFlagLikeAnswer|_start" bin/atris.js lib/known-commands.js lib/context-gatherer.js test/dogfood-p0-safety.test.js test/dogfood-pass4.test.js  # One-word start/go/audit/deploy and leftover _start no longer spawn runners; unmatched first tokens (`taks list`) exit 2 even with extra args; start --help is usage; flag titles never become tasks
...
rg "printUsage|spaceship plan|planLines|hasYesFlag|spaceship --help|NO_COLOR" commands/spaceship.js scripts/spaceship.sh test/dogfood-pass2-t1-t5.test.js test/dogfood-p0-safety.test.js  # spaceship --help is short usage; bare invoke is a plan; --yes starts the run; no color off TTY
...
rg "youtubeCommand|processYoutube|runYoutubeNotes|parseNotesArgs|runYoutubeNotesBatch|fileBriefFromNotes|runYoutubeDigest|collectVideoBriefs|tickWatch|watch add|parseSearchArgs|parseSearchStdout|Usage: atris youtube|process_youtube|ytnotes|ytsearch" commands/youtube.js bin/atris.js test/youtube.test.js test/youtube-batch.test.js test/youtube-watch.test.js test/youtube-digest.test.js test/youtube-search.test.js test/billed-command-auth.test.js scripts/det/ytnotes scripts/det/ytsearch atris/skills/youtube/SKILL.md  # YouTube CLI: `commands/youtube.js:26` help lists search (free), notes, process, digest, and watch; `commands/youtube.js:1261` parseSearchArgs; `commands/youtube.js:1308` parseSearchStdout accepts 5- or 6-field pipe lines; `commands/youtube.js:1393` runYoutubeSearch prefers PATH ytsearch, then bundled `scripts/det/ytsearch`, then yt-dlp ytsearchN; `commands/youtube.js:1449` routes search before notes/digest/watch/process; process uses ensureBilledCommandAuth with scope youtube and retries after mint; `bin/atris.js` help names free search + notes; `test/youtube-search.test.js` mocks the runner (no network/credits); notes/digest/watch unchanged; process remains the 5-credit cloud knowledge path.
...
rg "cmdAdd|cmdRetitle|cmdImport|cmdClaim|cmdReady|cmdAccept|cmdSweep|cmdClearDone|cmdAudit|cmdReviews|cmdDone|cmdNext|cmdNextTruth|renderTaskDesk|deskNextCommand|taskNextCommand|pickNext|readEndgameAgentAction|liveTaskWithTitle|getTaskDb|task accept --help|task claim --help|task ready --help|task step --help|task next --help" commands/task.js lib/first-minute.js test/task-retitle.test.js test/task-sweep-auto-accept.test.js test/task-clear-done.test.js test/task-audit.test.js test/task-accept-help.test.js test/task-claim-help.test.js test/task-ready-help.test.js test/task-step-help.test.js test/task-next-help.test.js test/task-desk-next.test.js test/task-next-truth.test.js test/first-minute.test.js  # Local agent task plane, title history, proof-ready review, human accept, accept/claim/ready/step/next --help usage locks, desk and task next share first-minute pick/taskCommand, low-stakes auto-accept sweep, completed-task cleanup, accepted-proof audit, task-next Endgame seed dedupe, and compact certified review queue
...
rg "matchProtectedMissionDiff|inspectMissionProtectedDiff|prepareMissionGitGuard|protected-lane-review|localPathPrefix" lib/mission-protected-lane.js commands/mission.js commands/probe.js test/mission-protected-lane.test.js  # Mission protected-lane gate: staged path/content inspection, denied-tag reuse, fail-closed Git commit wrapper across local runners, paused mission state, and receipt reasons
...
- Typo-plus-args: `bin/atris.js:1175` `unknownCommandToken` refuses `taks list` / `misson status` without creating a task. Regression: `test/dogfood-p0-safety.test.js` (`30:`).
- Headless autopilot: `commands/autopilot-front.js:200` refuses bare headless invoke; `--auto`/`--yes`/`--once` still proceed. Regression: `test/dogfood-p0-safety.test.js` (`32:`).
...
- task next help: `commands/task.js:13258` prints one usage line (`atris task next [--tag <tag>] [--create-next]`) for `--help`/`-h`/`help`/`-?` and does not list, claim, or print the desk. `atris task --help` stays the encyclopedia. Regression: `test/task-next-help.test.js`.
...
- **How it works:**
- `atris task` opens the compact task desk and refreshes `.atris/state/tasks.projection.json`; the desk `next:` line reuses `lib/first-minute.js` `pickNext`/`taskCommand` so certified review names `atris task accept <id>`, claimed names `atris task ready <id>` with no angle-bracket templates, open names claim, and idle boards name `task next` or `task new`. Regression: `test/task-desk-next.test.js`, `test/first-minute.test.js`
...
- **Task desk next:** `lib/first-minute.js` (`deskNextCommand`) reuses `pickNext`/`taskCommand` for `commands/task.js` `renderTaskDesk`; claimed desk copy is `atris task ready <id>`
...
- **Regression:** `test/first-minute.test.js`, `test/task-desk-next.test.js`, `test/task-next-truth.test.js`, `test/context-gatherer.test.js`, `test/cli-smoke.test.js`
- **Search:** `rg "shouldGatherContext|saveContextProfile|createStarterTask|buildFirstMinute|deskNextCommand|taskNextCommand|cmdNextTruth|Context gatherer" bin/atris.js lib/context-gatherer.js lib/first-minute.js commands/task.js test`
