...
- **atris youtube teach:** `commands/youtube.js:2108` `parseTeachArgs` takes `--section N` (1-based, default 1), `--save`, `--recap TEXT`, `--skip`, `--owed`/`owed`, and `--json`, plus `recap`/`skip` subcommands; empty argv (no url, no `--section`) sets `owed`+`resume` instead of help; `commands/youtube.js:2264` `extractTeachNumbers` keeps a number only with its unit or nearby claim; `commands/youtube.js:2294` `extractTeachMechanisms` keeps a named head (window/model/cycle) and drops quotes, swears, and caption crumbs; `commands/youtube.js:2333` `teachLessonFromCues` feeds those lists to `formatTeachLesson` (`commands/youtube.js:2365`) and `isThinTeachLesson` (`commands/youtube.js:2369`); printed lesson still has one fail-able check and `next: atris youtube teach "<url>" --section N+1`; `commands/youtube.js:2450` `rememberTeachOwed` writes an ephemeral `.atris/youtube-teach-owed.json` sidecar (cwd, not a wiki brief) after a successful section; `commands/youtube.js:2488` `printTeachOwed` reads that store and prints url-or-id, owed section, and unpaid check (`nothing owed` when empty; `--json` is structured); resume adds one next line (`recap TEXT` / `skip`, or `atris youtube teach <url>` when empty); `commands/youtube.js:2509` `applyTeachRecap` unlocks when recap text contains that chapter's number-with-units or named mechanism, and `--skip` clears without claiming they answered; `commands/youtube.js:2468` `printTeachUnlockNext` prints one `next: atris youtube teach "<url>" --section N` after a successful unlock when a url is known (`--json` stays quiet); `commands/youtube.js:2882` `runYoutubeTeach` refuses `--section N` (N>1) with the unpaid check and exit 2 until recap/skip (`--json` stays quiet on that refuse). First section never owes a recap. Recap/skip write no brief, apply, journal, or experiment pack. `commands/youtube.js:2846` `extractTeachSource` pulls yt-dlp chapters plus English captions with no network in tests. Rich ephemeral prints one apply next-step via `lib/apply-gate.js:103` `hintEphemeralApply` (`commands/youtube.js:2943`) and writes no files; thin, no-caption, or failed teach prints none. `--save` of a thin chapter (no number-with-units and no named mechanism) prints `thin: no number or named mechanism. no brief.`, writes no brief/journal/apply sidecar, and exits 2 (`TEACH_THIN_REFUSE` at `commands/youtube.js:2082`). A rich chapter files the brief, mints `atris/experiments/teach-<videoid>-s<N>/`, then `commands/youtube.js:2827` `ensureTeachApply` writes one apply sidecar plus a journal Inbox claim that name that pack path and the keep rule (`keep only if measure.py moves 0→1`; scores 1 only when the fixture contains the check tokens). The sidecar omits the chapter check tokens so a fresh `measure.py` score stays 0. `commands/youtube.js:2388` `teachExperimentSlug` lowercases the video id; `commands/youtube.js:2546` `fileTeachExperiment` mints the pack with `measure.py` derived from the printed fail-able check, plus `program.md` under 1200 chars, `loop.py` keep-0-to-1, `reset.py`, and `results.tsv` (notes `--save` reuses this helper with `notesExperimentSlug`). Thin `--save` still writes no experiment. Teach without `--save` still writes no `atris/` tree. No `/agent/process_youtube`, no `--paid`, no credits. Does not collide with `atris teach`. Agent path: `atris/skills/youtube/SKILL.md` Feature map (bare teach resumes owed; next `--section` refuses until recap/skip). Regression: `test/youtube-teach.test.js`. Keep/revert pack: `atris/experiments/teach-thin-save/`.
