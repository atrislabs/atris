...
- **Exact digest window:** the digest header says `last 24 hours`, matching `acceptedInLastDay()`; accepted counts and expanded stories come from task truth instead of the compact eight-done-task projection, and a clear accepted result fills any missing landing story, so a busy day is neither undercounted nor missing promised detail. Regression: `test/autoland.test.js` (`digest and alarm compose in plain language`, `digest acceptance history is not capped...`)
