...
rg "printUsage|spaceship plan|planLines|hasYesFlag|wantsJson|pass --yes to start|spaceship --help|NO_COLOR" commands/spaceship.js scripts/spaceship.sh test/dogfood-pass2-t1-t5.test.js test/dogfood-p0-safety.test.js  # spaceship --help is short usage; bare invoke is a plan; --json without --yes is JSON refuse exit 2; --yes starts the run; no color off TTY
...
rg "addTask|claimTask|doneTask|appendTaskCompletionLogs|listTasks|workspaceRoot|findWorkspaceRoot|isGenericScratchRoot" lib/task-db.js lib/scratch-root.js lib/mission-root.js  # SQLite task store + workspace walk-up; scratch roots (/tmp, /private/tmp, /var/tmp) are never inherited by child folders
...
- Headless autopilot: `commands/autopilot-front.js:223` refuses bare headless invoke and `--json`/`--once` unless `--yes` or `--auto`; `autopilot stop` kills `child_pid` then the wrapper. Regression: `test/dogfood-p0-safety.test.js` (`32:`) and `test/dogfood-pass4.test.js` (`30:`).
...
- `atris slack [channels]` (`commands/integrations.js:1377`): Slack channel listing; unbound scratch folders refuse bare `atris slack` via `requireAccountBound` before this runs
...
- **Routing:** `bin/atris.js:2737` (`command === 'gmail'`) and `bin/atris.js:2776` (`command === 'slack'`): always `requireAccountBound`; gmail extracts mailbox `--account <id>` first and does not treat that selector as the workspace opt-in, so scratch `gmail send --account foo` refuses like bare send; a refused gmail gate returns before `gmailCommand` so connect/send cannot fall through; `--help` still prints usage
...
- **Gmail regressions:** `test/gmail-account.test.js:99` (connect validation), `test/gmail-account.test.js:129` (poll success), `test/gmail-account.test.js:182` (timeout), `test/gmail-account.test.js:298` (inbox mailbox header), `test/gmail-account.test.js:386` (send request), `test/gmail-account.test.js:436` (unowned send refusal), `test/gmail-account.test.js:471` (send usage), `test/gmail-account.test.js:501` (voice clear), `test/gmail-account.test.js:531` (piped voice), `test/gmail-account.test.js:564` (account roster), `test/gmail-account.test.js:610` (interactive display-name picker), `test/dogfood-pass3-27-28.test.js` (unbound scratch `gmail connect` and `gmail send --account foo` exit 2 with no network, lookup, or send)
...
- **Scratch-root room:** `lib/scratch-root.js` `isGenericScratchRoot` names `/tmp`, `/private/tmp`, `/var/tmp`, and `os.tmpdir()`. `lib/task-db.js` `findWorkspaceRoot` stops before walking into those parents, so an empty child under `/tmp` does not inherit `/tmp`'s tasks or missions. `cd src` in a real project still walks up. Regression: `test/scratch-root-room.test.js`, `test/task-workspace-root.test.js`
