...
- `commands/engine.js` (`engineCommand`, `resolveDefaultEngine`, `roster`, `runEngineDeviceLoginCommand`) - bring any intelligence: roster of coding CLIs from the saved registry policy (`.atris/state/engines.json`), workspace default in `.atris/engine.json` (bare `atris engine <name>` / `reset`), and quick asks through `atris engine <name> "<prompt...>"` or `atris engine ask "<prompt>" --engine <name>`; house default atris-fast; `engine resolve` decides purely from policy (health flags, fallback order, receipts) with zero machine probes, preserves the winning id on stdout, explains the router pick in one stderr sentence, and includes `won_reason` in JSON; `atris engine doctor` is the explicit opt-in probe pass (`lib/engine-registry.js` `engineDoctorReport`) that checks binaries and syncs ready/not_installed into the policy file; a missing binary fails loudly at the execution stage (`requireEngineBin`); computer device login accepts normalized `--seat <name>` values so several Codex or Claude accounts do not overwrite one default slot; profiles in `lib/runner-command.js` `RUNNER_PROFILE_DEFS` are atris-fast, claude, codex, cursor, fable, composer, haiku, devin, grok, and droid; `atris/skills/engines/SKILL.md` documents the same live roster and three verb commands; `--engine <name>` maps to `ATRIS_RUNNER_PROFILE` at the CLI boundary (`bin/atris.js` `applyRunnerFlags`) so mission run / autopilot / run / pulse ride any engine with zero loop changes; `test/engine.test.js`, `test/engine-ask.test.js`
- `lib/engine-ask.js:274-438,514-631`, `lib/engine-job-lifecycle.js:6-27` - parallel read-only engine questions atomically write a running receipt with pid/pgid before workers start, name an adjacent append-only `live_log`, write each stdout/stderr chunk before exit, finalize every terminal path without dropping the log pointer, and fail closed when output or exit metadata is missing; regressions: `test/engine-ask.test.js:325-371`, `test/engine-job-lifecycle.test.js:23-35`.
- `lib/engine-validate.js:117-367`, `commands/engine.js:1424-1426`, `bin/atris.js:474` - `atris engine validate <receipt|latest>` checks answered ask receipts through a different read-only referee, stores atomic verdict receipts plus append-only `atris/benchmarks/validations.jsonl`, and renders `validate scoreboard`; regressions: `test/engine-validate.test.js:54-278`.
