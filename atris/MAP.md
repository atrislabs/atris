...
- **atris pack:** `commands/pack.js:268` publishes an Atris brain zip with `pack.json` versioning, denylist filtering, optional `--out`, and optional `--push` to the web registry, `commands/pack.js:410` pulls newer registry zips into `.upstream/` without touching local edits, `commands/pack.js:449` reports local vs last-pulled remote status, `commands/pack.js:517` installs local zip/url/slug payloads with zip-slip protection, `commands/pack.js:674` lists installed pack folders under cwd and `./packs`, `lib/zip.js:65` writes standard deflated PKZIP archives, `lib/zip.js:158` reads them, `bin/atris.js:517` adds the help line, `bin/atris.js:2605` routes the modular command, and `test/pack.test.js:70` covers round-trip, denylist, zip-slip, version bump, network publish/pull/status, list, and registry/url install behavior.
