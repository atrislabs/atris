...
- **Heartbeat verifier reuse:** one auto-accept process caches a verifier result by exact command plus workspace and reuses it across eligibility and landing decisions; the cache dies with the heartbeat, and each task still receives its own accept/revise decision. A two-task counting regression in `test/autoland.test.js` proves one execution instead of four
