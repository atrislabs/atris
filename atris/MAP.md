...
- **Liveness surface:** `status` text output includes `missionHeartbeatLines()` — ordinary missions show `last tick`, while active full-budget task work calls the older event its `last mission receipt`; cadence missions get a due countdown and budget work gets remaining time. `buildTickPrompt()` injects member identity and dated logging. Regression: `test/mission-heartbeat.test.js`
