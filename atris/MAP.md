...
rg "youtubeCommand|processYoutube|runYoutubeNotes|fileBriefFromNotes|tickWatch|watch add|Usage: atris youtube|process_youtube|ytnotes" commands/youtube.js bin/atris.js test/youtube.test.js test/youtube-watch.test.js scripts/det/ytnotes atris/skills/youtube/SKILL.md  # YouTube CLI: `commands/youtube.js:25` help lists notes, process, and watch add/list/remove/tick; `commands/youtube.js:477` files a wiki brief after notes exit 0; `commands/youtube.js:662` watch add writes `{channel, added}` to `.atris/state/youtube_watch.json`; `commands/youtube.js:724` tick fetches three latest ids, seeds a fresh channel, and briefs unseen via ytnotes; `commands/youtube.js:819` notes wraps bundled `scripts/det/ytnotes`; `commands/youtube.js:845` routes notes and watch before process; `bin/atris.js:575` help lists notes vs process; `test/youtube.test.js:246` covers missing-url and non-youtube exit 2; `test/youtube.test.js:265` covers brief + journal filing; `test/youtube-watch.test.js:1` covers add/list/remove, unseen-only briefs, fresh-channel newest-only seed, and failed-fetch skip. Process remains the 5-credit cloud knowledge path.
