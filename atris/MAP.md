...
rg "reviewAtris" commands/workflow.js  # Review command: default certified queue, --verbose legacy validator prompt
...
- **Workflow activation:** `commands/workflow.js` `atris plan` and `atris do` stop on owner-gated 0-shot routes before the legacy Navigator/Executor workflow prompt or agent execution; they print the current route, first read-only command, prompt/menu commands, owner-gate boundary, and `atris 0-shot --all`
