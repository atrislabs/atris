...
rg "modelForMode|buildPayload|formatZeroShotMessage|buildAxZeroShotPacket|zeroShotModelTierForMode|async function chat|postTurn" ax test/cli-smoke.test.js  # ax Atris2 local coding-agent CLI: Fast/Pro/Code Fast modes, model-specific 0-shot handoff context, SSE streaming, workspace_path, chat history, doctor/help
...
rg "planAtris|isWorkflowOwnerGatedZeroShot|printWorkflowOwnerGateStop" commands/workflow.js test/cli-smoke.test.js  # Plan command + 0-shot owner-gate stop
rg "doAtris|executeAgentSDKFast|isWorkflowOwnerGatedZeroShot|printWorkflowOwnerGateStop" commands/workflow.js test/cli-smoke.test.js     # Do command + direct `atris x` 0-shot owner-gate stop
rg "reviewAtris|isWorkflowOwnerGatedZeroShot|printWorkflowOwnerGateStop" commands/workflow.js test/cli-smoke.test.js  # Review command: default certified queue, --verbose/--execute 0-shot owner-gate stop
...
rg "activateAtris|showActivateHelp|activate and next --help" bin/atris.js commands/activate.js test/commands.test.js test/zero-shot.test.js # Activate command + wiki status, 0-shot first-command handoff, and non-mutating help
rg "autopilotAtris" commands/autopilot.js   # Autopilot command
rg "runImprove|buildImproveZeroShotPacket|isImproveOwnerGatedZeroShot|summarizeImproveResponse|shouldFallbackLocal|summarizeTickHistory|appendTickToJournal" commands/improve.js bin/atris.js test/improve.test.js  # Improve command: paid RL tick (POST /api/improve, deducts credits), 0-shot owner-gate preflight, scorecard row + journal, `improve history` compounding view, --member attribution, local fallback
rg "brainCommand|collectState|countStateRows|certifiedReviewTasks|latestTaskEpisodes|latestScorecard|isActionableScorecardNextMove|operatorActivationNextMove|activationZeroShotFirstMove|refreshCompiledZeroShot|normalizeMemberSlug|memberProfileIssues|renderMissingMemberCard|renderPlaceholderMemberCard|recordTaskEpisodeScorecards|renderActivationCard|modeNextMove|renderActivationGallery|readMemberContext|rememberOperator|recordFeedback|recordApproval|brain --help" commands/brain.js test/commands.test.js  # Brain compile/activate/gallery/feedback/approval/scorecard, certified review checkpoint routing, compile fallback 0-shot first move, compile-time durable 0-shot refresh, unknown-operator 0-shot first move, and workspace-free help
rg "zeroShotCommand|buildPacket|writeLatestPacket|buildLatestCheck|collectFreshness|collectMissions|collectCodexGoal|todoLookupCommand|renderHint|activationZeroShotLine|0-shot next move|command === 'zero-shot'|command === '0-shot'|showWelcomeVisualization|memberActivate" commands/zero-shot.js bin/atris.js commands/activate.js commands/brain.js commands/member.js test/zero-shot.test.js test/commands.test.js  # 0-shot cold-start router: brain/task/mission/goal packet for vague prompts, ambient latest files, freshness checks, boot visualization, member activation first-command handoff, brain hints with model-first and horizon-first picks, TODO-line lookup first commands, and 0-shot command alias
...
rg "radarCommand|collectRadar|loadZeroShot|zeroShotNextAction|zeroShotBucketLine|zeroShotModelFirstLine|zeroShotHorizonFirstLine|zeroShotModelSelectorLine|zeroShotHorizonSelectorLine|Operator radar|Agent process top|findTaskWorkspaceRoot|task_reason|task_action|task_load|whats going on|loadBusinessCollaboration|ctop" commands/radar.js bin/atris.js test/radar.test.js test/commands.test.js  # Live Atris OS radar: agent processes joined with nearest workspace task projection, ctop unmapped-session reasons/actions and task-load pileups, mission/loop, worktree, AgentXP, team/member, business collaboration readiness, brain scorecard, Swarlo/delegation, Codex goal state, current 0-shot route owner-gate context/freshness/buckets, first-by-model/horizon picks, human/JSON selection command maps, and owner-gated top Next read-only first command
...
rg "cmdAdd|cmdImport|cmdClaim|cmdNext|cmdReady|cmdAccept|cmdReviews|cmdDone|getTaskDb" commands/task.js  # Local agent task plane, proof-ready review, task-next 0-shot owner gates, human accept, and compact certified review queue
...
- **Entry point:** `bin/atris.js` routes `0-shot` and `zero-shot`, prints a 0-shot hint on bare `atris`, and refreshes the ambient latest files; default `atris 0-shot` / `atris next` human output includes direct model-tier and horizon selector commands, and `atris next --model/--horizon` dispatches through the 0-shot selector instead of planner input
- **Natural aliases:** `bin/atris.js` normalizes `0shot`, `0 shot`, `zeroshot`, and `zero shot` to `0-shot` before dispatch
- **First-message boot:** `bin/atris.js` `interactiveEntry()` and `showWelcomeVisualization()` walk upward from subdirectories, point initialized workspaces at the current 0-shot route, reason, durable freshness, exact first command, owner-gate boundary, `atris 0-shot --all`, copy-paste prompt, direct model-tier prompts, direct horizon prompts, and refresh the ambient latest files
- **Implementation:** `commands/zero-shot.js`
- **Activation surface:** `commands/activate.js` walks upward from subdirectories, prints the current 0-shot route plus owner-gate context, route inventory, horizon/model buckets, first-by-model and first-by-horizon picks, durable prompt/menu/model/horizon freshness, all-route menu/global/direct model/direct horizon prompt commands, and refreshes the ambient latest files
- **Member activation:** `commands/member.js` walks upward from subdirectories, prints the current 0-shot route plus owner-gate context, route inventory, horizon/model buckets, first-by-model and first-by-horizon picks, durable prompt/menu/model/horizon freshness, all-route menu/global/direct model/direct horizon prompt commands, and refreshes the ambient latest files when waking a named member
- **Brain activation:** `commands/brain.js` includes the 0-shot route plus owner-gate context, route inventory, horizon/model buckets, first-by-model and first-by-horizon picks, durable prompt/menu/model/horizon freshness, all-route menu, model-tier prompt commands, and horizon prompt commands in `atris brain activate`, and refreshes the ambient latest files
- **Workflow activation:** `commands/workflow.js` `atris plan`, `atris do`, legacy `atris review --verbose/--execute`, and direct `atris x` stop on owner-gated 0-shot routes before Navigator/Executor/Validator prompts, auth, backend SDK calls, or agent execution; they print the current route, first read-only command, prompt/menu commands, owner-gate boundary, and `atris 0-shot --all`
- **Fast/pro model surface:** `ax` prepends compact 0-shot handoff context to Fast, Pro, and Code Fast turn payloads from Atris workspaces, requests the matching 0-shot model tier (`fast` for Fast/Code Fast, `pro` for Pro), preserves human owner-gates ahead of model filtering, and includes first command, owner-gate boundary, model/horizon buckets, and direct selector prompt commands without changing the backend payload schema
- **Agent adapters:** `commands/init.js`, `commands/sync.js`, root `AGENTS.md`, `.cursorrules`, `CLAUDE.md`, `GEMINI.md`, `atris/CLAUDE.md`, and `atris/GEMINI.md` point vague/empty prompts at `atris 0-shot --all`, copy-paste handoffs at `atris 0-shot --prompt`, model-tier prompts at `atris 0-shot --model fast|pro|validator|human --prompt`, and horizon prompts at `atris 0-shot --horizon now|review|long|blocked|orient --prompt`; they tell ambient agents to trust global, menu, tier, and horizon prompt files only after `atris 0-shot --check` reports fresh
- **Regression:** `test/zero-shot.test.js`
- **Reads:** `atris/brain/STATUS.md`, `.atris/state/tasks.projection.json`, `atris/TODO.md` including Backlog/In Progress/Review/Blocked sections, `.atris/state/missions.jsonl`, `.atris/state/codex_goal.json`
- **TODO fallback/options:** If `.atris/state/tasks.projection.json` is missing or empty, `commands/zero-shot.js` parses `atris/TODO.md`, emits `kind: todo`, preserves read-only boundaries, and points the first command at an exact `rg -n --fixed-strings -- <ref-or-title> atris/TODO.md` lookup; when projection rows exist, deduped active TODO rows are still added as supplemental read-only route options so fast/long/blocked selections remain visible while the primary route stays projection-owned
...
- **Freshness check:** `atris 0-shot --check` and no-request `atris next --check` compare durable latest files, latest JSON selection command maps, global prompt content, route menu content, per-model prompt content, per-horizon prompt content, and current source fingerprints, then report fresh, stale, or missing and exit 0 only when fresh
- **Radar surface:** `atris radar` / `atris radar --json` includes the current computed 0-shot route, owner-gate context, `atris 0-shot --all` menu command, horizon/model bucket counts, first-by-model and first-by-horizon picks, human/JSON model-horizon selection commands, and durable prompt/menu/model/horizon freshness status; when the selected 0-shot route is owner-gated, the top `Next:` line leads with the read-only first command and stop-at-owner-gate boundary so the command-center view shows what any newly activated model should do next even if the latest file is stale
- **Status surface:** `atris status`, `atris status --quick`, and `atris status --json` walk upward from subdirectories and include the current computed 0-shot route, first command, owner-gate context, `atris 0-shot --all` menu command, first-by-model and first-by-horizon picks, human/JSON model-horizon selection commands, and durable prompt/menu/model/horizon freshness so ordinary health checks show the next safe move plus whether persisted 0-shot context is safe to trust
- **Now front door:** generated `atris/now.md` includes the current 0-shot route, `atris 0-shot --all` menu command, durable menu/prompt file freshness, queue counts, route inventory counts with `routes.all_options`, bucket counts, first-by-horizon and first-by-model picks, first command, owner-gate context, and model/horizon handoff hints so agents loading the first context file can inspect all lanes before acting; `atris now --refresh` refreshes the durable 0-shot packet/menu/prompt files before rendering generated now.md
...
- **Route index:** JSON includes bounded `routes.options[]` and full `routes.all_options[]` with ref, lane, horizon, work size, model tier, first command, prompt, directive, and owner-gate context for active work; both lists are sanitized and omit raw task source payloads
- **Selection commands:** JSON `commands.model_prompt.fast|pro|validator|human` and `commands.horizon_prompt.now|review|long|blocked|orient` expose direct copy-paste prompt commands so model-aware agents do not have to parse prose to select a lane
- **Route summary:** JSON includes zero-filled `routes.horizons` counts, `routes.horizon_first` first picks from the full route inventory, plus `routes.models` grouped by fast/pro/validator/human with count and first route for each tier; human prompts/menus render both first-by-horizon and first-by-model summaries
...
- `atris 0-shot --model fast` -> fast-model next move when one exists, or a safe no-route handoff
...
- `atris 0-shot --check` -> verify whether durable latest files are fresh; exits non-zero when stale or missing
...
- **0-shot helper:** `zeroShotStatus(root)` calls `commands/zero-shot.js` `buildPacket()` read-only, exposes selected route plus owner-gate context, menu command, horizon/model bucket counts, first-by-model and first-by-horizon picks, and JSON model/horizon selection commands; it falls back to `atris 0-shot --prompt` if route collection is unavailable
- **Reads:**
...
- **Output:**
- Default: Chief-of-staff summary (`Where we are` / `What is queued` / `What is blocking`) with `0-shot: <lane> <ref> -> <first_command>`, `0-shot menu: atris 0-shot --all`, owner-gate context when present, plus quick/long/review/blocked counts, model-tier counts, first picks by model/horizon, and model/horizon selector commands
- `--verbose` / `-v`: Legacy visual task board with 0-shot footer
- `--quick` / `-q`: One-line emoji summary with 0-shot lane/ref, owner-gate context when present, bucket counts, and first picks by model/horizon
- `--json`: Structured JSON (date, backlog, inProgress, completed, inbox, completions, lessons, team, zero_shot with owner_action/safe_agent_action, horizon_counts/model_counts, horizon_first/model_first, and model/horizon selection commands)
