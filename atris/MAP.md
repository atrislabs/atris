...
rg "runImprove|summarizeImproveResponse|summarizeLocalMissionRun|shouldFallbackLocal|summarizeTickHistory|appendTickToJournal" commands/improve.js bin/atris.js test/improve.test.js  # Improve command: paid RL tick (POST /api/improve, deducts credits), scorecard row + journal, `improve history` compounding view, --member attribution; local fallback runs exactly one due mission tick and only writes receipts after its verifier passes
