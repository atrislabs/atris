...
rg "youtubeCommand|processYoutube|runYoutubeNotes|parseNotesArgs|runYoutubeNotesBatch|fileBriefFromNotes|runYoutubeDigest|collectVideoBriefs|tickWatch|watch add|parseSearchArgs|parseSearchStdout|runPaidYoutubeSearch|/youtube/search|--paid|writeLocalSearchCache|readFreshLocalSearchCache|LOCAL_SEARCH_CACHE|Usage: atris youtube|process_youtube|ytnotes|ytsearch" commands/youtube.js bin/atris.js test/youtube.test.js test/youtube-batch.test.js test/youtube-watch.test.js test/youtube-digest.test.js test/youtube-search.test.js test/billed-command-auth.test.js scripts/det/ytnotes scripts/det/ytsearch atris/skills/youtube/SKILL.md  # YouTube CLI: `commands/youtube.js:27` help lists search (free), search --paid (5-credit watch permalinks), notes, process, digest, and watch; `commands/youtube.js:1301` parseSearchArgs accepts --paid; `commands/youtube.js:1416` writeLocalSearchCache stores the last free local rows as {query, savedAt, rows} under ~/.atris/youtube-search-cache.json; `commands/youtube.js:1435` readFreshLocalSearchCache serves that file after the 429 retry if the normalized query matches and age is under one hour; `commands/youtube.js:1559` runPaidYoutubeSearch hard-refuses on a fresh same-query free-cache hit before any paid network call, otherwise posts /youtube/search via ensureBilledCommandAuth (scope youtube), mints/retries like x-search, never /auth/cli, and never writes the free cache; `commands/youtube.js:1658` free runYoutubeSearch still prefers PATH ytsearch, then bundled `scripts/det/ytsearch`, then yt-dlp ytsearchN, retries once on LOCAL_SEARCH_RATE_LIMIT_RE, then cache-reads or prints LOCAL_SEARCH_RATE_LIMIT_MESSAGE; `commands/youtube.js:1742` routes search before notes/digest/watch/process; `bin/atris.js` --all help names paid search; `test/youtube-search.test.js` mocks runner, fs, and HTTP (no live credits); process remains the 5-credit cloud knowledge path. Skill Feature map in `atris/skills/youtube/SKILL.md` is the ordered agent path: free search, 429 cache-or-stop, `--paid` only if the user asked to buy, notes, then process; never paste tokens or hit `/auth/cli`.
