...
rg "refuseUnboundCloudComputer|parseMissionId|cloud-computer|--mission|speakFirstMinute|isFreshWorkspace|fallbackOnMissing|pickLiveLocalMission|wantsBareMissionArchive" commands/human-missions.js commands/mission.js lib/first-minute.js test/human-missions.test.js test/first-minute.test.js test/dogfood-p0-safety.test.js  # Empty-folder bare ask/mission reuse speakFirstMinute like bare atris; after init, bare ask stays on first-minute; bare mission shows one live in-flight card or the same desk next as bare atris, never the complete/stopped archive (that stays on mission list / --all); unbound scratch folders still need --mission <id> for stop/ready
