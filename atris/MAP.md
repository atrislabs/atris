...
- **Task next:** `commands/task.js` (`cmdNext`, `cmdNextTruth`) reuses `buildFirstMinute({ fresh: true })` in an uninitialized folder (same two spoken lines as bare `atris`, talk command, no `atris task new`); after init it reuses `pickNext`/`taskCommand`/`taskNextCommand` so `atris task next` names the same accept/show/claim/new command as bare atris and the desk; a claimed row never prints `No open tasks`; `--create-next` still seeds Endgame fallback; `--help` stays usage
