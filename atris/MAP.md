...
- `lib/engine-ask.js:467-612`, `lib/engine-job-lifecycle.js:18-35` - parallel read-only engine questions atomically write a running receipt with pid/pgid before workers start, finalize it after normal/error/timeout/signal exits, and fail closed when output is empty or exit metadata is missing; regressions: `test/engine-ask.test.js`, `test/engine-job-lifecycle.test.js`.
