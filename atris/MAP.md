...
- **Runners & models:** `mission start --runner manual|claude|atris2|codex_goal [--model <id>]`. `claude` spawns the configured local Claude binary (`ATRIS_CLAUDE_BIN`, default `claude`) per tick via `spawnClaudeTick()`; model resolves from `mission.model` → `ATRIS_CLAUDE_MODEL` → `opus` in `lib/runner-command.js`. `atris2` runs each tick as one `/atris2/turn` on the AtrisOS backend via `runAtris2Turn()` (exported from `commands/probe.js`; default model `atris:fast`, stateless per tick — continuity lives on disk; no creds → mission pauses `auth-required`; receipt carries `result.atris2` with engine/tools_run/receipt_text). `codex_goal` publishes the goal for a live Codex session to pull. Regression: `test/mission-atris2-runner.test.js`, `test/mission-model-resolution.test.js`
...
**Purpose:** Auto-chain plan → do → review cycles autonomously using the shared Claude runner command (`ATRIS_CLAUDE_BIN`, default `claude`) with `claude -p` compatible args
