...
- Default: Chief-of-staff summary (`Where we are` / `What is queued` / `What is blocking`) when a live mission is running or first-minute next is not `atris do` or `atris task claim`
...
- `--json`: Structured JSON (date, backlog, inProgress, completed, inbox, completions, lessons, team); empty-folder `--json` is first-talk refuse; post-mint keep-working `--json` is one-lap (`already yours` / `atris do`); post-init claim `--json` stays the factory board
