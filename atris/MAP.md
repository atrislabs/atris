...
rg "refuseUnboundCloudComputer|parseMissionId|cloud-computer|--mission|speakFirstMinute|isFreshWorkspace|fallbackOnMissing" commands/human-missions.js commands/mission.js lib/first-minute.js test/human-missions.test.js test/first-minute.test.js test/dogfood-p0-safety.test.js  # Empty-folder bare ask/mission reuse speakFirstMinute like bare atris; after init, bare ask stays on first-minute and bare mission shows a live mission if one is running, else the same two spoken lines as bare atris; unbound scratch folders still need --mission <id> for stop/ready
rg "printUsage|spaceship plan|planLines|hasYesFlag|wantsJson|pass --yes to start|spaceship --help|NO_COLOR|isUnboundScratchFolder|refuseUnboundScratch" commands/spaceship.js commands/autopilot-front.js lib/scratch-root.js scripts/spaceship.sh test/dogfood-pass2-t1-t5.test.js test/dogfood-p0-safety.test.js test/dogfood-spaceship-scratch.test.js  # spaceship --help is short usage; bare invoke is a plan; --json without --yes is JSON refuse exit 2; --yes starts only in a room; unbound scratch --yes exits 2 with no email, overnight process, or atris/ mint; no color off TTY
...
rg "function doAtris" commands/workflow.js     # Do command (line 865): empty folder reuses buildFirstMinute / freshMinuteJson like bare atris; PERSONA head stays first-minute after init; executor paste stays on --prompt/--verbose/--full; missing executor spec after init --minimal does not send you back to init
...
rg "initAtris|upsertAtrisClaudeBootBlock|upsertClaudeVoiceHook|upsertCursorVoiceCard|upsertAgentVoiceCard|--minimal|--with-adapters|--with-skills" commands/init.js lib/claude-boot-block.js lib/voice-card.js test/claude-boot-block.test.js test/voice-card.test.js test/dogfood-papercuts.test.js  # Init command, voice-card doors, and --minimal/--with-adapters/--with-skills gates
...
rg "youtubeCommand|processYoutube|runYoutubeNotes|parseNotesArgs|runYoutubeNotesBatch|fileBriefFromNotes|runYoutubeDigest|collectVideoBriefs|tickWatch|watch add|parseSearchArgs|parseSearchStdout|runPaidYoutubeSearch|/youtube/search|--paid|writeLocalSearchCache|readFreshLocalSearchCache|LOCAL_SEARCH_CACHE|Usage: atris youtube|process_youtube|ytnotes|ytsearch|credits refunded|paidSearchCredits" commands/youtube.js bin/atris.js test/youtube.test.js test/youtube-batch.test.js test/youtube-watch.test.js test/youtube-digest.test.js test/youtube-search.test.js test/billed-command-auth.test.js scripts/det/ytnotes scripts/det/ytsearch atris/skills/youtube/SKILL.md  # YouTube CLI: `commands/youtube.js:27` help lists search (free), search --paid (5-credit watch permalinks), notes, process, digest, and watch; `commands/youtube.js:1328` parseSearchArgs accepts --paid; `commands/youtube.js:1443` writeLocalSearchCache stores the last free local rows as {query, savedAt, rows} under ~/.atris/youtube-search-cache.json; `commands/youtube.js:1462` readFreshLocalSearchCache serves that file after the 429 retry if the normalized query matches and age is under one hour; `commands/youtube.js:1521` paidSearchCredits reads credits_used 0 or credits_refunded and empty/502 paid search prints credits refunded without a client refund POST or Apply; `commands/youtube.js:1618` runPaidYoutubeSearch hard-refuses on a fresh same-query free-cache hit before any paid network call, otherwise posts /youtube/search via ensureBilledCommandAuth (scope youtube), mints/retries like x-search, never /auth/cli, and never writes the free cache; `commands/youtube.js:1717` free runYoutubeSearch still prefers PATH ytsearch, then bundled `scripts/det/ytsearch`, then yt-dlp ytsearchN, retries once on LOCAL_SEARCH_RATE_LIMIT_RE, then cache-reads or prints LOCAL_SEARCH_RATE_LIMIT_MESSAGE; `commands/youtube.js:1801` routes search before notes/digest/watch/process; `bin/atris.js` --all help names paid search; `test/youtube-search.test.js` mocks runner, fs, and HTTP (no live credits) and covers empty/502 refund surfacing; process remains the 5-credit cloud knowledge path. Skill Feature map in `atris/skills/youtube/SKILL.md` is the ordered agent path: free search, 429 cache-or-stop, `--paid` only if the user asked to buy, notes (done when the brief exists), write Apply before process; never paste tokens or hit `/auth/cli`.
rg "xSearchCommand|parseXSearchArgs|runXSearch|formatXSearchResult|formatEmptyXSearchResult|ensureXSearchApply|xSearchHasResults|xSearchCredits|/x-search/search|research-person|Usage: atris x-search|ensureApply|writeApplyStub|credits refunded" commands/x-search.js lib/apply-gate.js bin/atris.js lib/known-commands.js test/x-search.test.js test/billed-command-auth.test.js atris/skills/x-search/SKILL.md  # X search CLI: commands/x-search.js uses ensureBilledCommandAuth (scope x-search) then apiRequestJson; on a missing or expired user wall it mints via POST /auth/agent-token, stores the scoped token, and retries; no stored JWT fails with one sentence and no /auth/cli wall; search posts /x-search/search with query, limit, days_back; person posts /x-search/research-person; after a successful pull with results, commands/x-search.js ensureXSearchApply requires one Apply sidecar via lib/apply-gate.js (x-search-<slug>.apply.md with change + receipt) or writes a claimable fill-this stub and exits 2; empty or 502 search does not owe an Apply and surfaces server-side refunded credits (credits_used 0 or credits_refunded) without a client refund POST; help and citation print covered in test/x-search.test.js with mocked apiRequestJson; test/x-search.test.js covers incomplete without Apply, complete with a receipt, and empty/502 refund surfacing; bin/atris.js help and dispatch sit next to youtube; skill Feature map in atris/skills/x-search/SKILL.md is login, x-search QUERY, one Apply, stop.
...
rg "founderCommand|buildFounderScorecard|renderFounderScorecard|command === 'founder'" commands/founder.js bin/atris.js lib/known-commands.js test/founder.test.js  # Founder scorecard: `commands/founder.js:222` builds deterministic weekly commit and task totals, `commands/founder.js:278` writes and renders them, `bin/atris.js:1898` routes the command, `lib/known-commands.js:3` registers it, and `test/founder.test.js:66` proves history, slope, and receipts.
...
rg "requireAccountBound|account-global|isBoundBusinessWorkspace|--account|helpOnly|Pass --yes to create a live business|Pass --yes to publish" lib/account-bound.js bin/atris.js commands/business.js commands/terminal.js commands/fleet-report.js commands/site-deploy.js commands/chat-scan.js test/dogfood-pass2-t1-t5.test.js test/dogfood-pass4.test.js  # Unbound folders refuse business/rainmaker/avail/feedback/agent/terminal/fleet-report/site deploy unless --account; bare business, --help, help, and quickstart print usage without --account; business ship and site deploy also need --yes; terminal/fleet-report parse --help/--json before slug
...
rg "founderCommand|--global|singleRepo|defaultScanRoot" commands/founder.js test/dogfood-pass2-t1-t5.test.js  # founder defaults to this workspace; --global scans siblings
...
rg "scanState|lessonFlagsState|failedTaskBatchSource|failedTaskBatchWhat|dissolvedSources|recentlyClosedSources|openBySource" commands/close.js test/close.test.js  # Waiting-on-you scan: source-backed close loops, archived-task cleanup, latest-row review checks, 30-day close dedupe, durable dissolve suppression, and automatic legacy-row migration
...
rg "businessRoom|businessProducts|businessOrders|businessStore|normalizeOrdersPayload" commands/business.js bin/atris.js test/business-ops.test.js  # Live business room, product/order lists, and the web-compatible storefront switch over the production HTTP API
...
- **atris youtube search cache:** `commands/youtube.js:1416` writes the last successful free local search to `~/.atris/youtube-search-cache.json`; `commands/youtube.js:1435` reads it after the existing 429 retry when the normalized query matches and the row is younger than one hour; miss, expiry, or a corrupt file keep `LOCAL_SEARCH_RATE_LIMIT_MESSAGE` and a nonzero exit; paid search hard-refuses on a fresh same-query hit before any paid network call and does not write this file. Agent path: `atris/skills/youtube/SKILL.md` Feature map (429 is cache or stop, never `--paid`; `--paid` also hard-refuses on a fresh free-cache hit). Regression: `test/youtube-search.test.js`.
- **atris youtube notes:** `commands/youtube.js:1167` runs the bundled `scripts/det/ytnotes` rail with inherited stdio; `commands/youtube.js:536` files `atris/wiki/briefs/youtube-<id>.md` plus a claimable journal line after exit 0; `commands/youtube.js:583` `ensureNotesApply` then writes a fill-this Apply sidecar (`youtube-<id>.apply.md`) and a `next: write one apply...` line when no receipt exists, and still exits 0 because notes is done when the brief exists; `commands/youtube.js:27` lists notes next to process; `bin/atris.js:645` names the help line; `test/youtube.test.js:246` and `test/youtube.test.js:257` cover usage and non-youtube exit 2; `test/youtube.test.js:267` and `test/youtube.test.js:296` cover brief filing and the silent no-wiki path; `test/youtube.test.js:332` and `test/youtube.test.js:355` cover notes without Apply (open stub, next-line, exit 0) and notes + Apply receipt (complete, no next-line).
...
- `atris task next` tells the same truth as bare `atris` and the desk via `pickNext`/`taskCommand`: an uninitialized folder reuses `buildFirstMinute({ fresh: true })` (same two spoken lines, `next: atris init --minimal`); certified review names `atris task accept <id>` (no invented seed), claimed names `atris task ready <id>` with no templates or `step`, open names `claim` and does not auto-claim, and an initialized idle board names `atris task new`. `--create-next` still materializes the Endgame fallback when asked. `--help` stays usage. Regression: `test/task-next-truth.test.js`, `test/first-minute.test.js`, `test/task-first-minute.test.js`
- First-minute helpers: `lib/first-minute.js` `pickNext` / `taskCommand` / `deskNextCommand` / `taskNextCommand` / `freshMinuteJson` / `speakFirstMinute`; `commands/wish.js` `printWishReceptionist` reuses `speakFirstMinute` for bare wish and leftover sentences in an empty folder; `commands/human-missions.js` `askCommand` and `commands/mission.js` `missionCommand` reuse `speakFirstMinute` for empty-folder and post-init bare ask, and for empty-folder and post-init bare mission when no live mission is running (`currentMissionCommand` `fallbackOnMissing`); `commands/task.js` `cmdAdd` reuses `speakFirstMinute` when `isUninitializedTaskFolder` and `cmdNext` reuses `buildFirstMinute` when there is no `atris/` folder and no leftover projection; `commands/log.js` `logAtris` and `commands/brainstorm.js` `brainstormAtris` reuse `speakFirstMinute` in an empty folder and do not write a journal; desk points idle history at `task next`, and `task next` itself names `task new` when the room is already there and nothing is active
...
- **Entry point:** `commands/status.js:124-384` (statusAtris function)
- **Help:** `bin/atris.js:730-743` (`showStatusHelp`) and `bin/atris.js:1455-1460` short-circuit `status --help` before workspace or business-status reads
...
- **Help:** `bin/atris.js:464` help line; **Routing:** `bin/atris.js:1856-1857` dispatch, `lib/known-commands.js:3` `knownCommands`
...
- **Regression:** `test/site-deploy.test.js` covers validation, dry runs, `--yes` refuse, static uploads, Render setup, proxy fallback, and domain registration.
...
- **Autopilot front door:** `commands/autopilot-front.js` (`autopilotFront` main loop; `legPlan` selection — member legs force `--runner atris2`; `driveLeg` records `child_pid` and polls the stop file every 2s so `atris autopilot stop` kills the running leg, not only the wrapper; any leg under 30s counts toward the 5-in-a-row no-progress/failure halt). Bare headless / `--json` / `--once` at `commands/autopilot-front.js:223` exits 2 unless `--yes` or `--auto` is passed. Unbound scratch `--yes`/`--auto` still exit 2 via `isUnboundScratchFolder` before `writeState` (no mission, no `.atris/` mint). A folder that already has `atris/` can still start.
...
- **Commands:**
- `atris gmail inbox [--account <id>]` (`commands/integrations.js:201`, `gmailInbox`): lists messages and names the resolved mailbox when its email is known
- `atris gmail send <to> <subject> <body...> [--account <id>]` (`commands/integrations.js:360`, `gmailSend`): resolves the sticky/default mailbox, verifies it is connected, and sends the joined body or `--body-file` contents
- `atris gmail voice [account] [--clear]` (`commands/integrations.js:431`, `gmailVoice`): edits, pipes, or clears per-account `voice_md` through the Gmail account API
- `atris gmail connect [name]` (`commands/integrations.js:475`, `gmailConnect`): starts browser auth and polls for the named account; unbound scratch folders refuse `atris gmail connect` via the same `requireAccountBound` lock (exit 2, no Gmail network, no OAuth)
- `atris gmail accounts` (`commands/integrations.js:535`, `gmailAccounts`): prints the default-first, aligned account roster and marks the sticky account
- `atris gmail use [<account_id|name>]` (`commands/integrations.js:548`, `gmailUse`): picks an account interactively or resolves an id/display name and persists it as sticky
- Gmail dispatch and help live in `commands/integrations.js:595` (`gmailCommand`); unbound scratch folders refuse bare `atris gmail` via `requireAccountBound` before this runs
...
- **First-minute screen:** `lib/first-minute.js` (`buildFirstMinute`, `freshMinuteJson`, `speakFirstMinute`) prints one win and one next command for bare `atris`, default `atris task` (`commands/task.js` `cmdFirstMinute`), `atris task next` (`commands/task.js` `cmdNext`), `atris task new` (`commands/task.js` `cmdAdd`), `atris plan`, `atris do`, and `atris review` (`commands/workflow.js` `planAtris` / `doAtris` / `reviewAtris`), bare `atris wish` (`commands/wish.js` `printWishReceptionist`), `atris log` (`commands/log.js` `logAtris`), and bare `atris ask` / `atris mission` (`commands/human-missions.js` `askCommand`, `commands/mission.js` `missionCommand`); empty folders, including `atris task next`, `atris task new "count the words"`, `atris plan`, `atris plan --json`, `atris do`, `atris do --json`, `atris review`, `atris review --json`, `atris ask`, `atris ask --json`, `atris mission`, `atris mission --json`, `atris log`, `atris log "friction"`, `atris log --json`, `atris brainstorm`, `atris brainstorm "count words"`, and `atris brainstorm --json`, name `atris init --minimal` (plan/do/review/ask/mission/log JSON adds `--yes`); after init, default `plan`/`do` match that two-line screen with no `PROMPT ONLY` banner (`--prompt` keeps the factory paste); leftover words after `plan` stay a request after init; empty-folder `atris task new "count the words"` matches that two-line screen and does not mint `.atris/state/tasks.projection.json` or a task id; after `init --minimal` `task new` still files; bare `atris wish` and leftover `atris wish "count the words"` in an empty folder match that two-line screen, do not mint `.atris/state/wishes.jsonl`, and do not dump grant/list/stats usage; after `init --minimal` a quoted wish still files; empty-folder `atris log` and leftover `atris log "friction"` match that two-line screen, do not create `atris/logs`, and do not say folder not found; after `init --minimal` a note still captures; empty-folder `atris brainstorm` and leftover `atris brainstorm "count words"` match that two-line screen, do not mint `atris/` or `.atris/`, and do not write an inbox row; after `init --minimal` a named idea still captures; empty-folder `ask` and `mission` do not mention `business.json`, `--mission`, or `atris ask`; after `init --minimal`, bare wish, ask, and `task next` stay on first-minute workspace truth (claim/ready/accept) and do not send you back to init; bare `review` after init matches first-minute when next is claim/ready/accept, keeps certified accept, and says `nothing is waiting on you.` only when that desk is empty; it does not bounce to init; bare mission after init shows a live mission if one is running, else the same two spoken lines as bare atris (desk next), and does not bounce to init or `atris ask`; `wish --help`, `ask --help`, `mission --help`, `task next --help`, `task new --help`, `review --help`, `log --help`, and `brainstorm --help` stay usage; `shouldAutoInitFresh` runs `init --minimal` on `--yes`/`-y` even under `ATRIS_NO_INTERACTIVE`, then the post-init screen names the seeded claim; `--json` and headless without `--yes` stay print-only; claimed work names `atris task ready <id>`; certified review names `atris task accept <id>`; throwaway names (`tmp`, `temp`, `atris-*`, hashes) stay "this folder"; a real basename like `launch-day` stays even under `/tmp`; person name prefers the saved account first name
- **Scratch-root room:** `lib/scratch-root.js` `isGenericScratchRoot` names `/tmp`, `/private/tmp`, `/var/tmp`, and `os.tmpdir()`. `isUnderGenericScratchRoot` / `isUnboundScratchFolder` treat an empty child under those roots as not a room (`--yes` is not an unlock). `commands/spaceship.js` and `commands/autopilot-front.js` refuse that start with `this folder is not a room` and exit 2 (no email, no overnight process, no `.atris/` or `atris/` mint). A child that already has `atris/` or `.atris/business.json` can still start. `lib/task-db.js` `findWorkspaceRoot` stops before walking into those parents, so an empty child under `/tmp` does not inherit `/tmp`'s tasks or missions. `cd src` in a real project still walks up. Regression: `test/scratch-root-room.test.js`, `test/task-workspace-root.test.js`, `test/dogfood-spaceship-scratch.test.js`
...
- **Task next:** `commands/task.js` (`cmdNext`, `cmdNextTruth`) reuses `buildFirstMinute({ fresh: true })` in an uninitialized folder (same two spoken lines as bare `atris`, `next: atris init --minimal`, no `atris task new`); after init it reuses `pickNext`/`taskCommand`/`taskNextCommand` so `atris task next` names the same accept/ready/claim/new command as bare atris and the desk; `--create-next` still seeds Endgame fallback; `--help` stays usage
- **Task new:** `commands/task.js` (`cmdAdd`) reuses `speakFirstMinute` in an uninitialized folder (same two spoken lines as bare `atris`, `next: atris init --minimal`, no task id, no `.atris/state/tasks.projection.json`); after `init --minimal` a title still files; `--help` stays usage
- **Log:** `commands/log.js` (`logAtris`) reuses `speakFirstMinute` in an uninitialized folder (same two spoken lines as bare `atris`, `next: atris init --minimal`, no `atris/logs`); after `init --minimal` a note still captures as inbox; `--json` is first-minute refuse before init and `inbox_capture` after; `--help` stays usage
- **Brainstorm:** `commands/brainstorm.js` (`brainstormAtris`) reuses `speakFirstMinute` in an uninitialized folder (same two spoken lines as bare `atris`, `next: atris init --minimal`, no inbox row); after `init --minimal` a named idea still captures and exits; `--json` is first-minute refuse before init and a capture receipt after; `--help` stays usage
- **Helper:** `lib/context-gatherer.js`
- **State:** `.atris/state/context_profile.json`
- **Regression:** `test/first-minute.test.js`, `test/brainstorm-headless.test.js`, `test/task-first-minute.test.js`, `test/task-desk-next.test.js`, `test/task-next-truth.test.js`, `test/context-gatherer.test.js`, `test/cli-smoke.test.js`, `test/workflow-command.test.js`, `test/wish.test.js`, `test/human-missions.test.js`, `test/log-inbox.test.js`, `test/scratch-root-room.test.js`, `test/task-workspace-root.test.js`
- **Search:** `rg "shouldGatherContext|saveContextProfile|createStarterTask|buildFirstMinute|freshMinuteJson|speakFirstMinute|isFreshWorkspace|printWishReceptionist|isWishDeskVerb|cmdFirstMinute|isUninitializedTaskFolder|cmdAdd|deskNextCommand|taskNextCommand|cmdNext|cmdNextTruth|reviewAtris|logAtris|brainstormAtris|Context gatherer" bin/atris.js lib/context-gatherer.js lib/first-minute.js commands/task.js commands/workflow.js commands/wish.js commands/human-missions.js commands/mission.js commands/log.js commands/brainstorm.js test
...
- `commands/engine.js:1419-1485` (`TASK_ID_TOKEN`, `shorthandModelError`, `engineCommand`) - bring any intelligence through one front door: `atris engine <name> "<question>"` routes to guarded ask with optional `--model`, `atris engine <name> <task-id>` reuses build dispatch, and bare `atris engine <name>` switches the workspace default; ambiguous task-plus-prose stops before either lane, while unsupported model selection names the registry's known-good examples. The roster comes from `.atris/state/engines.json`, the default lives in `.atris/engine.json`, and the house default is atris-fast; `engine resolve` decides purely from policy (health flags, fallback order, receipts) with zero machine probes, preserves the winning id on stdout, explains the router pick in one stderr sentence, and includes `won_reason` in JSON; `atris engine doctor` is the explicit opt-in probe pass (`lib/engine-registry.js` `engineDoctorReport`) that checks binaries and syncs ready/not_installed into the policy file; a missing binary fails loudly at the execution stage (`requireEngineBin`); computer device login accepts normalized `--seat <name>` values so several Codex or Claude accounts do not overwrite one default slot; profiles in `lib/runner-command.js` `RUNNER_PROFILE_DEFS` are atris-fast, claude, codex, cursor, fable, composer, haiku, devin, grok, and droid; `atris/skills/engines/SKILL.md:24-32` leads with the same three verbs and marks raw binaries fallback-only; regressions: `test/engine-ask.test.js:450-513`, `test/engine-dispatch.test.js:99-148`.
- `lib/engine-ask.js:274-438,514-631`, `lib/engine-job-lifecycle.js:6-27`, `lib/engine-registry.js` (`engineFailureHealthStatus`) - parallel read-only engine questions atomically write a running receipt with pid/pgid before workers start, name an adjacent append-only `live_log`, write each stdout/stderr chunk before exit, finalize every terminal path without dropping the log pointer, fail closed when output or exit metadata is missing, and persist each successful or failed engine run so auth/subscription lapses bench the engine until a later success restores it; regressions: `test/engine-ask.test.js:325-371`, `test/engine-job-lifecycle.test.js:23-35`.
