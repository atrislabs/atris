...
rg "refuseUnboundCloudComputer|parseMissionId|cloud-computer|--mission|speakFirstMinute|isFreshWorkspace|fallbackOnMissing|pickLiveLocalMission|isLiveInFlightMission|wantsBareMissionArchive|speakMissionDoor|wantsSpokenMissionStatus|helpFull" commands/human-missions.js commands/mission.js lib/first-minute.js test/human-missions.test.js test/first-minute.test.js test/dogfood-p0-safety.test.js test/dogfood-mission-lingo.test.js  # Empty-folder bare ask/mission/status/list reuse speakFirstMinute like bare atris; after init, bare ask stays on first-minute; bare mission and default mission status (no id, no --all) show one live in-flight card or the same desk next as bare atris; stalled / ready-with-no-driver / complete / stopped rows stay in the archive on mission list / status --all; mission --help is a short keep-working page (full dump on help --full); leftover mission hi does not mint; unbound scratch folders still need --mission <id> for stop/ready
