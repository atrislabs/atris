...
- **atris youtube notes:** `commands/youtube.js:1297` runs the bundled `scripts/det/ytnotes` rail with inherited stdio; default `atris youtube notes URL` is stdout only (`commands/youtube.js:1447`); `--save` of thin notes (no number-with-units and no named mechanism, same bar as teach via `extractTeachNumbers` / `extractTeachMechanisms`) prints `thin: no number or named mechanism. no brief.`, writes no brief/journal/apply/experiment, and exits 2 (`TEACH_THIN_REFUSE` at `commands/youtube.js:2020`). A rich `--save` files `atris/wiki/briefs/youtube-<id>.md` plus a claimable journal line (`commands/youtube.js:599`), mints `atris/experiments/notes-<videoid>/` through `fileTeachExperiment` (`commands/youtube.js:676` `saveRichNotes`, slug at `commands/youtube.js:648`), then `commands/youtube.js:694` `ensureNotesApply` writes one apply sidecar plus a journal Inbox claim that name that pack path and the keep rule (`keep only if measure.py moves 0→1`). The sidecar omits the notes check tokens so a fresh `measure.py` score stays 0. `commands/youtube.js:717` `unsaveYoutubeNotes` deletes the brief and apply sidecar if present and stays quiet when they are already gone; `commands/youtube.js:766` `ensureProcessApply` (`required: true`) refuses missing or stub-only Apply with one sentence and exit 2 before any `/agent/process_youtube` call; `commands/youtube.js:28` lists notes as ephemeral unless `--save`; `commands/youtube.js:56` says rich notes/teach mint a keep/revert experiment. Regression: `test/youtube.test.js` and `test/youtube-notes-save.test.js`.
- **atris youtube teach:** `commands/youtube.js:2042` `parseTeachArgs` takes `--section N` (1-based, default 1) and `--save`; `commands/youtube.js:2152` `extractTeachNumbers` keeps a number only with its unit or nearby claim; `commands/youtube.js:2182` `extractTeachMechanisms` keeps a named head (window/model/cycle) and drops quotes, swears, and caption crumbs; `commands/youtube.js:2221` `teachLessonFromCues` feeds those lists to `formatTeachLesson` (`commands/youtube.js:2253`) and `isThinTeachLesson` (`commands/youtube.js:2257`); printed lesson still has one fail-able check and `next: atris youtube teach "<url>" --section N+1`; `commands/youtube.js:2594` `extractTeachSource` pulls yt-dlp chapters plus English captions with no network in tests; `commands/youtube.js:2630` `runYoutubeTeach` prints stdout only unless `--save`. `--save` of a thin chapter (no number-with-units and no named mechanism) prints `thin: no number or named mechanism. no brief.`, writes no brief/journal/apply sidecar, and exits 2 (`TEACH_THIN_REFUSE` at `commands/youtube.js:2020`). A rich chapter files the brief, mints `atris/experiments/teach-<videoid>-s<N>/`, then `commands/youtube.js:2575` `ensureTeachApply` writes one apply sidecar plus a journal Inbox claim that name that pack path and the keep rule (`keep only if measure.py moves 0→1`; scores 1 only when the fixture contains the check tokens). The sidecar omits the chapter check tokens so a fresh `measure.py` score stays 0. `commands/youtube.js:2276` `teachExperimentSlug` lowercases the video id; `commands/youtube.js:2296` `fileTeachExperiment` mints the pack with `measure.py` derived from the printed fail-able check, plus `program.md` under 1200 chars, `loop.py` keep-0-to-1, `reset.py`, and `results.tsv` (notes `--save` reuses this helper with `notesExperimentSlug`). Thin `--save` still writes no experiment. Teach without `--save` still writes nothing. No `/agent/process_youtube`, no `--paid`, no credits. Does not collide with `atris teach`. Regression: `test/youtube-teach.test.js`. Keep/revert pack: `atris/experiments/teach-thin-save/`.
...
- `atris/experiments/x-search-<slug>/` - minted by rich `atris x-search --save` through the same `fileTeachExperiment` helper (`commands/youtube.js:2296`, called from `commands/x-search.js:424` `saveRichXSearch`, slug at `commands/x-search.js:358`); same keep-0-to-1 rule, apply sidecar `x-search-<slug>.apply.md`, thin `--save` writes no pack. Default x-search without `--save` writes no pack (`commands/x-search.js:544`).
...
**Search:** `rg "experimentsCommand|experimentsKeep|experimentsRevert|runMeasureJson|runResetScript|experimentsRun|experimentsCompare|experimentsReplay|buildBenchmarkArtifact|ensureExperimentsFramework|experimentsDaily|experimentsQueue|sessionstart-plant|teach-thin-save|fileTeachExperiment|ensureTeachApply|teachExperimentSlug|notesExperimentSlug|saveRichNotes|xSearchExperimentSlug|saveRichXSearch" commands/experiments.js commands/init.js lib/experiments/daily.js commands/autoland.js commands/youtube.js commands/x-search.js atris/experiments/sessionstart-plant atris/experiments/teach-thin-save`
