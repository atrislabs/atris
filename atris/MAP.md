...
```bash
# Core CLI logic
rg "async function interactiveEntry|shouldGatherContext|renderContextGathererPrompt|buildFirstMinute|listUserVisibleWork|visibleWorkTitle|headCommitSubject|startFirstTalk|freshWinLine|freshNextCommand|shouldAutoInitFresh|isTaskNextLook|isLeftoverVerbLook|isFirstTalkLine" bin/atris.js lib/context-gatherer.js lib/first-minute.js    # Main entry points: cold-start dispatcher, first-minute bare atris screen, first-contact context gatherer, leftover task next and leftover words after brainstorm/wish/log/plan/do are a look not first-talk, pasted talk line is first-talk not overview, a folder with user-visible files is named not empty, a git repo with a HEAD commit names that subject instead of the file listing, atris do starts from those files, legacy natural-language mode
...
rg "youtubeCommand|processYoutube|runYoutubeNotes|parseNotesArgs|runYoutubeNotesBatch|fileBriefFromNotes|unsaveYoutubeNotes|runYoutubeDigest|collectVideoBriefs|tickWatch|watch add|parseSearchArgs|parseSearchStdout|runPaidYoutubeSearch|/youtube/search|--paid|--save|--unsave|writeLocalSearchCache|readFreshLocalSearchCache|LOCAL_SEARCH_CACHE|Usage: atris youtube|process_youtube|ytnotes|ytsearch|credits refunded|paidSearchCredits" commands/youtube.js bin/atris.js test/youtube.test.js test/youtube-batch.test.js test/youtube-watch.test.js test/youtube-digest.test.js test/youtube-search.test.js test/billed-command-auth.test.js scripts/det/ytnotes scripts/det/ytsearch atris/skills/youtube/SKILL.md  # YouTube CLI: `commands/youtube.js:28` help lists search (free), search --paid (5-credit watch permalinks), notes (stdout only unless --save), unsave, process, digest, and watch; `commands/youtube.js:532` parseNotesArgs accepts --save and --unsave; `commands/youtube.js:1365` default notes prints only; `commands/youtube.js:570` files brief+journal and `commands/youtube.js:619` writes apply stub only on --save; `commands/youtube.js:636` unsave deletes youtube-<id>.md and youtube-<id>.apply.md with no paid calls; process remains the 5-credit cloud knowledge path and still needs a filled Apply. Skill Feature map in `atris/skills/youtube/SKILL.md` is the ordered agent path: free search, 429 cache-or-stop, `--paid` only if the user asked to buy, notes ephemeral unless `--save`, write Apply before process; never paste tokens or hit `/auth/cli`.
...
- **atris youtube notes:** `commands/youtube.js:1216` runs the bundled `scripts/det/ytnotes` rail with inherited stdio; default `atris youtube notes URL` is stdout only (`commands/youtube.js:1365`); `commands/youtube.js:570` files `atris/wiki/briefs/youtube-<id>.md` plus a claimable journal line only on `--save`; `commands/youtube.js:619` `ensureNotesApply` then writes a fill-this Apply sidecar (`youtube-<id>.apply.md`) and a `next: write one apply...` line when no receipt exists, and still exits 0 on the `--save` path; `commands/youtube.js:636` `unsaveYoutubeNotes` deletes the brief and apply sidecar if present and stays quiet when they are already gone; `commands/youtube.js:685` `ensureProcessApply` (`required: true`) refuses missing or stub-only Apply with one sentence and exit 2 before any `/agent/process_youtube` call; `commands/youtube.js:28` lists notes as ephemeral unless `--save`; `bin/atris.js:651` names the help line; `test/youtube.test.js:356` covers notes without `--save` writing nothing; `test/youtube.test.js:376` and `test/youtube.test.js:400` cover `--save` without Apply (open stub, next-line, exit 0) and `--save` + Apply receipt (complete, no next-line); `test/youtube.test.js:449` covers unsave.
...
- **Empty folder:** default spoken `atris recap` and `atris recap --json` reuse `speakFirstMinute` / `freshMinuteJson` only when there is no task history and no `atris/` room (same two spoken lines and next as bare `atris`, including a named file or last commit already here, where next is `atris do`); a folder with task history keeps recap; `--verbose` / `--share` keep the empty report; `--help` stays usage
