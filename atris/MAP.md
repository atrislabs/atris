...
```bash
# Core CLI logic
rg "async function interactiveEntry|shouldGatherContext|renderContextGathererPrompt|buildFirstMinute|shouldAutoInitFresh|isTaskNextLook|isFirstTalkLine" bin/atris.js lib/context-gatherer.js lib/first-minute.js    # Main entry points: cold-start dispatcher, first-minute bare atris screen, first-contact context gatherer, leftover task next is a look not first-talk, pasted talk line is first-talk not overview, legacy natural-language mode
