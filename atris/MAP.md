...
```bash
# Core CLI logic
rg "async function interactiveEntry|shouldGatherContext|renderContextGathererPrompt|buildFirstMinute|shouldAutoInitFresh|isTaskNextLook|isLeftoverVerbLook|isFirstTalkLine" bin/atris.js lib/context-gatherer.js lib/first-minute.js    # Main entry points: cold-start dispatcher, first-minute bare atris screen, first-contact context gatherer, leftover task next and leftover words after brainstorm/wish/log/plan/do are a look not first-talk, pasted talk line is first-talk not overview, legacy natural-language mode
...
- **Empty folder:** default spoken `atris recap` and `atris recap --json` reuse `speakFirstMinute` / `freshMinuteJson` (same two spoken lines and talk next as bare `atris`); `--verbose` / `--share` keep the empty report; `--help` stays usage
