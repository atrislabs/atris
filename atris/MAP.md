...
rg "askCommand|currentMissionCommand|approveCommand|stopCommand|speakNothingRunning|answerCommand|readyCommand|checkCommand|missionCard|speakFirstMinute|fallbackOnMissing|pickLiveLocalMission|isLiveInFlightMission|speakMissionDoor" commands/human-missions.js commands/mission.js commands/status.js lib/cloud-mission.js lib/first-minute.js bin/atris.js test/human-missions.test.js test/first-minute.test.js test/dogfood-mission-lingo.test.js test/dogfood-p0-safety.test.js  # Public human mission commands: empty-folder bare ask/mission/status/list reuse first-minute; empty-folder bare stop and empty-folder atris status talk first-talk (`nothing is running`, same talk next, no mint, no init); a file already here names that file and next is `atris do`, same as bare atris, and does not mint; after init they stay in the room; bare mission and default status show one live in-flight card or the desk next; stalled ready with no live driver yields the desk next, not the archive; ask, current card, answer, approve, stop, readiness, and check results over the cloud mission client
...
rg "function doAtris" commands/workflow.js     # Do command (line 868): empty folder reuses buildFirstMinute / freshMinuteJson like bare atris; files already here start first-talk via startFirstTalk / visibleWorkTitle (ready + already claimed as the speaker, next is atris do); a second do stays two first-minute lines (already yours / next: atris do); PERSONA head stays first-minute after init; executor paste stays on --prompt/--verbose/--full; missing executor spec after init --minimal does not send you back to init
...
rg "youtubeCommand|processYoutube|runYoutubeNotes|parseNotesArgs|runYoutubeNotesBatch|fileBriefFromNotes|unsaveYoutubeNotes|runYoutubeTeach|parseTeachArgs|extractTeachSource|formatTeachLesson|runYoutubeDigest|collectVideoBriefs|tickWatch|watch add|parseSearchArgs|parseSearchStdout|runPaidYoutubeSearch|/youtube/search|--paid|--save|--unsave|--section|writeLocalSearchCache|readFreshLocalSearchCache|LOCAL_SEARCH_CACHE|Usage: atris youtube|process_youtube|ytnotes|ytsearch|credits refunded|paidSearchCredits" commands/youtube.js bin/atris.js test/youtube.test.js test/youtube-teach.test.js test/youtube-batch.test.js test/youtube-watch.test.js test/youtube-digest.test.js test/youtube-search.test.js test/billed-command-auth.test.js scripts/det/ytnotes scripts/det/ytsearch atris/skills/youtube/SKILL.md  # YouTube CLI: `commands/youtube.js:28` help lists search (free), search --paid (5-credit watch permalinks), notes (stdout only unless --save), teach (one chapter, stdout only unless --save), unsave, process, digest, and watch; `commands/youtube.js:561` parseNotesArgs accepts --save and --unsave; `commands/youtube.js:1394` default notes prints only; `commands/youtube.js:599` files brief+journal and `commands/youtube.js:648` writes apply stub only on --save; `commands/youtube.js:665` unsave deletes youtube-<id>.md and youtube-<id>.apply.md with no paid calls; process remains the 5-credit cloud knowledge path and still needs a filled Apply. Skill Feature map in `atris/skills/youtube/SKILL.md` is the ordered agent path: free search, 429 cache-or-stop, `--paid` only if the user asked to buy, notes ephemeral unless `--save`, write Apply before process; never paste tokens or hit `/auth/cli`.
rg "xSearchCommand|parseXSearchArgs|runXSearch|formatXSearchResult|formatEmptyXSearchResult|ensureXSearchApply|xSearchHasResults|xSearchCredits|/x-search/search|research-person|Usage: atris x-search|ensureApply|writeApplyStub|credits refunded" commands/x-search.js lib/apply-gate.js bin/atris.js lib/known-commands.js test/x-search.test.js test/billed-command-auth.test.js atris/skills/x-search/SKILL.md  # X search CLI: commands/x-search.js uses ensureBilledCommandAuth (scope x-search) then apiRequestJson; on a missing or expired user wall it mints via POST /auth/agent-token, stores the scoped token, and retries; no stored JWT fails with one sentence and no /auth/cli wall; search posts /x-search/search with query, limit, days_back; person posts /x-search/research-person; after a successful pull with results, commands/x-search.js ensureXSearchApply writes a fill-this Apply sidecar via lib/apply-gate.js (x-search-<slug>.apply.md with change + receipt) and a next-line when no receipt exists, and still exits 0; empty or 502 search does not owe an Apply and surfaces server-side refunded credits (credits_used 0 or credits_refunded) without a client refund POST; help and citation print covered in test/x-search.test.js with mocked apiRequestJson; test/x-search.test.js covers search without Apply (open stub, next-line, exit 0), complete with a receipt, and empty/502 refund surfacing; bin/atris.js help and dispatch sit next to youtube; skill Feature map in atris/skills/x-search/SKILL.md is login, x-search QUERY, one Apply, stop.
...
- **atris youtube notes:** `commands/youtube.js:1216` runs the bundled `scripts/det/ytnotes` rail with inherited stdio; default `atris youtube notes URL` is stdout only (`commands/youtube.js:1394`); `commands/youtube.js:599` files `atris/wiki/briefs/youtube-<id>.md` plus a claimable journal line only on `--save`; `commands/youtube.js:648` `ensureNotesApply` then writes a fill-this Apply sidecar (`youtube-<id>.apply.md`) and a `next: write one apply...` line when no receipt exists, and still exits 0 on the `--save` path; `commands/youtube.js:665` `unsaveYoutubeNotes` deletes the brief and apply sidecar if present and stays quiet when they are already gone; `commands/youtube.js:714` `ensureProcessApply` (`required: true`) refuses missing or stub-only Apply with one sentence and exit 2 before any `/agent/process_youtube` call; `commands/youtube.js:28` lists notes as ephemeral unless `--save`; `bin/atris.js:651` names the help line; `test/youtube.test.js:356` covers notes without `--save` writing nothing; `test/youtube.test.js:376` and `test/youtube.test.js:400` cover `--save` without Apply (open stub, next-line, exit 0) and `--save` + Apply receipt (complete, no next-line); `test/youtube.test.js:449` covers unsave.
- **atris youtube teach:** `commands/youtube.js:1961` `parseTeachArgs` takes `--section N` (1-based, default 1) and `--save`; `commands/youtube.js:2071` `extractTeachNumbers` keeps a number only with its unit or nearby claim; `commands/youtube.js:2101` `extractTeachMechanisms` keeps a named head (window/model/cycle) and drops quotes, swears, and caption crumbs; `commands/youtube.js:2140` `formatTeachLesson` prints those numbers, named mechanisms, one fail-able check, and `next: atris youtube teach "<url>" --section N+1`; `commands/youtube.js:2224` `extractTeachSource` pulls yt-dlp chapters plus English captions with no network in tests; `commands/youtube.js:2260` `runYoutubeTeach` prints stdout only unless `--save`. Exit 0 even without Apply. No `/agent/process_youtube`, no `--paid`, no credits. Does not collide with `atris teach`. Regression: `test/youtube-teach.test.js`.
...
- **File folder, just minted, no live mission:** `speakKeepWorkingMinute` when first-minute next is `atris do` (`"notes.md" is already yours` / `next: atris do`). Not factory `let it run`. A live running mission (`pickLiveLocalMission`) still gets the board. `--verbose` keeps the factory dump.
- **Help:** `bin/atris.js` (`showStatusHelp`) short-circuits `status --help` before workspace or business-status reads
- **Regression:** `test/first-minute.test.js` (`atris status in an empty folder talks first-talk, not init`; `atris now in an empty folder talks first-talk, not init`; `atris now in a folder with a file names the file, not init`; `atris status after minting a file folder talks keep-working, not factory let-it-run`); `test/dogfood-p0-safety.test.js` (`26c`, `26d`, `26e`, `26f`); `test/now-json.test.js`; `test/commands.test.js` covers `status --help` without `atris/` or `$HOME/.atris`
...
- File folder after mint, no live mission: same two spoken lines as the next do (`"notes.md" is already yours` / `next: atris do`)
- Default: Chief-of-staff summary (`Where we are` / `What is queued` / `What is blocking`) when a live mission is running or first-minute next is not `atris do`
- `--verbose` / `-v`: Legacy visual task board
- `--quick` / `-q`: One-line emoji summary, or the keep-working two lines after mint
- `--json`: Structured JSON (date, backlog, inProgress, completed, inbox, completions, lessons, team); empty-folder `--json` is first-talk refuse; post-mint keep-working `--json` is one-lap (`already yours` / `atris do`)
...
**Search:** `rg "statusAtris|showStatusHelp|speakNothingRunning|speakKeepWorkingMinute|isKeepWorkingMinute|status and analytics --help" bin/atris.js commands/status.js lib/first-minute.js test/commands.test.js test/first-minute.test.js test/dogfood-p0-safety.test.js`
...
- **File folder after mint, no live mission:** default spoken `atris recap` and `atris recap --json` reuse `speakKeepWorkingMinute` when first-minute next is `atris do` (same `"notes.md" is already yours` / `next: atris do` as status and the next do). Not factory `Generate MAP.md` ready to claim. A claimed non-seed task still recaps that work. A live running mission and `--verbose` / `--share` keep the recap report
- **Next:** one certified accept names `atris task accept <id>`; recap does not send humans to `atris task reviews` when that accept is the next move; keep-working recap names `atris do`
...
- **Regression:** `test/recap.test.js` (buckets, jargon ban, share format, projection fallback, empty workspace, empty-folder first-minute, certified-only needs-you, spoken default, verbose report, headless mixed board, post-mint claimed file not factory map.md, claimed non-seed still recaps); `test/first-minute.test.js` (`atris status after minting a file folder talks keep-working, not factory let-it-run` also locks recap and spoken `now`); `test/dogfood-p0-safety.test.js` (`26e`, `26f`)

**Search:** `rg "recapAtris|renderRecapMinute|speakKeepWorkingMinute|isKeepWorkingMinute|plainCheck|isCertifiedReview|isRealTestRunnerProof|classifyVerifier" commands/recap.js lib/first-minute.js lib/verifier-quality.js commands/task.js test/recap.test.js test/first-minute.test.js test/dogfood-p0-safety.test.js test/dogfood-p1.test.js`
...
- `statusAtris()` → `commands/status.js` (empty folder reuses speakNothingRunning; a file already here names that file; after do mints that file folder with no live mission, keep-working first-minute; a live mission still gets the board)
