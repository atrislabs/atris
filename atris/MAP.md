...
- **Regression:** `test/recap.test.js` (buckets, jargon ban, share format, projection fallback, empty workspace, empty-folder first-minute, certified-only needs-you, spoken default, verbose report, headless mixed board, post-mint claimed file not factory map.md, claimed non-seed still recaps); `test/first-minute.test.js` (`atris status after minting a file folder talks keep-working, not factory let-it-run` also locks recap and spoken `now`; `atris now after init --yes --minimal talks the claim, not factory now.md`); `test/dogfood-p0-safety.test.js` (`26e`, `26f`, `26g`)
