...
rg "voiceTriggers|Unknown command|Did you mean|SINGLE_WORD_NATURAL_INTENTS|unknownCommandToken|firstMinuteAlias|isFlagLikeAnswer|_start" bin/atris.js lib/known-commands.js lib/context-gatherer.js test/dogfood-p0-safety.test.js test/dogfood-pass2.test.js test/dogfood-pass4.test.js test/one-lap-router.test.js  # One-word start/go/audit/deploy and leftover _start no longer spawn runners; unmatched first tokens (`taks list`) and lone verbs (`build`, `fix`) exit 2; lone `atris test` is the first-minute alias, including `--json`; quoted sentences / task new / wish still create work; start --help is usage; flag titles never become tasks
...
rg "runCloudWish|runCloudWishGrant|cloudDispatch|cloudWishOptionError|--cloud|findActiveWishFlight|attached_to_existing_flight|resolveDefaultVerifier|deriveVerifyPlan|namedTestFiles|verifierBudgetWarning|wishBodyText|buildClarityQuestions|printQuestion|oneLap|lapOwnsDispatch|dispatchWishHeadlessly|printWishReceptionist|isWishDeskVerb" commands/wish.js lib/cloud-mission.js lib/wish-audit.js lib/wish-delegate.js lib/first-minute.js lib/default-verifier.js test/wish-cloud.test.js test/wish.test.js test/cloud-mission.test.js test/first-minute.test.js test/one-lap-router.test.js  # Wish dispatch: empty-folder bare wish and leftover sentences reuse speakFirstMinute like bare atris and do not mint wishes.jsonl; help stays usage; after init a plain sentence still files; vague intake repeats the complete wish and asks one short question grounded in its words; cloud wishes, grant --cloud records task_id on the same wish without a local builder re-audit, Check lines, scoped verifiers, suite-window warnings, duplicate local slices, and one-lap ownership stay intact
...
rg "namedUnrunProofCommand|unrunNamedProofCommandIssue|proof_command_not_run|requireRanNamedProofCommand" lib/task-proof.js commands/task.js test/task-proof.test.js  # Ready, done, step-to-ready, and API ready fail when --proof names npm test, node --test, or git diff --check and this process did not run that command; accept tests use --verify so the named command actually runs; headless, no prompt
...
rg "integrationsStatus|--global|scope: 'workspace'|gmailAccountStatePath|readGmailStickyAccount" bin/atris.js commands/integrations.js test/dogfood-p1.test.js # Integrations default workspace; --global for account hosts; Gmail sticky account helpers stay internal (used by gmail use / --account, not exported)
...
- After init: stay in the room. If first-minute next is `atris task claim|ready|accept`, review prints those same two spoken lines. Certified leads with the win and `next: atris task accept <id>`. Uncertified stays "still being checked", not needs-you. `nothing is waiting on you.` only when there is no claim/ready/accept next.
...
- **Placeholder MAP:** after init, `bin/atris.js` `interactiveEntry` keeps `atris test` and other requests on first-minute when `MAP.md` is placeholder or missing. Lone `atris test` is a thin first-minute alias at `bin/atris.js:1241` (`firstMinuteAlias`), including `--json`. The generate-map task is already yours or ready to claim. It does not print `BOOTSTRAP REQUIRED` or ask an agent to rewrite MAP as a wall. Empty folders with no `atris/` can still name init. Headless never prompts. Regression: `test/first-minute.test.js`
