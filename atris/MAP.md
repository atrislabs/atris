...
- **Runners & models:** `mission start --runner manual|claude|atris2|codex_goal [--model <id>]`. `claude` spawns the configured local runner binary (`ATRIS_RUNNER_BIN`, legacy `ATRIS_CLAUDE_BIN`, default `claude`) per tick via `spawnClaudeTick()`; model resolves from `mission.model` -> `ATRIS_RUNNER_MODEL` -> legacy `ATRIS_CLAUDE_MODEL` -> `opus` in `lib/runner-command.js`; shell worker paths (`autopilot`/`run`/horizon checks) use the same shared runner command and expose per-run `--runner-bin`, `--runner-template`, and `--runner-model` flags for runner swaps/model changes. GLM-shaped example: `ATRIS_RUNNER_BIN=glm-runner ATRIS_RUNNER_MODEL=glm-5.2 ATRIS_RUNNER_COMMAND_TEMPLATE='{bin} --model {model} --prompt-file {promptFile} {allowedToolsFlag}' atris run --once`. `atris2` runs each tick as one `/atris2/turn` on the AtrisOS backend via `runAtris2Turn()` (exported from `commands/probe.js`; default model `atris:fast`, stateless per tick — continuity lives on disk; no creds → mission pauses `auth-required`; receipt carries `result.atris2` with engine/tools_run/receipt_text). `codex_goal` publishes the goal for a live Codex session to pull. Regression: `test/mission-atris2-runner.test.js`, `test/mission-model-resolution.test.js`, `test/runner-command.test.js`, `test/autopilot-runner-model.test.js`
...
- **Phase executor:** `commands/autopilot.js:445` (executePhaseDetailed function) — runs `buildRunnerCommand()` (default Claude-compatible `claude -p`, configurable with `ATRIS_RUNNER_*` env or runner flags)
...
- **Flags:** `--auto` (no approval), `--iterations=N`, `--verbose`, `--dry-run`, `--runner-bin PATH`, `--runner-template CMD`, `--runner-model MODEL`
...
- **Flags:** `pulse install [--cadence "11,40 * * * *"] [--days 7] [--verify "npm test"] [--model opus|glm-5.2] [--runner-bin /path/to/runner] [--runner-template "{bin} ..."]` exports `ATRIS_RUNNER_*` and legacy aliases into cron; `pulse tick [--no-claude] [--no-verify] [--verify "<cmd>"]`; `pulse run [--max-ticks N]`
