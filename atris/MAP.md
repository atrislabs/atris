...
rg "ensureNowFile|renderDefaultNow|renderPortfolioNow|currentMissionMoveLine|missionFullBudgetRemainingSeconds|missionBudgetContinuationText|missionHumanStatusText|compactMissionCommand|countOpenWorkItems|countOpenTodoItems|countJournalCompletedReceipts|todayTaskReceiptLines|storeTaskReceipt|now --help" bin/atris.js commands/now.js commands/mission.js test/now.test.js test/mission-status.test.js test/commands.test.js  # now.md and mission status front doors: a selected mission names one next action and asks for its proof instead of another decision; full-budget missions keep shipping while JSON preserves lifecycle state; workspace/member/last-landing views agree and task receipts stay complete
...
- **Liveness surface:** `status` text output includes `missionHeartbeatLines()` — `last tick: <age> ago (#n, status, verifier outcome)` plus a due countdown for ordinary cadence missions or remaining time for active full-budget work; `buildTickPrompt()` injects member identity (read `atris/team/<owner>/MEMBER.md` + `SOUL.md`, log lessons to the member's dated log). Regression: `test/mission-heartbeat.test.js`
...
- **Complete mission names:** when a proof-ready objective begins with a complete colon-delimited clause under the digest budget, the review action uses that clause instead of a clipped three-word label; long unstructured objectives retain the safe label fallback. Regression: `test/autoland.test.js` (`digest and alarm compose in plain language`)
...
- **Plain result counts:** digest score and waiting headers say `clear results`, `clear next actions`, and `clear review actions` instead of exposing the internal `explained` gate term; the two-check claim remains exclusive to strict autoland. Regression: `test/autoland.test.js` (`digest and alarm compose in plain language`)
- **Exact digest window:** the digest header says `last 24 hours`, matching `acceptedInLastDay()`; accepted history comes from task truth instead of the compact eight-done-task projection, so a busy day is not undercounted. Regression: `test/autoland.test.js` (`digest and alarm compose in plain language`, `digest acceptance history is not capped...`)
- **Contextual digest expansion:** the expanded CLI digest points to the full result story already printed below; compact daily and live messages retain the command that opens it. Regression: `test/autoland.test.js` (`expanded digest points to the full story below...`)
...
- **Implementation:** `commands/land.js` (collectBoard, printBoard, reap, landSummary); the board separates total tracked, still in the air, stale, overdue, and landed counts so completed branches never inflate live work and aging work is visible before expiry; cleanup guidance names what will be cleared
