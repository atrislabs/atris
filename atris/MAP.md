...
- **Flags:** `pulse install [--cadence "11,40 * * * *"] [--days 7] [--verify "npm test"] [--runner-model opus|glm-5.2] [--model legacy-alias] [--runner-profile atris-fast] [--runner-bin /path/to/runner] [--runner-template "{bin} ..."]` exports `ATRIS_RUNNER_*`, `ATRIS_RUNNER_PROFILE`, and legacy aliases into cron; `pulse tick [--no-claude] [--no-verify] [--verify "<cmd>"]`; `pulse run [--max-ticks N]`
