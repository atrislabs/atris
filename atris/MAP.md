...
- **Workflow activation:** `commands/workflow.js` `atris plan` and `atris do` print the computed 0-shot route, first command, prompt/menu commands, and owner-gate boundary before the legacy Navigator/Executor workflow prompt so agents following `plan -> do -> review` still start from the current safe next move
