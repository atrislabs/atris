...
rg "function doAtris" commands/workflow.js     # Do command (line 865): empty folder reuses buildFirstMinute / freshMinuteJson like bare atris; files already here start first-talk via startFirstTalk / visibleWorkTitle (ready + claim, no next: atris do); PERSONA head stays first-minute after init; executor paste stays on --prompt/--verbose/--full; missing executor spec after init --minimal does not send you back to init
...
- Files already here, no `atris/`: bare `atris` still names the files and `next: atris do`. `atris do` starts from those files (`lib/context-gatherer.js` `startFirstTalk`, title from `visibleWorkTitle`): `{file} is ready` / `next: atris task claim <id> --as <person>`. A second `do` and bare `atris` after that match the claim/ready truth. It does not print `next: atris do`, and it does not dump a factory banner. Empty or hidden-only folders still name the talk command and do not mint.
