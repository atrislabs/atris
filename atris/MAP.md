...
rg "modelForMode|buildPayload|formatZeroShotMessage|buildAxZeroShotPacket|async function chat|postTurn" ax test/cli-smoke.test.js  # ax Atris2 local coding-agent CLI: Fast/Pro/Code Fast modes, 0-shot handoff context, SSE streaming, workspace_path, chat history, doctor/help
...
- **Fast/pro model surface:** `ax` prepends compact 0-shot handoff context to Fast, Pro, and Code Fast turn payloads from Atris workspaces, including first command, owner-gate boundary, model/horizon buckets, and direct selector prompt commands without changing the backend payload schema
