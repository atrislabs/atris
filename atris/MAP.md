...
rg "runImprove|summarizeImproveResponse|summarizeLocalMissionRun|shouldFallbackLocal|summarizeTickHistory|appendTickToJournal|headlessOnly" commands/improve.js commands/mission.js bin/atris.js test/improve.test.js test/mission-status.test.js  # Improve command: paid RL tick (POST /api/improve, deducts credits), scorecard row + journal, `improve history` compounding view, --member attribution; local fallback skips caller-session placeholders, runs exactly one headless due mission tick, and only writes receipts after its verifier passes
