...
- **Proof-ready digest action:** `digestNextMoves()` preserves each move kind and short label; a `mission_ready` row renders one identifiable human review action without truncating the mission objective or suggesting a new owner. Regression: `test/autoland.test.js` (`digest and alarm compose in plain language`)
...
- **Digest heartbeat state:** tick receipts carry explicit `digest_due`; the human summary says `sent`, `saved`, or `not due`, so a due report without a configured phone is never reported as if it did not run. Regression: `test/autoland.test.js` (`heartbeat digest status distinguishes...`)
