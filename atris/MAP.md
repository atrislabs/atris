...
- **Runner profile flag precedence:** `applyRunnerFlags()` (`bin/atris.js`) treats `--runner-profile` as a clean per-command runner overlay by clearing inherited `ATRIS_RUNNER_*` / `ATRIS_CLAUDE_*` bin, template, and model env unless the same command also passes the explicit `--runner-bin`, `--runner-template`, or `--runner-model` override. Regression: `test/autopilot-runner-model.test.js`
