...
rg "ensureNowFile|renderDefaultNow|renderPortfolioNow|currentMissionMoveLine|compactMissionCommand|countOpenWorkItems|countOpenTodoItems|countJournalCompletedReceipts|todayTaskReceiptLines|now --help" bin/atris.js commands/now.js test/now.test.js test/commands.test.js  # now.md front door: local date, complete current-mission clause, compact untruncated next command, projection-backed open work counts, task receipts rendered as complete landed results with human proof states, portfolio signals, non-mutating help
