...
- `commands/sign.js` (`signCommand`, `installSignHook` `:26-40`) — co-author trailer on every commit in an atris workspace: prepare-commit-msg hook appends `Co-authored-by: Atris <noreply@atris.ai>` when `atris/` exists (skips merge/squash, dedupes on `^co-authored-by: atris`); `atris sign` / `sign off` / `sign status`; auto-installed by `atris init`, marked-block removal only; this repo gets it via `scripts/prepare-commit-msg` + npm prepare
