...
rg "xSearchCommand|parseXSearchArgs|runXSearch|formatXSearchResult|ensureXSearchApply|xSearchHasResults|/x-search/search|research-person|Usage: atris x-search|ensureApply|writeApplyStub" commands/x-search.js lib/apply-gate.js bin/atris.js lib/known-commands.js test/x-search.test.js test/billed-command-auth.test.js atris/skills/x-search/SKILL.md  # X search CLI: commands/x-search.js uses ensureBilledCommandAuth (scope x-search) then apiRequestJson; on a missing or expired user wall it mints via POST /auth/agent-token, stores the scoped token, and retries; no stored JWT fails with one sentence and no /auth/cli wall; search posts /x-search/search with query, limit, days_back; person posts /x-search/research-person; after a successful pull with results, commands/x-search.js:289 ensureXSearchApply requires one Apply sidecar via lib/apply-gate.js:81 (x-search-<slug>.apply.md with change + receipt) or writes a claimable fill-this stub and exits 2; empty or failed search does not owe an Apply; help and citation print covered in test/x-search.test.js with mocked apiRequestJson; test/x-search.test.js:375 and test/x-search.test.js:397 cover incomplete without Apply and complete with a receipt; bin/atris.js help and dispatch sit next to youtube; skill Feature map in atris/skills/x-search/SKILL.md is login, x-search QUERY, one Apply, stop.
