...
Four. **Staleness.** Features: for each `atris/features/<name>/idea.md`, read the `Last Updated:` or `Created:` line; flag if older than 60 days and the Status line does not contain complete, shipped, live, archived, parked, retired, or superseded. Folders under `atris/features/_archive` and `_templates` never count. Members: for each `atris/team/<name>/` folder with a MEMBER.md, exclude members with frontmatter `status: retired`, `parked`, or `archived`; for the rest, find the newest file under `logs/`; flag if older than 30 days or no logs at all. Print counts and the ten oldest.
