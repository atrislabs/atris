...
The command handles `atris init [--force] [--yes|-y]`. Help flags print usage without filesystem side effects. Normal execution refuses to run inside an existing `atris/` directory, inside a parent business workspace, or in a folder with `.atris/business.json` unless `--force` is present. `--yes`, `-y`, and non-TTY execution skip the interactive context gatherer after scaffolding.

## What Init Creates

- Core workspace: `atris/`, `atris/atris.md`, `atris/PERSONA.md`, `atris/GETTING_STARTED.md`, `atris/FOR_AGENTS.md`, `atris/MAP.md`, `atris/TODO.md`, `atris/now.md`, `atris/lessons.md`, and dated logs.
...
- Git attribution: in git workspaces, init installs the Atris co-author hook and skips quietly outside git.
...
The current generated `AGENTS.md` adapter also names `.atris/state/tasks.projection.json` as the readable UI/agent projection, separates functional owners from engine runtimes (`executed_by`), gives mission-shaped user intent priority over normal task picking, and tells agents to mirror `goal.visible_goal` after `atris mission goal --json`.

Generated adapters carry the operator reporting contract too: report capabilities in a one-screen result, and pair `atris task ready --proof` with a plain `--result` sentence.

Worktree guidance is generated too: use the current checkout for small clean fixes; reserve `atris worktree start` for dirty launchers, parallel agents, long proof, risky edits, or release/publish work; use `atris worktree cleanup` / `--apply` to remove clean merged worktrees.
