...
The loop can be walked by hand (`atris plan` -> `atris do` -> `atris review`) or reached through mission front doors:

- **`atris run`** - Starts or resumes one bounded mission pursuit, runs a tick, and completes when proof passes. `--legacy` retains the old autonomous `plan -> do -> review` chain.
- **`atris autopilot`** - Keeps selecting mission or member legs until `atris autopilot stop`. `--legacy` retains the old approval-checkpoint loop.

The default front doors no longer mean the same stage choreography, but both still depend on declared verification and durable proof. Legacy chaining changes who presses enter, not what counts as proof.

`atris pulse` sits outside the stage model: it installs or runs the durable overnight heartbeat, but the work it wakes still has to close through the same task, mission, verifier, and proof gates.
