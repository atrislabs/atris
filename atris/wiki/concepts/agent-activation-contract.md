...
`atris/CLAUDE.md` now also carries an explicit agent contract: before edits, claim or create one small task with `atris task` and write the goal/files/done/check contract into task dialogue; after edits, move proof-backed work to Review with `atris task ready <id> --proof "..."`.

Task owners are functional or feature members, not engine names. Use owners such as `task-planner`, `architect`, `mission-lead`, `validator`, `launcher`, or a feature owner for assignment; record coding-agent runtimes like Codex or Claude in `executed_by` when that distinction matters.

Proof-ready and human accept are separate gates: an agent's native goal can complete once proof is in Review, but only a human-run `atris task accept <id>` marks the task Done and awards AgentXP. Always-on agents complete the native goal after proof is in Review, then continue the mission loop; they must not accept XP for themselves.

Mission-shaped user intent wins before normal task selection. If the user asks for `atris mission run ...`, execute that first, run `atris mission goal --json`, and mirror `goal.visible_goal` into the live runtime goal before choosing unrelated work. Work that should outlive the chat runs through `atris mission` (start with a verifier, bounded step, `mission tick --verify`, then complete or continue).

Default to the current checkout for small, clean, single-agent fixes. Use worktrees for dirty launchers, parallel agents, long proof, risky edits, or release/publish work, and clean old merged worktrees with `atris worktree cleanup`.
...
2026-07-12 source check: `atris/CLAUDE.md` remains aligned and now makes result-first, one-screen reporting part of the agent contract after verified work.
