...
- Composition: autopilot, member wake/tick/review, business onboarding, business start/share surfaces, and app/workflow surfaces.
...
2026-07-12 source refresh: the local capability surface includes durable
heartbeat (`atris pulse`), live operating views (`atris now`, `radar`, `ctop`,
`recap`), native Codex goal cleanup (`atris codex-goal`), structured learnings
(`atris learn`), and explicit cloud/business sync (`atris pull`, `push`, `live`).
These are still capability primitives until a verified composition uses them for
a customer or operator outcome.

The business source now exposes a more complete inspectable composition: sparse
intake becomes a brief, first-loop hypothesis, one-pager, starter task, and
recorded recap; `business start|check|ready` renders operating state, while
`business share|handoff` packages it for a collaborator. Those flows make the
composition leg concrete, but they do not yet create real-time product
composition or an automatic roadmap from capability gaps.
