...
- `atris business quickstart` / `guide`: start-card for the business operating loop, including create, onboard, record, share, and sync.
- `atris business init "<name>"` / `workspace`: recommended path; creates the cloud business and local canonical workspace. `--template` can seed common shapes such as `saas`, `agency`, `ecommerce`, `content`, or `restaurant`.
- `atris business create "<name>"`: cloud-only by default; add `--workspace`, `--here`, or `--root <dir>` for local scaffold.
- `atris business add <slug>`: register an existing cloud business locally.
...
- `atris business remove <slug>` / `rm`: unregister a business locally without deleting the cloud business.
...
- `atris business deploy <slug>` / `push <slug>`: legacy push path for `atris/business/<slug>/` content.
...
Name-taking commands intercept `--help`, `-h`, `help`, and `-?` before creation, and creation rejects flag-like or accidental help names. `business doctor` also detects old help-shaped cloud rows and cache bindings; `--fix` removes only the safe local cache entries.
