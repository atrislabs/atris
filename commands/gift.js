 ...
 * complete. The authorization is minted by a human-authenticated call, and the
 * settle step is gated behind an explicit y/N (or --yes) money-lane confirm, so
 * the CLI can never move money without a person naming the amount first.
 */
...
/**
 * Money-lane gate. Settling a gift card moves real money, so a human must say
 * yes to the exact amount and recipient first. `--yes` is the explicit override
 * for a person who already decided; without it, a non-interactive run (no TTY)
 * refuses rather than charging silently in a script or an agent loop.
 */
async function confirmPurchase(prompt, { yes = false, input = process.stdin, output = process.stdout } = {}) {
  if (yes) return true;
...
async function buyAirbnb(args) {
  ...
  // Money-lane gate: a person confirms the exact charge before any money moves.
  const chargeUsd = ((total?.amount || 0) / 100).toFixed(2);
  const ok = await confirmPurchase(
    `\n${c.bold(`Charge $${chargeUsd} and send an Airbnb gift card to ${to}?`)} (y/N) `,
    { yes: flags.yes === true || flags.y === true }
  );
  if (!ok) {
    return fail(
      'Not confirmed. Nothing was charged.',
      'Re-run and answer y, or pass --yes to confirm non-interactively.'
    );
  }
...
${c.bold('atris gift')}  buy a gift card from the command line
  ...
  Flags for ${c.bold('airbnb')}:
    ...
    --yes, -y   confirm the charge non-interactively

  ${c.dim(`API: ${API_BASE}`)}
  ${c.dim('ATRIS_API_KEY is required to authorize payment.')}
  ${c.dim('The charge is confirmed by a human (y/N) or --yes before any money moves.')}
`);
  return 0;
}
...
module.exports = { giftCommand, confirmPurchase };
