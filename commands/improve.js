...
function buildLocalFallbackCommand(bin, opts = {}) {
  ...
  if (opts.model) argv.push('--runner-model', String(opts.model));
  return { cmd, argv };
}

function runLocalFallback(opts = {}) {
  const { cmd, argv } = buildLocalFallbackCommand(resolveAtrisBin(), opts);
